 ...
 */
function isLocalServer(domain: string): boolean {
  return (
    domain === "localhost" ||
    domain === "127.0.0.1" ||
  ...
  try {
    ...
    for (const currentDomain of domainsToTry) {
      try {
        ...
        console.debug(
          "[favicon] Attempting to fetch favicon metadata for:",
          currentDomain,
          "from:",
          faviconApiUrl
        );
        ...
        try {
          ...
          console.debug(
            "[favicon] Retrieved favicon metadata for:",
            currentDomain,
            "source:",
            data.source
          );
          ...
          if (!imageResponse.ok) {
            console.debug(
              "[favicon] Failed to fetch favicon image for",
              currentDomain,
              "status:",
          ...
          if (data.source === "default") {
            ...
            console.debug(
              "[favicon] Found default favicon for:",
              currentDomain,
          ...
          console.debug(
            "[favicon] Successfully retrieved non-default favicon for:",
            currentDomain,
            "source:",
            data.source,
            "size:",
        ...
        console.debug(
          "[favicon] Error processing domain",
          currentDomain,
          ":",
    ...
    console.debug(
      "[favicon] No non-default favicon found for:",
      serverUrl,
