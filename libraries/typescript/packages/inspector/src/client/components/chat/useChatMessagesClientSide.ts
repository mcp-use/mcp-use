  ...
  const sendMessage = useCallback(
    async (userInput: string) => {
      ...
      try {
        ...
        const resolveOAuthTokens = async (
          serverUrl: string,
          authCfg?: AuthConfig
        ) => {
        ...
        const addAuthHeaders = (
          ...
          if (
            ...
            authCfg.type === "oauth" &&
            authCfg.oauthTokens?.access_token
          ) {
            const tokenType = authCfg.oauthTokens.token_type
              ? authCfg.oauthTokens.token_type.charAt(0).toUpperCase() +
                authCfg.oauthTokens.token_type.slice(1)
              : "Bearer";
        ...
        if (mcpConfig) {
          ...
          for (const [serverName, serverCfg] of Object.entries(
            ...
            const authConfigResolved = await resolveOAuthTokens(
              serverUrl,
              authConfig || undefined
            );
            configWithAuth.mcpServers[serverName] = addAuthHeaders(
              serverCfg,
              authConfigResolved
            );
          }
          ...
          const serverConfigWithAuth = addAuthHeaders(
            serverConfig,
            authConfigResolved
          );
        ...
        try {
          ...
          if (clientInternal.sessions) {
            ...
            if (sessionIds.length > 0) {
              ...
              if (session?.tools) {
                ...
                console.log(
                  "[useChatMessagesClientSide] Stored metadata for tools:",
                  Array.from(toolsMetadataRef.current.keys()),
                  "sample:",
                  tools[0]
                );
              }
            ...
            if (agentClient.sessions) {
              ...
              if (sessionIds.length > 0) {
                ...
                if (session?.tools) {
                  ...
                  console.log(
                    "[useChatMessagesClientSide] Stored metadata from agent for tools:",
                    Array.from(toolsMetadataRef.current.keys())
                  );
                }
              }
          ...
          console.warn(
            "[useChatMessagesClientSide] Failed to load tools metadata:",
            error
          );
        }
            ...
            if (toolPart && toolPart.toolInvocation) {
              ...
              if (typeof result === "string") {
                try {
                  ...
                  console.warn(
                    "[useChatMessagesClientSide] Failed to parse result:",
                    error
                  );
                }
              }
                  ...
                  try {
                    ...
                    if (parsedResult?.structuredContent) {
                      ...
                      if (firstResult.output?.value?.structuredContent) {
                        structuredContent =
                          firstResult.output.value.structuredContent;
                    ...
                    if (
                      ...
                      if (toolPart.toolInvocation) {
                        ...
                        console.log(
                          "[useChatMessagesClientSide] Updated result:",
                          updatedResult
                        );
                        ...
                        console.log(
                          "[useChatMessagesClientSide] Messages state updated with resource"
                        );
                      }
                    } else {
                      console.warn(
