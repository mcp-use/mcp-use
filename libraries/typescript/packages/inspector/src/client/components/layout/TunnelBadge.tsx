...
export function TunnelStartButton({
  ...
  if (isTunnelStarting) {
    return (
      <button
        disabled
        className="flex items-center gap-2 h-8 px-3 bg-violet-50 dark:bg-violet-950/40 border border-violet-200 dark:border-violet-800 rounded-full opacity-75 cursor-wait shrink-0"
      >
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center gap-2 h-8 px-3 border rounded-full transition-colors shrink-0",
        canStart && !loadingDev
