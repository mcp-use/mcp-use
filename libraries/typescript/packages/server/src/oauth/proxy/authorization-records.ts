...
export interface OAuthProxyJsonObject {
  /** Named JSON-serializable identity claim or provider-specific nested value. */
...
export interface UpstreamGrantRecord {
  ...
  readonly createdAt: number;
  readonly expiresAt: number;
  /** Optional hard grant lifetime, retained while a pending code uses a shorter lease. */
...
function validateOAuthProxyRecord(
  ...
  switch (expectedKind) {
    ...
    case "upstream-grant":
      ...
      timestamp(record.createdAt);
      timestamp(record.expiresAt);
      optionalTimestamp(record.absoluteExpiresAt);
      return;
