...
import type {
  ...
  UpstreamTokenRevocationRequest,
  UpstreamTokenSet,
...
export class UpstreamOAuthClient {
  ...
  async refreshToken(
    ...
    if (request.scope !== undefined) {
      supplementalParams.scope = normalizeScope(request.scope)!;
    }
    const extraParams = validateExtraParams(
      request.extraParams,
      TOKEN_RESERVED_PARAMS,
    ...
    try {
      const tokens = await refreshAuthorization(this.#authorizationServerUrl, {
        ...
        fetchFn: this.#sdkFetch(
          "token refresh",
          { ...supplementalParams, ...extraParams },
          [...this.#clientAuthenticationSecrets(), refreshToken],
          request.signal
        ),
      });
  ...
  /** Revokes a token using the small proxy-owned RFC 7009 adapter. */
  async revokeToken(request: UpstreamTokenRevocationRequest): Promise<void> {
    ...
    await this.#addClientAuthentication(headers, params);
    const extraParams = validateExtraParams(
      request.extraParams,
      REVOCATION_RESERVED_PARAMS,
    ...
    await this.#http.postForm({
      endpoint: this.#revocationEndpoint,
      params,
      headers,
      operation: "token revocation",
      sensitiveValues: [...this.#clientAuthenticationSecrets(), token],
