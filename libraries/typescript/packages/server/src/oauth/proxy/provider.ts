...
export interface OAuthProxyUser {
  /** Stable subject identifier verified by the upstream provider. */
  readonly id: string;
  /** Email address, when present in the verified identity. */
  readonly email?: string;
  /** Display name, when present in the verified identity. */
  readonly name?: string;
  /** Preferred username, when present in the verified identity. */
  readonly username?: string;
  /** Profile-picture URL, when present in the verified identity. */
...
export interface OAuthProxyOptions<TUser = OAuthProxyUser> extends Omit<
  OAuthResourceOptions,
  "scopesSupported"
> {
  ...
  /** How the fixed upstream application authenticates at its token endpoint. */
  readonly tokenEndpointAuthMethod: OAuthProxyTokenEndpointAuthMethod;
  /** Upstream application secret; required for secret methods and forbidden for `none`. */
 ...
 *
 * @param options - Upstream endpoints, credentials, identity verification and storage.
 * @returns A provider that installs broker routes when the MCP server mounts.
 * @remarks
 * The default store is process-local and loses sessions on restart. Use a
 * serializable encrypted persistent store for multi-instance deployments.
 * Downstream MCP credentials are opaque local tokens, never upstream tokens.
 */
    ...
    bind(resource) {
      ...
      const tokenVerifier: OAuthTokenVerifier = {
        async verifyAccessToken(token: string): Promise<AuthInfo> {
          if (!/^[A-Za-z0-9_-]{43}$/u.test(token)) {
            throw invalidToken("Invalid OAuth proxy access token");
          }
          const validated = await core
            .validateAccessToken(token, canonicalResource.href)
            .catch((error: unknown) => {
              if (
              ...
              // Storage/configuration failures remain server errors; only a
              // rejected credential should trigger client reauthorization.
              throw error;
            });
          return {
            token,
      ...
      return {
        ...
        sensitivePaths: [
          ...
          "revoke",
          "token",
        ].map((path) => new URL(endpoint(path)).pathname),
