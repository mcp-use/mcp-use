...
export class UpstreamOAuthHttpClient {
  ...
  /** Executes one manual-redirect, timeout- and size-bounded form POST. */
...
function replacePercentEncodedVariant(value: string, variant: string): string {
  ...
  let cursor = 0;
  while (true) {
    const index = normalizedValue.indexOf(normalizedVariant, cursor);
    if (index === -1) {
      chunks.push(value.slice(cursor));
      return chunks.join("");
    }
    chunks.push(value.slice(cursor, index), "[REDACTED]");
