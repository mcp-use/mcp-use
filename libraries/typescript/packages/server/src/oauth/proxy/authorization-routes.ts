...
export interface OAuthProxyUpstreamResourceContext {
  /** Upstream grant operation for which the resource is being selected. */
  readonly phase: "authorization" | "refresh";
  /** Canonical MCP resource; only an explicitly mapped value is sent upstream. */
  readonly localResource: string;
  /** Locally approved MCP scopes, independent of the upstream scope namespace. */
  readonly scopes: readonly string[];
  /** Previously verified identity, when selecting a resource for refresh. */
...
export interface OAuthAuthorizationRoutesOptions {
  ...
  /** Scopes requested upstream; defaults to local scopes, or omit scope with an empty array. */
  readonly upstreamScopes?: readonly string[];
  /**
   * Verifies the upstream access token and returns persistable trusted identity
   * data. A string `sub`, when supplied, must remain unchanged across refresh.
   */
...
async function authorizePost(
  ...
  const params = strictParams(
    form,
    ["transaction_id", "csrf_token", "decision"],
    true
  );
  if (
  ...
  const browser = requireBrowserBinding(request, options);
  ...
  const authorization = await options.upstreamClient.createAuthorizationRequest(
    {
      ...
      ...(options.upstreamScopes.length === 0
        ? {}
  ...
  // This response follows a browser form POST. 303 is explicit that the
  // provider authorization URL must be loaded with GET; 302 leaves method
  // rewriting ambiguous and some clients can resubmit the one-time consent.
  return redirect(authorization.url.toString(), 303);
}
...
async function callback(
  ...
  // RFC 6749 requires clients to ignore unrecognized response parameters.
  // Provider extensions never affect trust, while duplicate known fields fail.
  const callbackParams = strictParams(
    url.searchParams,
  ...
  try {
    ...
    reportOperationalFailure("callback", error);
    return downstreamRedirect(callbackState.redirectUri, {
      error:
  ...
  if (params.grant_type === "authorization_code") {
    ...
    const scopes =
      params.scope === undefined
        ? undefined
        : parseScope(params.scope, options.scopeSet);
  ...
  try {
    ...
    const updatedTokens =
      tokens.scope === undefined && attempt.upstreamTokens.scope !== undefined
        ...
        : tokens;
    const userPayload = await options.verifyToken(tokens.accessToken);
    const previousSubject = attempt.userPayload["sub"];
    if (
      typeof previousSubject === "string" &&
      userPayload["sub"] !== previousSubject
    ) {
      // A refresh must not silently turn an existing local grant into access
      // for another account. Custom payloads can opt in by normalizing `sub`.
      throw new RouteOAuthError("invalid_grant", 400);
    }
...
function browserBinding(
  ...
  const isSecure = options.issuer.protocol === "https:";
  // __Host- cookies must be Secure, omit Domain, and use Path=/, preventing
  // sibling subdomains from planting a known browser-binding cookie.
  const path = isSecure ? "/" : options.prefix;
  const secure = isSecure ? "; Secure" : "";
  return {
    value,
    setCookie: `${name}=${value}; Path=${path}; HttpOnly; SameSite=Lax${secure}`,
  };
}

function browserCookieName(options: ResolvedOptions): string {
  return options.issuer.protocol === "https:"
    ? `__Host-${BROWSER_COOKIE}`
    : BROWSER_COOKIE;
}

function requireBrowserBinding(
  request: Request,
  options: ResolvedOptions
): string {
  const value = readCookie(
    request.headers.get("Cookie"),
    browserCookieName(options)
  );
...
function strictParams(
  ...
  for (const [key, value] of params) {
    if (ignoreUnknown && !allowedSet.has(key)) continue;
...
function consentHtml(input: {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Authorize MCP client</title></head><body><main><h1>Authorize ${escapeHtml(input.clientName)}</h1><p>Client <code>${escapeHtml(input.clientId)}</code> is requesting access to <code>${escapeHtml(input.resource)}</code>.</p><p>After approval, access returns to <code>${escapeHtml(input.redirectUri)}</code>. Only continue if you trust this address.</p><ul>${scopes}</ul><form method="post" action="${escapeHtml(input.action)}"><input type="hidden" name="transaction_id" value="${escapeHtml(input.transactionId)}"><input type="hidden" name="csrf_token" value="${escapeHtml(input.csrfToken)}"><button type="submit" name="decision" value="approve">Allow</button><button type="submit" name="decision" value="deny">Deny</button></form></main></body></html>`;
}
...
function routeFailure(error: unknown): Response {
  ...
  if (error instanceof UpstreamOAuthError) {
    reportOperationalFailure("request", error);
...
function reportOperationalFailure(phase: string, error: unknown): void {
  const upstream = error instanceof UpstreamOAuthError ? error : undefined;
  const route = error instanceof RouteOAuthError ? error : undefined;
  if (upstream?.code === "access_denied") return;
  if (route !== undefined && route.status < 500) return;
  const code = upstream?.code ?? route?.code ?? "internal_error";
  const status = upstream?.status ?? route?.status;
  console.error(
    `[mcp-use] OAuth proxy ${phase} failed (${code}${status === undefined ? "" : `, HTTP ${status}`})`
  );
}
