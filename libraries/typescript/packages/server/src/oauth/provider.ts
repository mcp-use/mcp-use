...
export interface OAuthProviderBinding<TUser> {
  ...
  /** Exact provider-owned paths whose request/response payloads must never be logged. */
  readonly sensitivePaths?: readonly string[];
}

/** OAuth provider whose metadata and verifier factory are known up front. */
...
export interface ResourceBoundOAuthProvider<TUser> {
  /** Canonical public MCP resource override used before binding. */
...
/** OAuth provider accepted by the mcp-use server constructor. */
export type ServerOAuthProvider<TUser> =
  | OAuthProvider<TUser>
...
export interface BoundOAuthProvider<TUser> {
  ...
  readonly middleware?: FetchMiddleware;
  /** Snapshotted provider paths excluded from payload logging before shared middleware. */
  readonly sensitivePaths?: readonly string[];
}
