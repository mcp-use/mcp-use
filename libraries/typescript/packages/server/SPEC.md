...
- Official v2 SDK only, pinned exact while in beta. No v1 (`@modelcontextprotocol/sdk`) imports.
...
- Schemas are typed against the SDK's [Standard Schema](https://standardschema.dev/) contracts, not zod: `schema`/`outputSchema` accept any `StandardSchemaWithJSON` (validation + JSON Schema conversion — zod v4, ArkType, Valibot, …); `completable()` needs only `StandardSchemaV1`. Both types are re-exported. Zod is **not** a dependency of this package (devDep only, for tests); it stays the documented example library and ships anyway as the SDK's own dependency.
...
const server = new MCPServer({
  ...
  host: "127.0.0.1",           // optional; "0.0.0.0" + allowedHosts for public
  allowedHosts: undefined,     // optional, e.g. ["api.example.com"]
  allowedOrigins: undefined,   // optional origin hostnames
});
...
5. `listen()` binds `127.0.0.1` by default (old package: `0.0.0.0`) and Host/Origin validation is on: localhost-class binds get DNS-rebinding protection automatically (from `createMcpHonoApp`); public serving is an explicit `host: "0.0.0.0"` + `allowedHosts` opt-in. Secure-by-default posture inherited from the official adapter.
...
**Examples** (`examples/vercel`, `examples/railway`): the two deployment doors, each verified end-to-end. Vercel = serverless via `getHandler()` exported as `export default { fetch }` from an `api/` function, `allowedHosts`/`allowedOrigins` derived from `VERCEL_*` env vars. Railway = long-lived `listen()` on `0.0.0.0` + `allowedHosts: [RAILWAY_PUBLIC_DOMAIN]`, SIGINT/SIGTERM → `close()`.

## Later phases (each gets its own scope + delta notes before work starts)

- **Phase 2 — serving hardening:** auth seam (`authInfo` via `handler.fetch` → `ctx.http.authInfo`), mounting into a user's existing Hono app (validation middleware guidance), stdio serving decision (`serveStdio` works off the same factory), expose the underlying `McpServerFactory` (`server.factory()`) so any official adapter can consume it. Plus DX debts found building the examples: (1) `listen()`'s returned `url` is hardcoded to `localhost` — wrong for public binds; (2) setting `allowedHosts` *replaces* the localhost allowlist (localhost then 403s) and leaves Origin validation on its localhost default unless `allowedOrigins` is also set — document loudly or unify the defaulting; (3) no diagnostic when `basePath` drifts from where the handler is actually mounted (silent 404) — warn at `getHandler()` time; (4) a helper/recipe for PaaS host derivation (`VERCEL_URL`, `RAILWAY_PUBLIC_DOMAIN`, …); (5) document SIGINT/SIGTERM → `close()` as the container-platform integration pattern.
