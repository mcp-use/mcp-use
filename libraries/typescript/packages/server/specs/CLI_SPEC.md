...
The framework bin is a tiny proxy to the prebuilt `@mcp-use/cli` package. `@mcp-use/cli` exposes a side-effect-free programmatic `main(argv, { frameworkVersion })` entry: the framework bin injects the version compiled from the `mcp-use` manifest, while the standalone CLI bin uses the `@mcp-use/cli` manifest version. Both packages retain the public `mcp-use` executable for compatibility. If npm links the CLI package's bin when both packages are installed, that bin resolves the installed framework manifest without evaluating the framework and reports its version, making the collision behaviorally harmless. The CLI dispatches every substantial command with `await import(...)` to a real sibling chunk built with code splitting enabled. At minimum the CLI output has distinct chunks for `start`, `dev`, `build`, cloud identity, organizations, servers, deployments, deploy, client, and screenshot.
...
On file change (only files in the entry's module graph count): Vite invalidates and dev schedules a project reconciliation. Server and structural view events share one monotonic revision stream; each candidate re-imports the entry and primes it from an immutable view snapshot captured for that revision. A candidate superseded while it is being prepared is discarded before it can swap the handler, publish notifications, or report a terminal failure. The newest settled candidate swaps the handler reference only after it mounts successfully. There is no registration diffing: the next request hits the new handler, which is correct by construction under the stateless model. Every handler generation shares one process-scoped SDK `ServerEventBus`; after a successful swap, dev publishes `tools/list_changed`, `prompts/list_changed`, and `resources/list_changed`. Modern clients with an open `subscriptions/listen` stream therefore refetch the authoritative lists from the new handler. Publishing all three is deliberate invalidation, not change detection — the protocol carries no delta, and avoiding schema/function comparison keeps server reload independent of registry internals. A failed latest generation keeps the old handler and publishes nothing. Stateless legacy clients receive no push but remain correct on their next manual list request.
...
- **Tunnel:** `--tunnel` starts a public tunnel as soon as the HTTP listener is bound. The framework connects directly to the first-party relay using Node's built-in WebSocket client and proxies the bound local HTTP port in-process; it never launches a native binary, package runner, or separately installed tunnel package. The reservation credential is authenticated in the first encrypted application message because the standards-based WebSocket API does not expose arbitrary upgrade headers; unauthenticated sockets cannot proxy traffic and expire after 10 seconds. Request/response bodies and public WebSocket messages use bounded binary frames while control messages remain size-limited JSON. Public WebSocket upgrades are multiplexed by connection ID to the local listener, preserving Vite's real view-HMR channel through the public tunnel. The inspector UI can also start/stop the same tunnel manager at runtime through dev-only API routes (below) without restarting the dev process.
...
- **Shutdown:** SIGINT, SIGTERM, and the programmatic abort signal stop accepting new requests, terminate active MCP subscription streams and Vite HMR WebSockets, release the tunnel, and resolve successfully without waiting for clients to disconnect themselves.
...
4. **Inspector:** `--with-inspector` lazily imports the bundled `@mcp-use/inspector`, mounts it at `${basePath}/inspector` on the same listener, and leaves the build output and manifest unchanged. The opt-in prints the mounted Inspector URL on startup. The default start path neither imports nor exposes Inspector routes, and stays silent about the Inspector rather than adding a line most production starts do not need. The listener continues through `MCPServer.listen()` so its Host validation and OAuth resource initialization are identical in both modes; production Inspector proxy routes disallow loopback targets.
