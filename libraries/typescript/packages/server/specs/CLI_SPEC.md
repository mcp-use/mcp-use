...
`buildId` is a random hex id, `createdAt` is an ISO timestamp, and `inspector` records whether `--with-inspector` was passed. `views` contains the mode-neutral registry for runtime adapters; `start` consumes `entryPoint` and relies on the generated wrapper's embedded copy. By default, view entries reference external asset paths. `mcp-use build --inline` instead embeds each view's bundled JavaScript and CSS so `resources/read` returns them directly in the HTML document. There is no `--no-inline`; omitting `--inline` keeps the default external-assets mode. Every build copies project-root `public/` into `build/views/public/` when present, including tool-only servers and inline builds whose views reference public files.
