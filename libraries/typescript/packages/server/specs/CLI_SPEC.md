...
- **Tunnel:** `--tunnel` starts a public tunnel after the HTTP listener binds. The shared `@mcp-use/tunnel` package forwards bounded HTTP and WebSocket traffic through the managed relay without launching a native binary or package runner. The Inspector can start or stop the same manager through its dev-only routes.
...
Tunnel state is stored with owner-only permissions under `.mcp-use/state/`. The relay API origin can be overridden with `MCP_USE_WS_RELAY` or the standalone CLI's `--relay` option.
