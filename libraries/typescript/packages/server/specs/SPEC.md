...
Greenfield framework on the official v2 SDK (`@modelcontextprotocol/server`, stateless 2026-07-28 protocol), with its server API and runtime built in `packages/server`. The framework exposes the complete first-party CLI through its `mcp-use` executable, while `packages/cli` owns the CLI implementation and toolchain.
...
- One framework install. `mcp-use` ships the server runtime and exposes the complete first-party CLI through its dependency on `@mcp-use/cli`: `dev`, `build`, `start`, cloud auth, organizations, servers, deployments, deploy, client, and screenshot (`CLI_SPEC.md`). `@mcp-use/cli` owns the implementation and toolchain. Each substantial command is a genuine lazy chunk; `dev` and `build` are separate chunks, and neither `start` nor a library export may statically evaluate Vite or unrelated command code.
- Vite is a regular dependency of `@mcp-use/cli`, which is a regular dependency of `mcp-use`, so `npm install mcp-use` is sufficient for `mcp-use dev` and `mcp-use build`. `@vitejs/plugin-react` follows the same ownership for views. `react` and `react-dom` remain optional peers owned by view applications.
...
- **Build/dev/start CLI: implemented** — contract in **`CLI_SPEC.md`** (`mcp-use` bin backed by the lazily imported `@mcp-use/cli` toolchain, `.mcp-use/` workspace, project-local development Inspector, entry contract).
