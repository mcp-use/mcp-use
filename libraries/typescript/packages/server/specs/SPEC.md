...
- **Elicitation & context:** MRTR state, typed form and URL elicitation, progress, and logging helpers are implemented. Each per-request SDK server advertises MCP `logging`, so `ctx.sendLog()` emits request-scoped `notifications/message` entries. Push-style sampling/roots remain legacy-only and are deprecated in the 2026-07-28 spec.
