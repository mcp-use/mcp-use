...
- **Phase 5 — views (MCP Apps):** contract lives in **`VIEWS_SPEC.md`** (supersedes the earlier sketch on this line: single-protocol MCP Apps with no adapter system, `ToolRef`-based zero-codegen typing instead of typegen, `/react` subpath runtime, view naming throughout).
...
- ~~React view runtime: `/react` subpath vs separate package (Phase 5).~~ Answered: `/react` subpath with optional `react` peer — see `VIEWS_SPEC.md`.
