...
- Official v2 SDK only, pinned as one coordinated set. `client`, `core`, and `server` use the exact registry release `2.0.0-beta.5`. The Apps SDK temporarily resolves through the registry alias `@modelcontextprotocol/ext-apps -> @mcp-use/ext-apps@1.7.4-pr720.0`, built from ext-apps PR #720, until the dependency-isolation work ships under the official package. SDK docs: <https://ts.sdk.modelcontextprotocol.io/v2/>. No v1 (`@modelcontextprotocol/sdk`) imports.
...
- Schemas are typed against the SDK's [Standard Schema](https://standardschema.dev/) contracts, not zod: tool `inputSchema`/`outputSchema` (and prompt `schema`) accept any `StandardSchemaWithJSON` (validation + JSON Schema conversion — zod v4, ArkType, Valibot, …); `completable()` needs only `StandardSchemaV1`. Both types are re-exported. The SDK's `fromJsonSchema` helper and `JsonSchemaType` are also re-exported so JSON Schema-first libraries such as TypeBox can supply validation without a direct SDK dependency. Zod stays a **devDependency only** (tests/examples; it remains the documented example library) — no zod type in any public signature, and no runtime zod dependency either. Nothing needs one, auth included: tokens are `jose`'s job, our RFC 9728 document is validated with the SDK's _own exported schema value_ (calling `.parse()` on an imported object needs no zod dependency of ours), and adapter claim-mapping works from hand-declared interfaces + narrowing helpers (see `AUTH_SPEC.md` §Validation posture). Since an internal validator would be invisible to users, adopting one later is a zero-migration change if adapter mappers get gnarly in practice.
