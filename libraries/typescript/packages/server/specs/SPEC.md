...
- Official v2 SDK only, pinned as one coordinated set. `client`, `core`, and `server` use the exact stable registry release `2.0.0`. The Apps SDK temporarily resolves through the registry alias `@modelcontextprotocol/ext-apps -> @mcp-use/ext-apps@1.7.4-pr720.1`, built from ext-apps PR #720, until the dependency-isolation work ships under the official package. SDK docs: <https://ts.sdk.modelcontextprotocol.io/v2/>. No v1 (`@modelcontextprotocol/sdk`) imports.
...
- **Prefer raw wire result shapes; deprecated response helpers are thin shims.** Callbacks should return the SDK's raw wire shapes — `CallToolResult` / `ReadResourceResult` / `GetPromptResult`, re-exported from the package root. Rationale: models are trained on the official SDK idiom and v1 evals showed they don't discover wrapper helpers; the raw shapes already typechecked. Deprecated helpers (`text()`, `object()`, `array()`, `error()`, …) remain exported for upgrade compatibility and map to those same envelopes. `array()` uses the 2026 any-JSON root (no `{data}` wrap). Resource and prompt registration still accept helper-shaped `CallToolResult` returns and convert them via a small ContentBlock mapper (`text`→text contents, `image`/`audio` `data`→`blob`, unwrap embedded `resource`, skip `resource_link`; prompts map each block to a `user` message). The one footgun the docs must carry: for **object-shaped** structured output the SDK appends no text fallback, so callbacks include both halves themselves — `{ content: [{ type: "text", text: JSON.stringify(data) }], structuredContent: data }`. View-bound tools use plain `CallToolResult` with `view:` on the definition (the deprecated `widget()` helper only shapes the same envelope).
- **The v1 subpath is a deprecated migration bridge, not a second v2 API.** `mcp-use/server` resolves to `src/compat-v1.ts`, a facade over the native stateless server. It accepts the common v1 constructor and Zod registration shapes, direct built-in OAuth providers, response helpers, unchanged `listen()`, and the narrow legacy-view contract owned by `VIEWS_SPEC.md`. It does not expose sessions, sampling, arbitrary notifications, blocking elicitation, OAuth proxy mode, stdio proxying, raw `uiResource`, array schemas, unsigned-token bypasses, or shared one-view/many-tool bindings. Unsupported construction/registration fails with `MCP_USE_V1_COMPAT` guidance. Its public symbols are deprecated, construction emits one process-level deprecation warning, and the entire subpath is removed in v3. Native docs, examples, scaffolds, generated code, and agent instructions use only `mcp-use`.
...
- The HTTP app shell is Hono. `MCPServer.app` owns routing, `server.get()`/`post()`/etc. delegate to it, `server.use()` accepts Hono middleware unless the first argument starts with the mandatory `mcp:` namespace, and `server.fetch` is the one Web-standard deployment handler. The MCP endpoint still mounts via the SDK's `createMcpHandler` (fresh server per request — stateless, no session affinity). Node `listen()` and `mcp-use dev` reuse `server.fetch` through the vendored `toNodeHandler` bridge (`mcp-use/node`); `@hono/node-server` and `@modelcontextprotocol/hono` are unnecessary.
- No feature may require session state; cross-request continuity uses explicit handles in results.
- One framework package. `mcp-use` ships the server runtime and the complete first-party CLI: `dev`, `build`, `start`, cloud auth, organizations, servers, deployments, deploy, client, and screenshot (`CLI_SPEC.md`). Each substantial command is a genuine lazy chunk; `dev` and `build` are separate chunks, and neither `start` nor a library export may statically evaluate Vite or unrelated command code.
...
- `mcp-use` installs the optimized `@mcp-use/inspector` package. `mcp-use dev` mounts its Fetch handler on the development listener, and `mcp-use start --with-inspector` lazily mounts it as Hono middleware on the production application. Library imports, `server.fetch`, `listen()`, `build`, and plain `start` neither evaluate nor expose it. `@mcp-use/client` remains an independently published SDK and an **optional peer** — `server.proxy()`, `mcp-use client`, and `mcp-use screenshot` load it on demand; human CLI commands auto-install it when missing, while JSON commands return a structured install requirement without mutation. The library entry and plain `start` never evaluate it.
...
server.fetch; // universal web handler (Vercel, Hono, Workers)
...
| Need                                 | API                                                    |
| ------------------------------------ | ------------------------------------------------------ |
| Workers / fetch-native object export | `export default server`                                |
| Vercel / Next route methods          | `export const POST = server.fetch`                     |
| Existing Hono app                    | `app.mount("/", server.fetch)`                         |
| Node (normal)                        | `await server.listen(port)`                            |
| Custom `http.Server`                 | `createServer(toNodeHandler({ fetch: server.fetch }))` |
| Custom HTTP routes                   | `server.get(...)`, `server.post(...)`, or `server.app` |

`server.fetch` accepts the native Web `Request` plus optional Hono bindings/execution context and always returns a promise. `server.app` is the typed Hono route-registration surface; direct route methods are bound delegates for v1 ergonomics. Serving and external mounting use `server.fetch`, which wraps that application with the framework's security, OAuth, CORS, and logging layers. The v1 `getHandler()` name remains only as a deprecated identity alias returning `server.fetch`; it owns no lifecycle or configuration. `getNodeHandler()` does not exist: Node composition is the explicit `toNodeHandler({ fetch: server.fetch })` adapter from `mcp-use/node`.

**Handlers vs middleware vs events:** `server.tool()` implements business logic; `server.use('mcp:…')` intercepts MCP dispatch with `next()` (mutate params, short-circuit); `server.on('mcp:…')` observes read-only (`:complete` suffix for after-handler). The `mcp:` prefix is mandatory, leaving `server.use(handler)` and `server.use('/path/*', handler)` unambiguously available to Hono middleware. Tool/resource/prompt and MCP middleware contexts augment the active Hono `Context`: `ctx.request` is the preferred Hono request, deprecated `ctx.req` is the same request for v1 source compatibility, `ctx.request.raw` is the native Web Request, and `ctx.get()`/`ctx.env` expose typed middleware variables and runtime bindings.
...
Callback context (`ctx`, second parameter) is the active typed Hono `Context` augmented with `{ request?, signal, client, elicit, inputResponses?, requestState, sendNotification, reportProgress, sendLog, auth? }` — request-scoped only. `ctx.request` is Hono's request wrapper, `ctx.request.raw` is the native Web Request, and deprecated `ctx.req` is the same object. HTTP middleware variables set with `c.set()` are read with `ctx.get()` inside local and proxied MCP dispatch. `ctx.elicit` is the typed form/URL convenience path; `inputResponses` and `requestState()` expose the official v2 multi-round-trip state for advanced flows. The package root re-exports `inputRequired`, `inputResponse`, `acceptedContent`, and `createRequestStateCodec`. `ctx.sendNotification(method, params?)` preserves the v1 callback API over the official v2 `ctx.mcpReq.notify(...)` primitive: it sends a custom one-way notification on the originating request's response stream before the final result. It is not a session, broadcast, or post-response channel; cross-request list/resource changes use the `MCPServer.notify*` subscription helpers. `ctx.client` projects current-request metadata without session fallback: `capabilities()` returns the official `ClientCapabilities`, `info()` returns the client `Implementation` metadata with a v1-compatible partial type, `can(name)` checks top-level capability presence, `extension(id)` reads one extension settings object, and `supportsViews()` checks the MCP Apps declaration from the modern request envelope. `user()` separately normalizes the current request's ordinary `_meta` fields using OpenAI's documented keys (`openai/locale`, legacy `webplus/i18n`, `openai/userAgent`, `openai/userLocation`, `openai/subject`, `openai/session`, and `openai/organization`) and returns `undefined` when no recognized valid field is present. Envelope object accessors return shallow copies; `user()` returns a fresh outer object and location object. No accessor infers or caches values from earlier requests. All client metadata, including subject, conversation, and organization identifiers, is self-declared and unverified; authentication and authorization use `ctx.auth`. With OAuth configured, `ctx.auth` is present (`AUTH_SPEC.md` / `AUTH_IMPLEMENTATION.md`). Nothing session-scoped is added.
...
`ServerConfig.favicon?: string` controls the conventional browser route. A present, valid `favicon` always wins. Otherwise a non-empty `icons` list uses the first icon in author order. An empty `icons` array selects nothing. An explicitly empty `favicon` is invalid rather than silently treated as absent.
...
Hono preserves author registration order: user middleware and custom routes are registered first, then framework favicon, public assets, built view assets, and the exact MCP route are mounted lazily before the first request. Framework security, CORS, JSON parsing, logging, and OAuth discovery middleware wrap the entire Hono application; the bearer gate still wraps only the exact MCP endpoint. `listen()` and `server.fetch` use the same application and neither mounts development tooling. When configured, global `cors` middleware owns the branding responses' CORS headers; otherwise shared view/public assets retain their existing wildcard header. The landing page uses the request-resolved absolute `/favicon.ico` URL for both its browser favicon and hero icon. Views obtain server icons and `websiteUrl` through standard MCP initialization metadata; view documents do not invent another branding channel. `MCPServer.branding` exposes the frozen normalized `{ favicon?, icons?, websiteUrl? }` so browser integrations do not duplicate selection or file handling.
...
2. MCP capability, proxy, and MCP middleware registrations are rejected after `listen()` or the first `server.fetch` request — the registry is replayed per request, so late registration would be silently inconsistent. Hono route registration follows Hono's application lifecycle.
...
5. `listen()` binds `127.0.0.1` by default (old package: `0.0.0.0`) and localhost-class binds get DNS-rebinding Host protection automatically: `Host` on every request. Origin validation is off unless `allowedOrigins` is set explicitly (SDK-aligned: the handler is open by default). When enabled, Origin is checked only on non-GET/HEAD (sandboxed view iframes send `Origin: null` on asset GETs; the MCP wire is POST). Validation follows the threat model, not the adapter's host-keyed defaults: DNS rebinding targets locally bound servers, so `server.fetch` — which never binds and is expected to sit behind a platform edge that only routes the deployment's own hostnames — applies **no** validation unless `allowedHosts`/`allowedOrigins` are set, and a public `listen(host: "0.0.0.0")` serves unvalidated with a one-line warning. Configured `allowedHosts`/`allowedOrigins` are **additive** to the localhost allowlists (local runs keep working). The one guarded footgun: a localhost `listen()` after `server.fetch` already mounted the app without validation throws.
...
Registration data comes from the same registry that `listen()` or the first `server.fetch` request freezes when it mounts the app. Late registrations throw, so a page can never advertise primitives that the per-request SDK factory cannot replay.
...
The server package emits anonymous, fetch-only usage metrics through one internal `mcp_use_sdk_event` event. `feature` and `action` distinguish the once-per-server composition snapshot (`server/configured`), once-per-client-tuple protocol observation (`client/observed`), operation outcome (`operation/completed`), and once-per-View-runtime public React hook adoption (`view/hook_used`). Successful operations are sampled at 10%; errors and `input_required` results are not. Every event carries `sdk_generation: "v2"`, schema and package-version markers, a process-random runtime identity, and a stable server identity when storage is available. The latter is a random UUID lazily persisted in the server project's gitignored `.mcp-use/usage.json`; read-only and non-Node runtimes silently fall back to process scope, while `MCP_USE_TELEMETRY_PROJECT_ID` remains the explicit highest-priority identity. No identity is derived from host information. PostHog person profiles and GeoIP are suppressed. `MCP_USE_ANONYMIZED_TELEMETRY=false` opts out before any file access; browser Views also honor the existing global and local-storage opt-out forms. `MCP_USE_TELEMETRY_VALIDATION_ID` marks canaries and bypasses sampling. Properties are flat bounded scalars; request/response content, arguments, headers, URLs, user metadata, auth/session identifiers, and other content-shaped values are rejected before capture. The transport is dependency-free, bounded, fire-and-forget, and failure-isolated; it adds no public API.

**Intentionally absent from the core primitive layer:** legacy push-style sampling/roots APIs, typegen, stdio serving, and framework-specific Connect/Express adapters. Hono HTTP routes and middleware, MCP operation middleware (`server.use('mcp:…')`), and observer events (`server.on('mcp:…')`) are implemented on the canonical server application. OpenAPI import is an integration built on top of `tool()`, described below; it does not add a second registration path. Views are governed by `VIEWS_SPEC.md`. Typed `ctx.elicit` and the raw v2 MRTR primitives (`inputRequired`, `inputResponse`, `acceptedContent`) operate through explicit `input_required` returns without introducing session state.

**Examples** (`examples/vercel`, `examples/railway`): the two deployment doors, each verified end-to-end. Vercel = serverless via `server.fetch` exported from an `api/` function — zero host config (delta 5). Railway = the CLI entry contract (`CLI_SPEC.md`): the entry default-exports the server and never calls `listen()` itself; `mcp-use build` + `mcp-use start` own the socket (host selection via `RAILWAY_PUBLIC_DOMAIN` stays constructor config, which `start`'s `listen()` honors), and the bin handles SIGINT/SIGTERM → `close()`.
...
`ctx.elicit(key, message, schema)` handles typed form mode; `ctx.elicit(key, message, url)` handles URL mode. It returns `{ status: "required", result }` on first entry (and after invalid Standard Schema form data), `{ status: "accept", data }` for an accepted typed form, `{ status: "accept" }` for accepted URL mode, or `{ status: "decline" | "cancel" }`. The explicit developer key is deterministically scoped to the logical MCP method, target, parameters, and elicitation mode before becoming the `inputRequests` wire key. A response accidentally attached to a different logical operation therefore reads as missing and produces a fresh `input_required` result instead of authorizing that operation. This request scoping is correlation defense, not cryptographic authorization or single-use replay protection.

`ctx.elicit` is asynchronous for Standard Schema validation and request-scoped key derivation. It never waits for user input inside the handler: the stateless 2026 protocol still requires the handler to return `input_required`, after which the client retries the original tool. Keeping the `required` return branch explicit makes handler re-entry visible and avoids a hidden suspension mechanism that could make repeated pre-elicitation side effects surprising. URL mode needs no `elicitationId`; correlation is the scoped key, with integrity-protected `requestState` for sequential or security-sensitive rounds. A URL-mode `accept` records consent to open the URL, not completion of the out-of-band interaction; callbacks verify backend state bound to the authenticated user before reporting success. No server-side request remains open while the user responds.
...
- **Serving hardening:** stdio serving remains an explicit future decision. The canonical Hono application now owns HTTP composition, while `createMcpMount` remains the low-level fetch-native MCP mount for advanced adapters. `listen()` reports a browsable URL appropriate to its bind host, and `server.fetch` removes the separate-handler mount-point drift that existed in the earlier design.
...
- **Product shell:** OAuth providers + scope guards + `.well-known` (with auth, above), Hono HTTP middleware and custom routes, operation middleware (`server.use("mcp:*")`) and observer events (`server.on("mcp:*")`), optional `ServerConfig.cors`, the browser landing page, and resource-subscription ergonomics are implemented. Cross-request v2 list/resource notifications are implemented through `MCPServer.notify*`, backed by the SDK handler bus. Every per-request SDK server advertises tool/prompt/resource `listChanged` (and resource `subscribe`) even while a registry is empty, so a client connected before the first primitive is added can subscribe. `mcp-use dev` injects one process-scoped bus through an internal lifecycle hook across every reload generation and publishes all three list invalidations after a successful handler swap; this is intentionally not another public serving API.
- **Elicitation & context:** MRTR state, typed form and URL elicitation, custom request-scoped notifications, progress, and logging helpers are implemented. `ctx.sendNotification()` maps the v1-compatible method/params signature to the official v2 request notification primitive. Each per-request SDK server advertises MCP `logging`, so `ctx.sendLog()` emits request-scoped `notifications/message` entries. Push-style sampling/roots remain legacy-only and are deprecated in the 2026-07-28 spec.
- **Integration:** OpenAPI import, upstream server composition, and anonymous server-package usage metrics are implemented. Usage capture is a private fetch-only primitive rather than a `posthog-node` dependency. The independently published `@mcp-use/client` remains the SDK boundary; the framework consumes it on demand for `server.proxy()` and `mcp-use client` rather than folding or re-exporting the SDK from the server runtime.
