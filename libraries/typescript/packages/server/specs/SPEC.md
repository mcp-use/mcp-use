...
- Hono is the long-lived, publicly exposed app shell. `MCPServer.app` is created during construction so consumers can register custom HTTP routes before startup; framework-wide JSON parsing, request logging, Host/Origin validation, and OAuth protection are installed before those routes. Views, OAuth metadata, MCP, and inspector routes mount onto that same app at first startup/direct dispatch. The MCP endpoint uses the SDK's `createMcpHandler` (fresh server per request — stateless, no session affinity), while Host/Origin validation uses the SDK's official runtime-neutral response helpers behind the early Hono bootstrap middleware. Prefer official adapters and primitives over hand-rolled plumbing wherever they exist.
- No feature may require session state; cross-request continuity uses explicit handles in results.
- One package. This package ships the `mcp-use` bin and the `dev`/`build` toolchain itself (`CLI_SPEC.md`), with the toolchain reachable only through a lazily imported chunk so `start` and library consumers never pay for it. No dependency of any kind on `@mcp-use/inspector` (the inspector is reached over HTTP/CDN only) or on the old v1 packages.
...
- Dependency budget: runtime deps stay minimal (`@modelcontextprotocol/server`, `hono`, `@hono/node-server`, `jose`). Target install ≤ ~25 MB (vs 190 MB today).
...
HTTP app contract: `MCPServer.app` is a `public readonly Hono` created in the constructor. Reading it is side-effect free: it does not freeze MCP registrations, choose the fetch-handler Host/Origin policy, or resolve deferred OAuth resource identity. Consumers register ordinary Hono routes and middleware directly (including multi-method `app.on([...], path, handler)`) before `listen()`, `getHandler()`, or the first direct `app.fetch()` / `app.request()` dispatch. No v1 HMR custom-route registry is carried forward. At first startup, framework views/OAuth/MCP/inspector routes are appended to the same app; a direct first dispatch synchronously performs that mount before Hono builds its matcher, so it cannot freeze out the MCP route. That transition freezes tool/resource/prompt/view registration, and the server guards Hono's route/middleware registration surface after startup (Hono's matcher independently becomes immutable after first dispatch). Constructor-installed framework-wide middleware runs before every user route, so custom routing cannot bypass JSON parsing, logging, configured Host/Origin validation, or the OAuth gate on the MCP path. The exposed HTTP router is long-lived, but MCP serving remains stateless: `createMcpHandler` builds a fresh official SDK `McpServer` from the frozen registry for every MCP request.
...
2. MCP registrations are rejected after `listen()`/`getHandler()` or the first direct `app.fetch()` / `app.request()` dispatch — the registry is replayed per request, so late registration would be silently inconsistent. Custom Hono route registration has the same pre-start/pre-first-dispatch lifecycle and uses Hono directly; there is no v1 HMR custom-route map. (The old package allowed live MCP registration because it kept long-lived per-session servers; that model is gone.)
...
5. `listen()` binds `127.0.0.1` by default (old package: `0.0.0.0`) and localhost-class binds get DNS-rebinding protection automatically: `Host` on every request, `Origin` only on non-GET/HEAD (sandboxed view iframes send `Origin: null` on asset GETs; the MCP wire is POST). Validation follows the threat model, not the adapter's host-keyed defaults: DNS rebinding targets locally bound servers, so `getHandler()` and direct `app.fetch()` / `app.request()` dispatch — which never bind and are expected to sit behind a platform edge that only routes the deployment's own hostnames — apply **no** validation unless `allowedHosts`/`allowedOrigins` are set, and a public `listen(host: "0.0.0.0")` serves unvalidated with a one-line warning. Configured `allowedHosts`/`allowedOrigins` are **additive** to the localhost allowlists (local runs keep working), and `allowedOrigins` defaults to mirroring the effective Host allowlist. The one guarded footgun: a localhost `listen()` after fetch-handler-mode mounting without validation throws.
...
- **Views (MCP Apps): next** — contract in **`VIEWS_SPEC.md`** (single-protocol MCP Apps with no adapter system, `ToolRef`-based zero-codegen typing, `/react` subpath runtime, view naming throughout; extends the CLI contract with a client Vite environment).
...
- **Product shell:** OAuth providers + scope guards + `.well-known` (with auth, above), operation middleware (`server.use("mcp:*")`), landing page, notifications (`handler.notify`/bus) + resource subscriptions (also the prerequisite for dev-reload `list_changed` emission, `CLI_SPEC.md`/`VIEWS_SPEC.md`).
- **Elicitation & context:** `ctx.elicit` on `InputRequiredResult` (multi-round-trip), progress reporting; sampling/roots posture decision (deprecated in the 2026-07-28 spec — the SDK's legacy-path methods throw on modern-era requests).
- **Integration & cutover:** OpenAPI import, telemetry (posthog-node, opt-out), repoint inspector/cli/templates, rename to `mcp-use`, delete the old package.
