...
Icon and favicon sources accept exactly three forms: a safe path relative to project `public/`, an absolute HTTP(S) URL, or an `image/*` data URL. Local paths must not start with `/`, contain `..` segments, backslashes, a query, a fragment, or empty path segments. Local icon sources become request-scoped absolute implementation URLs under `<server-origin><basePath>/_mcp-use/public/<encoded-path>` by default (`basePath: "/"` produces `/_mcp-use/public/…`). With an explicit `MCP_ASSETS_URL`, that complete prefix is used instead and `basePath` is not appended. HTTP(S) and data URLs pass through unchanged. Invalid values fail at `MCPServer` construction.
