...
- `mcp-use` installs the optimized `@mcp-use/inspector` package. `mcp-use dev` mounts its Fetch handler on the development listener, and `mcp-use start --with-inspector` lazily mounts it on the production listener. Library imports, `getHandler()`, `listen()`, `build`, and plain `start` neither evaluate nor expose it. `@mcp-use/client` remains an independently published SDK and an **optional peer** — `server.proxy()`, `mcp-use client`, and `mcp-use screenshot` load it on demand and print install instructions when it is missing; the library entry and plain `start` never evaluate it.
...
**Handlers vs middleware vs events:** `server.tool()` implements business logic; `server.use('mcp:…')` intercepts with `next()` (mutate params, short-circuit); `server.on('mcp:…')` observes read-only (`:complete` suffix for after-handler). Every operation context exposes the originating HTTP request as optional `ctx.request`, including middleware and observers, so operation-specific policy may inspect request headers. HTTP `server.use(morgan)` is not on `MCPServer` — use your framework for pre-MCP middleware and custom routes.
