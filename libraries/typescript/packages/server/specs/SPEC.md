...
Operation selection matches v1: `tags` is an include-any filter; each `exclude` rule ANDs its populated `operationId`, `path`, `method`, and `tags` criteria; operation-level parameters replace same-location/same-name path parameters. Tool names prefer `operationId`, otherwise derive from method + path, are sanitized to MCP-safe 64-character names, and receive deterministic `_2`, `_3`, … suffixes on collision. Descriptions join the operation summary, description, and `HTTP: METHOD /path` line.

Generated input schemas flatten non-cookie path/query/header parameters and a JSON request body (`body`) into one object. Path parameters are always required. Local bundled references are resolved; schema references are rewritten into JSON Schema `$defs` so recursive schemas remain valid. OpenAPI 3.0 `nullable` becomes a JSON Schema null union. The SDK's `fromJsonSchema` adapter supplies Standard Schema validation and JSON Schema emission, preserving the core package's no-Zod-runtime-dependency rule.

Calls use `options.baseUrl`, then the first `spec.servers` URL. Path values are encoded, array query values repeat the key, headers are mapped directly, and JSON request bodies set `content-type` unless already present. Static headers plus bearer/custom-header upstream authentication are supported. Successful JSON responses return both a JSON text block and native `structuredContent`; successful non-JSON responses return text; non-2xx responses return an `isError` text result. External references, cookie parameters, non-JSON request bodies, OpenAPI callbacks/webhooks, and automatic security-scheme credential discovery are intentionally unsupported. A custom `fetch` may be supplied for runtime integration, but tests exercise the default implementation over real HTTP.
