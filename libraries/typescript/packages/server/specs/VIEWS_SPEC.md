...
The package pins the official `client`, `core`, and `server` registry packages at `2.0.0-beta.5`, the first coordinated release that exposes the public `Protocol` base required by the Apps iframe channel. Until ext-apps PR #720 is released officially, `@modelcontextprotocol/ext-apps` resolves through the registry alias `@mcp-use/ext-apps@1.7.4-pr720.0`, built from that PR. This stack keeps `ui/initialize` as the only iframe handshake and accepts Standard Schema validators for App tools. Consequences:
