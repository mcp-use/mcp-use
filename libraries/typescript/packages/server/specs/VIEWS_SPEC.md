...
- **Dependency mechanics:** ext-apps remains an **optional peer** of this package — the `vite` pattern from `CLI_SPEC.md`. A peer dependency cannot carry a URL specifier, so the package metadata pins the preview's underlying `1.7.4` version while view projects declare the ext-apps #712 and TypeScript SDK #2501 preview URLs explicitly. Tool-only servers install neither ext-apps nor its client-side protocol stack, keeping the install-budget ground rule honest. The package's v2 SDK pins move as a coordinated set so ext-apps and the server share one protocol implementation. Replace all preview URLs with the corresponding published releases together once both PRs land.
