...
15. **Legacy widgets are a temporary source adapter only.** When and only when the server entry uses deprecated `mcp-use/server`, CLI discovery also accepts `resources/<name>/widget.tsx`, reads `widgetMetadata`, and compiles the component through the same v2 bootstrap and manifest. Legacy `McpUseProvider`, `useWidget`, split widget hooks, and metadata types are composed in `src/react/compat-v1.tsx`; they do not restore the v1 bridge or wire protocols. `exposeAsTool` defaults to `false`, matching v1 runtime behavior. A custom `widget: { name }` binding needs no exported tool constant, but the native one-tool/one-view rule still applies: a second legacy tool for the same view fails with migration guidance. Native v2 discovery and APIs remain `views/<name>/view.tsx` and `view` only. The adapter is deprecated and removed in v3.
