...
13. **Framework-authored tool visibility lives inside `view`.** `view.visibility?: "model" | "app"` emits `_meta.ui.visibility: ["model"] | ["app"]` on the view-bound tool's `tools/list` descriptor and is omitted when unset (host default: callable by the model, visible to the app). The server always lists every registered tool; filtering is host policy (MCP Apps: hosts MUST hide `visibility: ["app"]` tools from the model and MUST reject app `tools/call` for tools without `"app"`). Visibility is never added to tool results. A viewless app-private helper has no `view` config, so it uses the protocol-level escape hatch `_meta: { ui: { visibility: ["app"] } }`; definition `_meta` passes through unchanged when no `view` field generates the same key.
...
| tool `_meta`     | `ui.visibility`                                             | `["model"]` / `["app"]` when the bound tool's `view.visibility` is set; raw definition `_meta` may also supply the protocol key for viewless helpers. Omitted when neither source supplies it (host default: callable by the model, visible to the app). Declaration only — the server always lists every tool; hosts filter by this key. |
...
Authors may also provide custom tool definition `_meta`. The registration boundary shallow-copies it and merges framework-generated tool keys deterministically. `view.name` generates nested `ui.resourceUri` plus legacy flat `"ui/resourceUri"`; `view.visibility` generates nested `ui.visibility`. A generated value wins a collision, but when a view field is absent the corresponding author-supplied `_meta` value passes through unchanged. Other top-level vendor keys and other fields of an object-valued `ui` entry are also preserved. The merge never mutates the caller's object, never recursively assigns user keys into an existing target, and treats keys such as `__proto__` as data. This definition merge affects only `tools/list`; tool-result `_meta` retains the separate result-stamping rules below.
...
**The wire surface is unconditional.** `tools/list` always includes every registered tool regardless of client capabilities or advertised `ui.visibility`. View-bound tools always carry `_meta.ui.resourceUri` (plus the legacy flat `"ui/resourceUri"` key) on `tools/list`, and every non-error result from that tool is stamped with the same link keys — regardless of whether the client advertises the UI extension. View resources always carry `_meta.ui` (framework auto-CSP with the serving origin) on both `resources/list` entries and each `resources/read` content item. When `view.visibility` is set, `_meta.ui.visibility` is emitted as a declaration (`["model"]` or `["app"]`); raw definition `_meta` may carry the same protocol declaration for a viewless tool. Filtering is **client policy** — the server never omits tools from `tools/list`.
...
The `view:` config on `server.tool()` binds the tool to a view resource. Resource wire facts (`description`, `csp`, `permissions`, `domain`, `prefersBorder`) and tool UI visibility are authored on that tool's `view:` — the single binder owns all framework-generated UI facts (decisions 10 and 13). The view file exports the component (and optional `viewConfig`); the framework reads the binder's `view:` fields at registration and emits resource facts on the view's MCP resource plus visibility on the tool descriptor.

```ts
view: {
  name: string;                    // view directory name, e.g. "product-search-result"
  visibility?: "model" | "app";   // → tool _meta.ui.visibility on tools/list; omitted = host default (model + app)
...
A viewless app-private helper uses raw definition metadata instead:
...
Users export the refs of tools views care about (`export const searchFruits = server.tool(…)`) — the module is the registry; no map API, no `export type AppType` ritual, no user-written `declare module`. The name union covers **every exported ref** regardless of advertised `ui.visibility` (a view may call model-visible tools too; the host, not the server, hides tools carrying `["app"]` from the model). `typeof import()` is a live tsserver edge: add a tool, and every view's `useCallTool` union updates with no process running. Multi-file registration composes via re-exports (`export * from "./tools/fruits.js"`).
...
| server tool, app-visible | `server.tool({ _meta: { ui: { visibility: ["app"] } }, … })` | the view, via `useCallTool` | server process; host hides from the model per `_meta.ui.visibility` |
...
- **e2e over HTTP** (official client): view resource listing/reading with correct mimetype and framework auto-CSP in `_meta.ui.*` on both `resources/list` entries and `resources/read` content items for all clients; `tools/list` includes every registered tool for all clients (including view-bound tools with `view.visibility: "app"` and viewless helpers with raw `_meta.ui.visibility: ["app"]`); custom tool definition `_meta` coexists with generated view metadata, generated collision values follow the declared contract while keys not generated by `view` pass through unchanged, caller objects are not mutated or prototype-polluted, and per-request SDK reconstruction does not leak metadata between concurrent requests; **channel separation** — handler `{ structuredContent, content, _meta }` lands on the wire as `structuredContent` / `content` / `_meta` respectively, with handler `_meta` absent from everything model-facing; `_meta.ui.resourceUri` (plus legacy flat `"ui/resourceUri"`) auto-stamped on every non-error view-bound tool result; error results carry no resource-URI stamp; no custom tool-name metadata on results.
...
9. Tool config `invoking`/`invoked`/`widgetAccessible` removed (openai overlay, no spec equivalent; `view.visibility` covers app/model narrowing for a bound tool, with raw definition `_meta.ui.visibility` for a viewless helper).
