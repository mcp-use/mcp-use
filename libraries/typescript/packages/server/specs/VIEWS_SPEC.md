...
**srcdoc iframes have no document base URL.** Hosts render view documents via `srcdoc`, so every asset URL the view loads must be absolute — root-relative paths resolve against the *host page* origin, not the MCP server. Production build assets get absolute URLs from `import.meta.url` (`base: "./"` in the client build). Dev sets Vite `server.origin` to the dev server's listening origin so imported assets emit absolute `http://…` URLs. Public assets (below) resolve through a request-scoped config global injected into the synthesized document.
