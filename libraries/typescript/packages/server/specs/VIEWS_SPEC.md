...
- View documents are synthesized per `resources/read` with `/@vite/client` and the virtual entry served through Vite middleware (`kind: "external"`). Assets flow through Vite transform, with no build step or manifest file. The in-memory view registry stays current through Vite's watcher: server and structural view events are reconciled as one revisioned project generation, and a fresh `MCPServer` is primed from that generation's immutable view snapshot. Superseded candidates never swap the active handler or expose intermediate registry state; view-code edits use client HMR. The next `tools/list`/`resources/list` reflects committed directory changes, and subscribed modern clients are prompted to refetch. The `public/` route serves `<projectRoot>/public` directly.
