...
describe("OAuthAuthorizationCore", () => {
  ...
  it.each(["rotateRefreshToken", "beginUpstreamRefresh"] as const)(
    "%s does not revoke a family for replay under a different client or resource",
    async (operation) => {
      const core = createCore();
      const first = await issueTokens(core);
      const second = await core.rotateRefreshToken({
  ...
  it("preserves an explicit upstream grant lifetime when promoting a pending code", async () => {
    vi.useFakeTimers();
    ...
    const issued = await core.persistGrantAndIssueAuthorizationCode({
      ...
      scopes: ["read"],
      codeChallenge: await s256(verifier),
      grant: upstreamGrant({ expiresAt: Date.now() + 2000 }),
    });
    const tokens = await core.exchangeAuthorizationCode({
      code: issued.code,
      clientId,
  ...
  it("rejects an already expired explicit upstream grant before issuing a code", async () => {
    const core = createCore();
    await expect(
      core.persistGrantAndIssueAuthorizationCode({
        ...
        scopes: ["read"],
        codeChallenge: await s256(verifier),
        grant: upstreamGrant({ expiresAt: Date.now() - 1 }),
      })
    ).rejects.toThrow("upstream grant expiry");
  });
  ...
  it("atomically rolls back a pending grant when code creation fails", async () => {
    ...
    await expect(
      readOAuthProxyStoreEntry(underlying, grantKeys[0]!)
  ...
  it.each(["accessToken", "refreshToken"] as const)(
    "binds %s revocation to the submitting client",
    async (tokenType) => {
      const core = createCore();
      const tokens = await issueTokens(core);
      ...
      await core.revokeLocalToken({ token: tokens[tokenType], clientId });
      await expectCoreError(
        core.validateAccessToken(tokens.accessToken, resource),
        "invalid_token"
      );
