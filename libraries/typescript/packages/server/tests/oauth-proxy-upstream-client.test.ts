...
describe("UpstreamOAuthClient authorization", () => {
  ...
  it("rejects reserved overrides and configured endpoint query collisions", () => {
    const { fetch } = recordingFetch();
    expect(() =>
      createClient(fetch, {
    ...
    expect(() =>
      createClient(fetch, {
        authorizationParams: { state: "attacker" },
      })
    ).toThrow(/reserved parameter state/);
    expect(() =>
      createClient(fetch, {
        authorizationEndpoint: `${authorizationEndpoint}?tenant=acme`,
        authorizationParams: { tenant: "other" },
      })
    ).toThrow(/collides with configured parameters/);
  });
...
describe("UpstreamOAuthClient token operations", () => {
  ...
  it("preserves omitted refresh tokens and accepts rotation", async () => {
    const { calls, fetch } = recordingFetch(
    ...
    expect(preserved).not.toHaveProperty("scope");
    ...
    expect(bodyParams(calls[0]!).has("scope")).toBe(false);
  });
});
...
describe("UpstreamOAuthClient response validation", () => {
  ...
  it("redacts repeated percent-encoded secrets in one pass", async () => {
    const encodedSecrets = Array.from({ length: 1_000 }, (_, index) =>
      index % 2 === 0 ? "client%2fsecret" : "client%2Fsecret"
    ).join(" ");
    const { fetch } = recordingFetch(
      jsonResponse(
        {
          error: "invalid_client",
          error_description: encodedSecrets,
        },
        401
      )
    );
    ...
    await expect(exchange(client)).rejects.toMatchObject({
      code: "invalid_client",
      description: expect.not.stringContaining("client%2"),
    });
  });
...
describe("UpstreamOAuthClient callback bindings", () => {
  it("rejects mixed success and error callbacks without exposing an authorization code", async () => {
    const { fetch } = recordingFetch();
    const client = createClient(fetch);
    const created = await authorization(client);
    ...
    await expect(
      client.exchangeAuthorizationCode({
        authorizationResponse: new URLSearchParams({
          ...
          error: "access_denied",
          error_description: "Failure for authorization-code-secret",
          state: created.transaction.state,
          iss: issuer,
        }),
        transaction: created.transaction,
      ...
      message: "OAuth authorization response contains both code and error",
    });
    expect(fetch).not.toHaveBeenCalled();
  });
