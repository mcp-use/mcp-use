...
describe("oauthProxy", () => {
  it("requires explicit coherent upstream client authentication", () => {
    ...
    ).toThrow(/must encrypt secrets/u);
    expect(() =>
      oauthProxy(
        proxyOptions({
          store: {
  ...
  it.each(["/oauth/.", "/oauth/..", "/oauth/../login"])(
    "rejects route prefixes that URL normalization would change: %s",
    (authorizationServerPath) => {
      expect(() =>
        oauthProxy(proxyOptions({ authorizationServerPath }))
      ).toThrow(/authorizationServerPath/u);
    }
  );

  it.each(["invalid-credential", "", "x".repeat(43)])(
    "reports malformed or unknown local credentials as invalid_token: %j",
    async (token) => {
      const binding = oauthProxy(proxyOptions()).bind(resource);
      await expect(
        binding.tokenVerifier.verifyAccessToken(token)
      ).rejects.toMatchObject({ code: "invalid_token" });
      // The SDK must recognize the error, not turn an ordinary failed login
      // into an HTTP 500 that prevents the client from reauthorizing.
      const { requireBearerAuth } = await import("../src/oauth/index.js");
      const response = await requireBearerAuth({
        verifier: binding.tokenVerifier,
      })(
  ...
  it("keeps storage outages distinct from invalid credentials", async () => {
    const outage = new Error("storage unavailable");
    const store: OAuthProxyStore = {
      ...inMemoryOAuthStore(),
      transaction: async () => {
        throw outage;
      },
    };
    const binding = oauthProxy(proxyOptions({ store })).bind(resource);
    await expect(
      binding.tokenVerifier.verifyAccessToken("x".repeat(43))
    ).rejects.toBe(outage);
  });
  ...
  it("keeps downstream tokens local while mapping the provider token explicitly", async () => {
    ...
    const getUserInfo = vi.fn((payload: Record<string, unknown>) => ({
