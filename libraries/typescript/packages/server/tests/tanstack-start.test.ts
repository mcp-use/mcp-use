...
describe("TanStack Start handler", () => {
  ...
  it("decodes assets only on the first GET and keeps response bodies independent", async () => {
    loadBuild.mockResolvedValue({
      ...build,
      assets: {
        "public/cache.svg": {
          body: btoa("<svg>cache</svg>"),
    ...
    try {
      const url = "http://localhost/api/mcp/_mcp-use/public/cache.svg";
...
describe("TanStack Start plugin configuration", () => {
  it.each([
    ...
    "/api?mcp",
    "/api#mcp",
  ])("rejects invalid basePath %j before building", (basePath) => {
    expect(() => mcpUseTanStackStart({ basePath })).toThrow(
