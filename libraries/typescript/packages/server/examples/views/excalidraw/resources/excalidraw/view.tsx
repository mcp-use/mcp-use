...
const morphdom = (
  (morphdomNs as unknown as { default?: typeof morphdomNs }).default ??
  morphdomNs
) as unknown as (
  fromNode: Element,
  toNode: Node | string,
...
function DiagramView({ toolInput, isFinal, displayMode, onElements, editedElements, onViewport, loadCheckpoint }: { toolInput: any; isFinal: boolean; displayMode: string; onElements?: ((els: any[]) => void) | undefined; editedElements?: any[] | undefined; onViewport?: ((vp: ViewportRect) => void) | undefined; loadCheckpoint?: ((id: string) => Promise<{ elements: any[] } | null>) | undefined }) {
...
export default function ExcalidrawView() {
  ...
  const { callTool: exportToExcalidrawTool } = useCallTool("export_to_excalidraw");
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape" && displayMode === "fullscreen") void toggleFullscreen();
    };
      ...
      {mountEditor && (
        ...
        >
          <Excalidraw
            ...
            theme="light"
            onChange={(els: readonly Record<string, unknown>[]) => onEditorChange(els as any)}
            renderTopRightUI={
              isNarrow
                ? undefined
                : () => (
