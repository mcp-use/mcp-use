import { useCallTool, useView } from "mcp-use/react";
...
export default function WeatherCard({
  ...
  return (
    <div className={`rounded-xl overflow-hidden shadow-lg max-w-sm text-white`}>
      ...
      <div
        className={`p-3 flex flex-wrap gap-2 text-xs ${isDark ? "bg-gray-800" : "bg-gray-50"}`}
      >
        <button
          onClick={() =>
            requestDisplayMode(
        ...
        <button
          onClick={() =>
            callEcho({ message: `Weather in ${city}: ${temperature}°C` })
          }
      ...
      {echoData && (
        <div
          className={`px-3 pb-3 text-xs ${isDark ? "bg-gray-800 text-gray-400" : "bg-gray-50 text-gray-500"}`}
        >
