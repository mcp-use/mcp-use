...
import { createMCPServer, oauthWorkOSProvider, error, object } from "mcp-use/server";
...
if (!WORKOS_API_KEY) {
  console.warn("Warning: MCP_USE_OAUTH_WORKOS_API_KEY not set. API calls will fail.");
}
...
server.tool({
  ...
  try {
    ...
    if (!res.ok) {
      return error(`Failed to fetch user from WorkOS: ${res.status} ${res.statusText}`);
    }
