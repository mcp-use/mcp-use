...
>(
  ...
  } as UseViewResult<TProps, TState, TOutput, TMetadata, TToolInput>;
}
 ...
 * @example
 * ```tsx
 * const props = useViewProps<{ city: string; temperature: number }>();
 * ```
 */
export function useViewProps<TProps = UnknownObject>(
  defaultProps?: TProps
): Partial<TProps> {
  const { props } = useView<TProps>(defaultProps);
  return props;
}
 ...
 * ```
 */
export function useViewTheme(): Theme {
  const { theme } = useView();
  return theme;
}
 ...
 * ```
 */
export function useViewState<TState>(
  defaultState?: TState
): readonly [
  TState | null,
  (state: TState | ((prev: TState | null) => TState)) => Promise<void>,
] {
  const widget = useView<UnknownObject, TState>();
...
/** @deprecated Use `useView` instead. @alias */
export const useWidget = useView;

/** @deprecated Use `useViewProps` instead. @alias */
export const useWidgetProps = useViewProps;

/** @deprecated Use `useViewTheme` instead. @alias */
export const useWidgetTheme = useViewTheme;

/** @deprecated Use `useViewState` instead. @alias */
export const useWidgetState = useViewState;
