...
import type { CallToolResponse, UnknownObject } from "./widget-types.js";
 ...
 */
type ResolveResponse<T> =
  ExtractOutput<T> extends UnknownObject
...
export function createTypedHooks<
  T extends Record<string, { schema?: unknown; outputSchema?: unknown }>,
>() {
  return {
    useCallTool: <TName extends keyof T & string>(
      name: TName
    ): UseCallToolReturn<ExtractInput<T[TName]>, ResolveResponse<T[TName]>> => {
      return useCallToolBase(name) as unknown as UseCallToolReturn<
        ExtractInput<T[TName]>,
        ResolveResponse<T[TName]>
      >;
    },
  };
}
