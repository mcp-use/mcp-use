...
interface WidgetBuildManifest {
  buildId?: string;
  widgets?: string[] | Record<string, WidgetMetadata>;
  inlineWidgetTools?: Record<
    string,
    Partial<InlineWidgetManifestEntry> & { widgetName?: string }
  >;
}
...
export async function mountWidgetsProduction(
  ...
  let widgets: string[] = [];
  let widgetsMetadata: Record<string, Record<string, unknown>> = {};

  try {
    let manifestContent: string | undefined;
    for (const manifestPath of manifestPaths) {
      try {
        manifestContent = await fsHelpers.readFileSync(manifestPath, "utf8");
        break;
      } catch {
        // Try the legacy manifest path next.
      }
    }
    if (!manifestContent) {
      throw new Error("No mcp-use build manifest found");
    }
    const manifest = JSON.parse(manifestContent) as WidgetBuildManifest;
    ...
    if (
      ...
      widgetsMetadata = manifest.widgets as Record<
        string,
    ...
    if (
      ...
      for (const entry of Object.values(manifest.inlineWidgetTools)) {
        const widgetName = entry.widgetName;
        if (!widgetName) continue;
        if (!widgets.includes(widgetName)) widgets.push(widgetName);
        widgetsMetadata[widgetName] = {
          ...(widgetsMetadata[widgetName] || {}),
          description: `Inline widget: ${widgetName}`,
          metadata: {
  ...
  for (const widgetName of widgets) {
    ...
    const metadata = (widgetsMetadata[widgetName] || {}) as WidgetMetadata;
