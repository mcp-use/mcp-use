...
import { createParamsSchema } from "../tools/schema-helpers.js";
import {
...
interface RegistrationEntry<TConfig> {
  config: TConfig;
  handler?: unknown;
}

type WidgetToolCallback = (
...
interface UIResourceServer {
  ...
  registrations?: {
    tools: Map<string, RegistrationEntry<ToolDefinition>>;
    resources: Map<string, RegistrationEntry<ResourceDefinition>>;
    resourceTemplates: Map<
      string,
      RegistrationEntry<ResourceTemplateDefinition>
    >;
  };
  ...
  tool: (definition: ToolDefinition, callback?: WidgetToolCallback) => unknown;
  propagateWidgetResourcesToSessions?(widgetName: string): void;
  addWidgetTool?(
    definition: ToolDefinition,
    callback: WidgetToolCallback
  ): void;
}
 ...
 *
 * Supports UI resource types:
 * - appsSdk: OpenAI Apps SDK compatible widgets (text/html+skybridge)
 * - mcpApps: MCP Apps Extension (SEP-1865) widgets
 *
 ...
 * @param definition.type - Type of UI resource (appsSdk or mcpApps)
...
export function uiResourceRegistration<T extends UIResourceServer>(
  ...
  switch (enrichedDefinition.type) {
    ...
    default:
      throw new Error(
        `Unsupported UI resource type. Must be one of: appsSdk, mcpApps`
      );
  }
  ...
  if (!isUpdate) {
    ...
    if (
      ...
      server.resourceTemplate(
        ...
        templateReadCallback as unknown as ReadResourceTemplateCallback
      );
    }
  ...
  if (
    !isUpdate &&
    server.sessions &&
    typeof server.propagateWidgetResourcesToSessions === "function"
  ) {
  ...
  if (exposeAsTool) {
    ...
    if (propsOrSchema && typeof propsOrSchema === "object" && !isZodSchema) {
      ...
      const hasTypeObject =
        (propsOrSchema as { type?: unknown }).type === "object" &&
    ...
    if (isZodSchema) {
      toolDefinition.schema = propsOrSchema as z.ZodObject<z.ZodRawShape>;
      ...
      if (jsonSchema.properties) {
        ...
        const inputs = Object.entries(jsonSchema.properties).map(
          ([name, prop]) => ({
            name,
        ...
        toolDefinition.schema = z.object(createParamsSchema(inputs));
      }
    } else if (propsOrSchema) {
      toolDefinition.schema = z.object(
        createParamsSchema(
          convertPropsToInputs(
