...
export async function readBuildManifest(): Promise<{
  ...
  widgets: string[] | Record<string, unknown>;
  buildTime?: string;
  buildId?: string;
} | null> {
  const cwd = isDeno ? "." : getCwd();
  const manifestPaths = [
    pathHelpers.join(cwd, ".mcp-use", "build", "manifest.json"),
    pathHelpers.join(cwd, "dist", "mcp-use.json"),
  ];
...
type WidgetRegistrationMetadata = Record<string, unknown> & {
  title?: string;
  description?: string;
  props?: unknown;
  inputs?: unknown;
...
function createWidgetRegistration(
  widgetName: string,
  metadata: WidgetRegistrationMetadata,
  html: string,
  ...
  if (widgetType === "appsSdk") {
    const appsSdkMetadata = metadata.appsSdkMetadata;
    const widgetCSP = appsSdkMetadata?.["openai/widgetCSP"];
    return {
      ...baseRegistration,
      ...
      appsSdkMetadata: {
        ...
        "openai/widgetCSP": {
          ...
          ...(widgetCSP?.frame_domains
            ? { frame_domains: widgetCSP.frame_domains }
            : {}),
          // redirect_domains for openExternal redirects (optional per OpenAI spec)
          ...(widgetCSP?.redirect_domains
            ? { redirect_domains: widgetCSP.redirect_domains }
            : {}),
        },
      },
    };
...
): WidgetRegistrationMetadata {
  const result: WidgetRegistrationMetadata = { ...metadata };
