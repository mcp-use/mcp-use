 ...
 * - Uses useView() for `props` (structured tool output) and useStreamableProps() for streamed updates
 * - Always mounts the component with `<Component {...mergedProps} />` (no isPending gate)
 * - Does not pass host/runtime fields (isPending, toolInput, theme, etc.) as props — use useView() in the component
...
import { useView, useStreamableProps } from 'mcp-use/react';
import './styles.css';
...
if (container && Component) {
  const rootContainer = container as HTMLElement & { __mcpRoot?: ReturnType<typeof createRoot> };
  let root = rootContainer.__mcpRoot;
  if (!root) {
    root = createRoot(container);
    rootContainer.__mcpRoot = root;
  }
  root.render(<Wrapper />);
 ...
 */
interface InlineWidgetTempFiles {
  entryTsx: string;
  indexHtml: string;
  stylesCss: string;
}
 ...
 * @internal CLI build pipeline — consumed by inline-widget-bundler.ts
 */
