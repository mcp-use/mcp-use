 ...
 *
 * Shared utilities for MCP Apps metadata building and widget URI generation.
...
/**
 * Create singleton instance of the MCP Apps protocol adapter
 */
...
export function buildDualProtocolMetadata(
  ...
  // Build tool metadata for MCP Apps
  const mcpAppsToolMeta = adapter.buildToolMetadata(definition, uri);

  // Per SEP-1865: Tool _meta.ui only has resourceUri.
  return {
    ...existingMetadata,
...
export function buildResourceUiMeta(
  ...
  const adapter = createProtocolAdapter();
  const mcpAppsResourceMeta = adapter.buildResourceMetadata(definition);
  const uiMeta =
