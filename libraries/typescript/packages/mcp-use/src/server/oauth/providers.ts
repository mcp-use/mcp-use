 ...
 * ```
 */
export function oauthSupabaseProvider(config: Partial<SupabaseProviderConfig> = {}): OAuthProvider {
 ...
 * ```
 */
export function oauthAuth0Provider(config: Partial<Auth0ProviderConfig> = {}): OAuthProvider {
 ...
 * ```
 */
export function oauthKeycloakProvider(config: Partial<KeycloakProviderConfig> = {}): OAuthProvider {
 ...
 * ```
 */
export function oauthWorkOSProvider(config: Partial<WorkOSProviderConfig> = {}): OAuthProvider {
  ...
  if (clientId) {
    ...
    console.log('[WorkOS OAuth]   - Make sure this client exists in WorkOS Dashboard');
    console.log('[WorkOS OAuth]   - Configure redirect URIs to match your MCP client');
  } else {
    console.log('[WorkOS OAuth] Using Dynamic Client Registration (DCR) mode');
    console.log('[WorkOS OAuth]   - MCP clients will register themselves automatically');
    console.log('[WorkOS OAuth]   - Make sure DCR is enabled in WorkOS Dashboard');
  }
 ...
 * ```
 */
export function oauthCustomProvider(config: CustomProviderConfig): OAuthProvider {
