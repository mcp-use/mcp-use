...
export function setupOAuthRoutes(
  ...
  if (mode === 'proxy') {
    ...
    const handleAuthorize = async (c: Context) => {
      ...
      authUrl.searchParams.set('code_challenge_method', codeChallengeMethod as string || 'S256');
  ...
  const handleAuthorizationServerMetadata = async (c: Context) => {
    ...
    if (mode === 'direct') {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(`[OAuth] Failed to fetch provider metadata: ${response.status}`);
          return c.json({
            error: 'server_error',
        ...
        const hasRegisteredClient = provider.getRegistrationEndpoint && 
          (provider as any).config?.clientId;
        
        if (hasRegisteredClient) {
          console.log(`[OAuth] Provider has pre-registered client - removing DCR endpoint`);
        ...
        console.log(`[OAuth]   - Registration endpoint: ${metadata.registration_endpoint || 'not available (using pre-registered client)'}`);
  ...
  app.get('/.well-known/oauth-protected-resource', (c: Context) => {
    ...
    return c.json({
      ...
      resource_documentation: mode === 'direct' 
