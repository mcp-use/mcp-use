...
function packFramework(destination: string): string {
  // `files` is just ["dist"] and prepack only checks the README, so packing an
  // unbuilt package produces a tarball with no framework in it.
  if (
    !existsSync(join(workspaceRoot, "packages", "server", "dist", "bin.js"))
  ) {
    throw new Error(
      "packages/server is not built, so the packed mcp-use tarball would be " +
        "empty. Run `pnpm --filter mcp-use build` first."
    );
  }
  const packed = run(
    "pnpm",
  ...
  // pnpm reports an absolute path today; resolving against the destination is
  // a no-op for that and still correct if a version returns a bare name.
  return resolve(destination, filename);
}

/**
 * Quote a `cmd.exe` argument.
 *
 * Quotes anything that is not plainly safe rather than listing the characters
 * that are dangerous, because the dangerous set is easy to under-count: `(`,
 * `)` and `%` all mean something to cmd.exe, and `%` still expands inside
 * double quotes, so it is escaped as `%%`.
 */
