...
import { bindBasicToolToView } from "./helpers.js";
...
describe("runBuild", () => {
  ...
  it.for([
    ...
    "builds a tool-only server with %s",
  ...
  it("honors an --entry override", async ({ project }) => {
    const { cwd } = project;
  ...
  it("does not overwrite an existing mcp-env.d.ts", async ({ project }) => {
    const { cwd } = project;
...
describe("runBuild (views)", () => {
  ...
  viewsTest(
    "resolves host tsconfig aliases in standalone view builds",
    async ({ project }) => {
      const { cwd } = project;
  ...
  viewsTest(
    "builds external view assets by default",
    async ({ project }) => {
      const { cwd } = project;
      ...
      try {
        ...
        const resources = (
          listBody["result"] as { resources: { uri: string }[] }
        ).resources;
        expect(
          resources.some(
        ...
        expect(resourceDomains?.some((d) => d.includes("localhost"))).toBe(
          true
        );
  ...
  viewsTest(
    "embeds view JavaScript and CSS in resources with --inline",
    async ({ project }) => {
      const { cwd } = project;
  ...
  viewsTest(
    "builds a view that contains </script> in source as external assets",
    async ({ project }) => {
      const { cwd } = project;
  ...
  it("emits source maps only when requested", async ({ project }) => {
    const { cwd } = project;
  ...
  viewsTest(
    "builds a view module that uses browser globals at module scope",
    async ({ project }) => {
      const { cwd } = project;
  ...
  it("ignores resources/<name>/widget.tsx entries", async ({ project }) => {
    const { cwd } = project;
  ...
  viewsTest(
    "fails when a tool binds a missing view",
    async ({ project }) => {
      const { cwd } = project;
  ...
  viewsTest(
    "warns on unbound views but still succeeds",
    async ({ project }) => {
      const { cwd } = project;
  ...
  viewsTest(
    "rewrites manifest asset paths when MCP_ASSETS_URL is set",
    async ({ project }) => {
      const { cwd } = project;
  ...
  viewsTest(
    "uses server basePath in CDN manifest when MCP_ASSETS_URL is set",
    async ({ project }) => {
      const { cwd } = project;
