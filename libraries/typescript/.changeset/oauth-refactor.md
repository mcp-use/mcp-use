...
- Provider configurations no longer accept `clientId`/`clientSecret` - use the new `oauthProxyProvider` helper for providers that don't support DCR

**New Features:**
- Added `oauthProxyProvider` helper for creating proxy-mode OAuth providers (useful for Google, GitHub, etc.)
