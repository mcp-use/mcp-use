...
- Resources, resource templates, prompts, and views always require sign-in with the provider's `requiredScopes`.
...
The `mixed-oauth` example now uses this API instead of a hand-rolled gate, covers every `securitySchemes` shape plus sign-in resources, prompts, and views, and can run behind `mcp-use dev --tunnel` for testing in Claude and ChatGPT.
