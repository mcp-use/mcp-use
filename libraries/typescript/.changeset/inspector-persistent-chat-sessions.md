...
- **Normalized storage** — session metadata and messages are stored in
  separate IndexedDB stores (`mcp-inspector-sessions-db` /
  `mcp-inspector-messages-db`) to keep session list rendering fast and avoid
...
- **Storage usage indicator** — live IndexedDB quota bar at the bottom of the
  sidebar with an amber warning when usage exceeds 80% of quota
