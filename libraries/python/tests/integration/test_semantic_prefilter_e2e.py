...
# Cache result of OpenAI embeddings API probe (one check per test run)
_openai_embeddings_available: bool | None = None


async def _require_openai_embeddings() -> None:
    """Skip the test if OpenAI embeddings API is unavailable (e.g. 429 rate limit)."""
    global _openai_embeddings_available
    if _openai_embeddings_available is False:
        pytest.skip("OpenAI embeddings API unavailable (e.g. 429 rate limit)")
    if _openai_embeddings_available is True:
        return
    prefilter = SemanticPreFilter(
        embeddings_url="https://api.openai.com/v1/embeddings",
        embeddings_api_key=os.getenv("OPENAI_API_KEY"),
    )
    embedding = await prefilter._get_embedding("probe", use_cache=False)
    if not embedding:
        _openai_embeddings_available = False
        pytest.skip("OpenAI embeddings API unavailable (429 rate limit or other error)")
...
def sample_tools():
    ...
    return [
        ...
        MockTool(
            ...
            {"type": "object", "properties": {"directory": {"type": "string", "description": "Directory path"}}},
        ),
        # Database tools
        MockTool(
            "query_database",
            "Execute a SQL query on the database",
            {"type": "object", "properties": {"sql": {"type": "string", "description": "SQL query string"}}},
        ),
        MockTool(
            "create_table",
        ...
        MockTool(
            ...
            {"type": "object", "properties": {"numbers": {"type": "array", "items": {"type": "number"}}}},
        ),
        MockTool(
            "calculate_average",
        ...
        MockTool(
            ...
            {"type": "object", "properties": {"document": {"type": "string", "description": "Document content"}}},
        ),
        MockTool(
            "translate_text",
...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not os.getenv("OPENAI_API_KEY"), reason="OPENAI_API_KEY not set - skipping tests that require real API calls"
)
class TestSemanticPreFilterE2E:
    ...
    async def test_filter_tools_with_file_query(self, sample_tools):
        ...
        await _require_openai_embeddings()
