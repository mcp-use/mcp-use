...
class LitellmMCPAgent:
    ...
    def __init__(
        self, model: str, client: MCPClient, disallowed_tools: list[str] | None = None, system_prompt: str | None = None
    ):
        if model is None:
    ...
    async def run(self, query: str):
        ...
        messages = [
            {
                "role": "system",
                "content": self.system_prompt or "You are a helpful assistant that can use tools to answer questions.",
            },
        ...
        if response.choices[0].message.tool_calls:
            ...
            for tool_call in response.choices[0].message.tool_calls:
                ...
                messages.append({"role": "tool", "tool_call_id": tool_id, "name": tool_name, "content": tool_result})
    ...
    async def _execute_tool(self, tool_name: str, tool_args: dict[str, Any]):
        ...
        try:
            ...
            for (
                session
            ) in self._sessions.values():  # That's a bit inefficient, we're going to engineer a better solution
                try:
