...
For a deeper explanation of widget state, model-visible context, and the `useWidget()` data model, see the [useWidget API reference](/v2/typescript/api-reference/react/usewidget).
...
Apps SDK APIs remain useful for ChatGPT-specific host extensions that are not part of the shared MCP Apps spec yet. Use [Apps SDK compatibility](/v2/typescript/mcp-apps/apps-sdk-compatibility) when you need to understand that boundary.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="openai"
    href="/v2/typescript/mcp-apps/apps-sdk-compatibility"
  >
