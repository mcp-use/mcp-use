...
It is called with the `MCPClient` as `this`, and owns cleanup for whatever
runtime it wraps — `client.close()` does not release anything on its behalf.

```typescript
{ codeMode: {
    enabled: true,
    executor: async (code, timeout) => ({
      result: await myRuntime.run(code, timeout),
      logs: [],
      error: null,
      execution_time: 0,
    }),
