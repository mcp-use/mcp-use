...
`ErrorBoundary` is included automatically when you use [`McpUseProvider`](/v2/typescript/api-reference/react/mcpuseprovider), so you usually do not need to mount it yourself. The provider wraps your widget content with the boundary as the innermost wrapper, ensuring all render errors in the subtree are caught.
...
- [`McpUseProvider`](/v2/typescript/api-reference/react/mcpuseprovider) includes `ErrorBoundary` automatically.
- [`useWidget`](/v2/typescript/api-reference/react/usewidget) is the widget hook for accessing props and actions.
