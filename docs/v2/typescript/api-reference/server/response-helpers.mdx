...
<Warning>
**Deprecated.** Prefer returning raw MCP envelopes (`CallToolResult`, `ReadResourceResult`, `GetPromptResult`). These helpers remain for upgrade compatibility and map to those shapes. See [Response Helpers](/v2/typescript/server/response-helpers) for the migration table.
</Warning>
...
### markdown
Creates a Markdown content response. Identical shape to [`text`](/v2/typescript/api-reference/server/response-helpers), but sets the `_meta.mimeType` to `text/markdown` so clients render it as Markdown.
...
If `data` is an array, `object` delegates to [`array`](/v2/typescript/api-reference/server/response-helpers) internally (array as `structuredContent` directly — no `{ data }` wrap). Note that the `_meta.mimeType` is only set on the non-array path.
...
<Note>
Arrays are automatically wrapped in an object with a `data` field. If you pass an array to [`object`](/v2/typescript/api-reference/server/response-helpers), it calls `array()` internally.
</Note>
...
### binary
Creates a binary content response from base64-encoded data. The base64 string is placed in a `text` content item, `_meta.mimeType` is set to the provided MIME type, and `_meta.isBinary` is set to `true`. Unlike [`image`](/v2/typescript/api-reference/server/response-helpers) and [`audio`](/v2/typescript/api-reference/server/response-helpers), `mimeType` is required.
...
### widget
Creates a response for a tool that renders a widget. The widget configuration (name, invoking and invoked status, and so on) is set on the tool's `widget` property at registration time. `widget()` carries only the runtime data described by [`WidgetResponseConfig`](/v2/typescript/api-reference/server/response-helpers).
...
Runtime configuration passed to [`widget`](/v2/typescript/api-reference/server/response-helpers). It carries only runtime data, not the widget's registration-time configuration. All fields are optional.
