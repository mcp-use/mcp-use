...
Resources expose data and content that MCP clients can read by URI: configuration, files, database records, API responses, and more. Register a single fixed resource with [`server.resource()`](/v2/typescript/api-reference/server/resources#server-resource), or register a parameterized family of resources with [`server.resourceTemplate()`](/v2/typescript/api-reference/server/resources#server-resourcetemplate). Both methods are members of the `MCPServer` class and return `this`, so calls can be chained.
...
Configuration object for a static resource registered through [`server.resource()`](/v2/typescript/api-reference/server/resources#server-resource). This is the inline-callback shape: it carries a required `mimeType` and a `readCallback`. To pass the callback as the second argument to `server.resource()` instead, use `ResourceDefinitionWithoutCallback`, whose `mimeType` is optional and inferred from response helpers such as `text()` or `object()`.
