...
<Callout type="warn" title="Removed in v2">
  ...
  not exported from `mcp-use` or the temporary `mcp-use/server` compatibility
  entry. Use Zod schemas with [`ctx.elicit()`](/v2/typescript/api-reference/server/tool-context#elicit)
  instead.
</Callout>
...
See the [Elicitation guide](/v2/typescript/server/elicitation) for form vs URL mode,
...
[`ctx.elicit()` overload](/v2/typescript/api-reference/server/tool-context#elicit)
...
<CardGroup cols={2}>
  <Card title="Elicitation guide" icon="check" href="/v2/typescript/server/elicitation">
    When to use form vs URL mode and how to handle user decisions.
  </Card>
  <Card title="Tool context: elicit" icon="terminal" href="/v2/typescript/api-reference/server/tool-context#elicit">
