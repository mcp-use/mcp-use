...
Low-level utilities for building MCP UI resources, dual-protocol widget metadata, and SEO landing pages. Most servers register widgets declaratively with `server.uiResource(...)` (see [UIResource](/v2/typescript/api-reference/server/resources)), which calls these helpers internally. The functions and types here are the building blocks for that pipeline and are also exported for advanced use.
...
Create a `UIResourceContent` from a high-level [UIResourceDefinition](/v2/typescript/api-reference/server/resources). This is the main router: it inspects the discriminated `type` field and delegates to the appropriate builder.
