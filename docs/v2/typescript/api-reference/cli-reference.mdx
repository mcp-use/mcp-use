...
| `--with-inspector` | Mount the bundled Inspector under `basePath`, so `/inspector` when `basePath` is `/`. | Disabled                              |
...
**Notes and behavior:** The command reads `~/.mcp-use/config.json`. When that file selects no organization, it reports the account default, matching `org current` and the `servers`/`deployments` commands. This is the usual case when authenticating with `MCP_USE_API_KEY` instead of `mcp-use login`.
...
| `--width <px>`              | Host and widget width, an integer from `1` to `10000000`.          | `768`                        |
| `--height <px>`             | Host viewport height, an integer from `1` to `10000000`. The PNG is cropped to the widget bounds. | `720`                        |
