...
<CodeGroup>
  ```bash npm npm install supabase --save-dev ``` ```bash pnpm pnpm add -D
  supabase ``` ```bash bun bun add -d supabase ``` ```bash brew brew install
...
<Note>
  **Prerequisites for the script** - Supabase CLI installed (`npm install -g
  supabase`) - Project initialized (`supabase init`) - Logged in to Supabase
  (`supabase login`) - Project linked (`supabase link --project-ref
  YOUR_PROJECT_REF`) - Docker running (for deployment)
</Note>
...
Widgets in the resources folder are automatically registered. Additional widgets can be registered manually. (see [UI Widgets](/v2/typescript/mcp-apps))
...
# CSP_URLS: Supabase project URL (for Content Security Policy). See [Content Security Policy](/v2/typescript/mcp-apps/content-security-policy) for full CSP docs.
...
<CodeGroup>
  ```bash npm npm run build ``` ```bash pnpm pnpm build ``` ```bash bun bun run
  build ```
</CodeGroup>
...
**Alternative**: You can also configure CSP per widget with `widgetMetadata.metadata.csp` instead of using the global `CSP_URLS` environment variable. See [Content Security Policy](/v2/typescript/mcp-apps/content-security-policy) for examples.
...
<Note>
  This typically happens after multiple failed connection attempts or when the
  connection is interrupted unexpectedly. The ban is temporary and automatic.
</Note>
...
- [mcp-use Documentation](/v2/typescript/getting-started)
