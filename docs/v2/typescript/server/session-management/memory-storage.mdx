...
This guide focuses on when to use in-memory storage. Use the [Sessions API reference](/v2/typescript/api-reference/server/sessions#inmemorysessionstore) for exact method behavior and the `SessionStore` contract.
...
| Need | Use |
| --- | --- |
| Hot reload during local development | [File System Storage](/v2/typescript/server/session-management/filesystem-storage) |
| Load-balanced or distributed servers | [Redis Storage](/v2/typescript/server/session-management/redis-storage) |
...
<CardGroup cols={2}>
  <Card title="File System Storage" icon="folder" href="/v2/typescript/server/session-management/filesystem-storage">
    Keep sessions across hot reloads in local development.
  </Card>
  <Card title="Redis Storage" icon="/images/icons/redis.svg" href="/v2/typescript/server/session-management/redis-storage">
    Share session metadata and streams across server instances.
  </Card>
  <Card title="Sessions API reference" icon="terminal" href="/v2/typescript/api-reference/server/sessions">
    Look up session store methods, defaults, and contracts.
  </Card>
  <Card title="ServerConfig API reference" icon="server" href="/v2/typescript/api-reference/server/server-config">
