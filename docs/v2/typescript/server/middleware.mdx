...
This guide focuses on where middleware fits and how to apply common patterns. Use the [Middleware and proxy API reference](/v2/typescript/api-reference/server/middleware) for exact context fields, pattern matching rules, adapter behavior, and proxy options.
...
Use [server authentication](/v2/typescript/server/authentication/index) to verify the bearer token before relying on `ctx.auth`.
