---
title: "User Context"
description: "Use verified OAuth identity inside TypeScript MCP server callbacks."
icon: "user"
---
...
declare function createYourVerifier(resource: URL): OAuthTokenVerifier;
...
const server = new MCPServer({
  ...
  oauth: oauthCustomProvider<AppUser>({
    ...
    createTokenVerifier(resource) {
      return createYourVerifier(resource);
    },
...
`createTokenVerifier` receives the canonical MCP resource and must validate it when verifying each token. `mapAuthInfo` runs only after SDK auth information has been verified and must return `{ user, payload, permissions }`. It does not rewrite `ctx.auth.scopes`; those come from the verifier's verified auth info.
