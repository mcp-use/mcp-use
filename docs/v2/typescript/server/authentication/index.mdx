...
To let clients connect and use some tools before signing in, add `mixedAuth: true` and declare `auth` on each public or optional tool, resource, or prompt. See [Mixed Authentication](/v2/typescript/server/authentication/mixed-auth).
