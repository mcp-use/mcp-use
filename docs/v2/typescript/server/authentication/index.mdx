...
To let clients connect and call some tools before signing in, add `mixedAuth: true` and declare `securitySchemes` with `{ type: "noauth" }` on each public or optional tool. See [Mixed Authentication](/v2/typescript/server/authentication/mixed-auth).
...
<CardGroup cols={2}>
  ...
  >
    Serve public, optional, and sign-in tools from one endpoint.
  </Card>
  <Card
