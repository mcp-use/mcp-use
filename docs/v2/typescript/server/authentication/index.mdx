...
<Warning>
  mcp-use does not include OAuth Proxy or fixed-client token brokering. If an
  identity provider gives you only one pre-registered client ID and secret, put
  a standards-compliant authorization server or broker in front of it.
</Warning>
