...
To let clients connect and call some tools before signing in, add `allowAnonymous: true` and declare each tool's `securitySchemes`. See [Mixed Authentication](/v2/typescript/server/authentication/mixed-auth).
...
<CardGroup cols={2}>
  ...
  >
    Serve public and protected tools from one endpoint.
  </Card>
  <Card
