...
- A tool's view resource follows its tool. The view of a public or optional tool can be read signed out, so its UI loads before sign-in. The view of a sign-in tool needs a token with the baseline scopes.
...
The [mixed-oauth example](https://github.com/mcp-use/mcp-use/tree/canary/libraries/typescript/packages/server/examples/auth/mixed-oauth) runs a Better Auth authorization server and a `mixedAuth` server on `http://localhost:3000/mcp`. `public_ping` is public, `welcome` is optional with the `profile` scope, and `protected_profile` omits `auth`. From the example's directory:
...
To test the full sign-in and retry loop in a host, expose the server over a public HTTPS tunnel and add it as a custom connector. Ask for the public tool first, then the sign-in one.
