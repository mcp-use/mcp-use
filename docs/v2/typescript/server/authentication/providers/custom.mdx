...
| Member | Purpose |
| --- | --- |
| `resource`, `basePath` | The resolved canonical resource URL and the MCP endpoint path. |
| `use(pattern, handler)` | Registers `mcp:` middleware, like `server.use()`. |
| `registerTool`, `registerResource` | Register provider-owned tools and static resources. |
...
Provider-owned tools and resources sit behind the same bearer gate as application items, and `ctx.auth` carries the provider's user type. Registering a name that the application already uses throws, so reserved names never silently replace user tools.
