...
description: "Integrate a DCR-capable OAuth authorization server with a TypeScript MCP server."
icon: "code"
---
...
<Warning>
  A custom provider is not an OAuth proxy. Fixed-client brokering requires an
  external authorization-server implementation; mcp-use never receives or stores
