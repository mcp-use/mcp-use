...
const oauth = oauthCustomProvider<AppUser>({
  createTokenVerifier,
  oauthMetadata,
  mapAuthInfo,
  setup(host: OAuthProviderHost<AppUser>) {
    ...
    host.use("mcp:tools/call", async (ctx, next) => {
      // Let the consent tool through, or consent could never be recorded.
      if (ctx.params.name === "grant_consent") return next();
...
`setup` must be synchronous, and the host only works while it runs; calling it later throws. If `setup` throws or returns a promise, the server does not start, and every later request fails with the same error.
