...
If your provider only supports a pre-registered application with a fixed `clientId` and `clientSecret`, use [OAuth Proxy](/v2/typescript/server/authentication/providers/oauth-proxy) instead.

This guide shows the shape of a custom provider. Use the [auth providers API reference](/v2/typescript/api-reference/server/auth-providers#oauthcustomprovider) for exact required fields, optional fields, defaults, and return contracts.
...
<CardGroup cols={2}>
  ...
  <Card title="Custom provider API reference" icon="terminal" href="/v2/typescript/api-reference/server/auth-providers#oauthcustomprovider">
