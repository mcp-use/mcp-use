...
Normalize a stable provider account identifier into `payload.sub`, even when
using a custom `getUserInfo` mapper. If an initial verified payload has a string
`sub`, refreshing must return the same subject; a changed or missing subject
terminates that grant. For custom payloads without `sub`, your verification
logic is responsible for account continuity.

Use timeouts in your own verification/API hooks, as shown above. The proxy's
`timeoutMs` bounds its token-endpoint requests, not arbitrary application hooks.
...
keeps secrets inside the process, but registrations and sessions disappear on
restart and are not shared across server instances.

Persistent custom stores must encrypt every payload before writing it and
declare `secretProtection: "store-encrypted"`. The proxy rejects persistent
stores without that declaration; it does not manage application encryption
keys.

Custom stores implement one byte-oriented multi-key `transaction` method. Its
callback receives create, read, replace, and consume operations. Transactions
must be serializable across all instances that share the store. Import
`OAuthProxyStore` from `mcp-use/oauth` for the complete contract.

The built-in store is bounded to 10,000 entries and 64 MiB of payload bytes;
it rejects writes when full. Use a shared encrypted store for multiple replicas
and apply rate limiting to public authorization and registration endpoints at
your gateway. The built-in HTTP logger emits only method/path/status summaries
for proxy endpoints, even at trace level, so codes, tokens and consent secrets
are not dumped. Custom logging middleware must apply equivalent protection.

Local scopes describe MCP permissions; `upstreamScopes` describe permissions at
the provider and may be different. The application must choose an appropriate
mapping. Local revocation does not revoke the upstream provider's grant.
...
| `store`                              | Optional process-local or store-encrypted transactional state storage.   |
...
## Run the credential-free acceptance test

From the repository's `libraries/typescript` directory:

```sh
pnpm install --frozen-lockfile
pnpm build
pnpm --filter mcp-use exec vitest run tests/oauth-proxy.e2e.test.ts
```

The test runs real localhost HTTP servers with a simulated non-DCR provider.
The official MCP SDK performs discovery, registration, PKCE and token exchange,
then calls a protected tool. It checks refresh, expiry, replay rejection and
revocation against both source and built package entrypoints. No real provider
credentials are needed; a real GitHub login remains a separate integration test.

This generic broker does not itself implement Firebase authentication. A
Firebase-specific adapter or login bridge still needs its own implementation
and real-project validation.
