...
If your provider supports Dynamic Client Registration, use a built-in provider or [Custom Provider](/v2/typescript/server/authentication/providers/custom). In DCR mode, clients register directly with the upstream provider and the MCP server only verifies tokens.

This guide covers the proxy workflow. Use the [auth providers API reference](/v2/typescript/api-reference/server/auth-providers#oauthproxy) for exact `oauthProxy()` options, defaults, payload contracts, and verifier behavior.
...
<CardGroup cols={2}>
  ...
  <Card title="OAuth Proxy API reference" icon="terminal" href="/v2/typescript/api-reference/server/auth-providers#oauthproxy">
