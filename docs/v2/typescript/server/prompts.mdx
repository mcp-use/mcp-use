...
This guide focuses on prompt design and common patterns. Use the [Prompts API reference](/v2/typescript/api-reference/server/prompts) for exact `server.prompt()` signatures, callback types, completion overloads, and return shapes.
...
| Display interactive UI | [Tool with an MCP App widget](/v2/typescript/mcp-apps) |
...
See [Response Helpers](/v2/typescript/server/response-helpers) for a chooser and [Response helpers API reference](/v2/typescript/api-reference/server/response-helpers) for exact conversion behavior.
...
Use `ctx.auth` only for verified identity from server authentication. Throw when the prompt request should fail instead of returning prompt content. See [User Context](/v2/typescript/server/authentication/user-context) for guide-level patterns and [Tool context API reference](/v2/typescript/api-reference/server/tool-context) for exact context fields.
...
See [Notifications](/v2/typescript/server/notifications) for server-to-client notification patterns.
...
<CardGroup cols={2}>
  <Card title="Prompts API reference" icon="terminal" href="/v2/typescript/api-reference/server/prompts">
    Look up prompt definitions, callback signatures, completion helpers, and return types.
  </Card>
  <Card title="Response Helpers" icon="drill" href="/v2/typescript/server/response-helpers">
  ...
  <Card title="Resources" icon="folder-open" href="/v2/typescript/server/resources">
