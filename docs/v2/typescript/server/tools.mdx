...
This guide focuses on tool design and common implementation patterns. Use the [Tools API reference](/v2/typescript/api-reference/server/tools) for `ToolDefinition`, callback signatures, type inference, defaults, and return types.
...
Prefer raw MCP `CallToolResult` shapes. Deprecated [response helpers](/v2/typescript/server/response-helpers) still work for upgrades, but new code should return the wire envelopes directly.
...
See [Response Helpers](/v2/typescript/server/response-helpers) for the deprecated-helper migration table and [Response helpers API reference](/v2/typescript/api-reference/server/response-helpers) for signatures.
...
The model sees `content`. The view reads `structuredContent` via `useToolContext()`. See [MCP Apps](/v2/typescript/mcp-apps) for view workflow guidance.
...
See the [Tool context API reference](/v2/typescript/api-reference/server/tool-context) for every `ctx` method, field, capability check, log level, and return type.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="drill"
    href="/v2/typescript/server/response-helpers"
  >
    Choose the right helper for text, JSON, errors, resources, media, and
    widgets.
  </Card>
  <Card title="MCP Apps" icon="blocks" href="/v2/typescript/mcp-apps">
