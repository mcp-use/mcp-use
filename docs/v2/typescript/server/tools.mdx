...
verified provider user for access control. On a server with
`mixedAuth: true`, a tool's `securitySchemes` field marks it public
(`noauth`), optional (`noauth` and `oauth2`), or sign-in with extra `scopes`;
see
[Mixed Authentication](/v2/typescript/server/authentication/mixed-auth).
