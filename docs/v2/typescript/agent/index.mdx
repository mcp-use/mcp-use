...
Use `MCPAgent` when you want the model to decide which MCP tools to call. Use [`MCPClient`](/v2/typescript/client/index) directly when your application should choose exact tools, resources, or prompts itself.
...
Install `@mcp-use/agent`, `@mcp-use/client`, and one LangChain provider package for the LLM you want to use.
...
For constructor options, defaults, run options, memory methods, streaming signatures, and observability methods, see the [MCPAgent API Reference](/v2/typescript/api-reference/agent/mcp-agent).
...
See [Structured Output](/v2/typescript/agent/structured-output) for details.
...
`agent.prettyStreamEvents` and `agent.streamEvents` give formatted and low-level output. See [Streaming](/v2/typescript/agent/streaming).
...
Pass `memoryEnabled: false` to `new MCPAgent({ ... })` to disable it. See [Memory Management](/v2/typescript/agent/memory-management).
...
<CardGroup cols={3}>
  <Card
    ...
    icon="brain-circuit"
    href="/v2/typescript/agent/llm-integration"
  >
  ...
  <Card
    ...
    icon="braces"
    href="/v2/typescript/agent/structured-output"
  >
  ...
  <Card
    ...
    icon="server"
    href="/v2/typescript/agent/server-manager"
  >
  ...
  <Card
    title="Observability"
    icon="chart-line"
    href="/v2/typescript/agent/observability"
  >
  ...
  <Card
    ...
    icon="terminal"
    href="/v2/typescript/api-reference/agent/mcp-agent"
  >
