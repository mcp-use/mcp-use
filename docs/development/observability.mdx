...
<Warning>
  **Completely Optional**: Observability is entirely opt-in and requires zero
  code changes to your existing workflows.
</Warning>
...
<Info>
  **Advanced Debugging**: LangChain offers LangSmith, a powerful tool for
  debugging agent behavior that integrates seamlessly with mcp-use.
</Info>
...
<Tip>
  LangSmith provides detailed traces of your agent's execution, making it easier
  to understand complex multi-step workflows.
</Tip>
...
<Tip>
  **Pro Tip**: Start with one platform first to get familiar with observability,
  then add the second platform if you need different features or perspectives.
</Tip>
