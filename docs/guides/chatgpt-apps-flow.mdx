...
<Info>
This guide focuses on shipping an MCP App to ChatGPT specifically. For the general reference on widgets, props, CSP, and auto-discovery across clients, see [MCP Apps](/typescript/server/mcp-apps).

**Best Practice**: Build widgets once with the MCP Apps stack. They run in ChatGPT, Claude, and any MCP-compatible client from a single codebase.
...
mcp-use handles the local server, widget bundling, Inspector integration, and metadata generation for ChatGPT widgets.
...
2. **Automatic Widget Discovery**: Drop `.tsx` files in `resources/` folder and they're automatically registered
3. **Built-in Bundling**: Hot reload and production builds work out of the box
4. **Pre-built React Hooks**: Use `useWidget()` for props, state, tool calls, and display modes; [streaming tool input](/typescript/api-reference/react/usewidget#widget-lifecycle) for live previews when the host supports it (MCP Apps / Inspector)
5. **Automatic CSP Configuration**: Content Security Policy is configured automatically
6. **Custom Tool Pattern**: Pair widgets with explicit tools using `widget: { name }` and `widget({ props, output })`
...
await server.listen(3000); // omit the port to use the default (3000); for serverless use server.getHandler()
```
...
- State is persisted by the host (for example, ChatGPT, Claude, or the Inspector)
...
<CardGroup cols={2}>
  <Card
    ...
    icon="paintbrush"
    href="/typescript/server/mcp-apps"
  >
...
- [Response helpers API reference](/typescript/api-reference/server/response-helpers) - Helper functions for tool responses
- [Widget Components Reference](/typescript/api-reference/react/usewidget) - Complete API documentation
