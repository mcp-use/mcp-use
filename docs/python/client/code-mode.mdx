...
#### `__init__(code_mode=True | CodeModeConfig)`

```python
from mcp_use.client.client import CodeModeConfig, SemanticSearchConfig

# Simple boolean (defaults to string_match search)
client = MCPClient(
    config="config.json",
    code_mode=True
)

# With semantic search configuration
client = MCPClient(
    config="config.json",
    code_mode=CodeModeConfig(
        enabled=True,
        semantic=SemanticSearchConfig(
            mode="embeddings",  # "string_match", "fuzzy", or "embeddings"
            embeddings_url="https://api.example.com/v1/embeddings"  # Optional
        )
    )
)
```
...
Search available tools across all servers using the configured search mode.
...
- **`fuzzy`**: Fuzzy string matching using `thefuzz` - better for typos
  - Requires: `pip install thefuzz[speedup]`
- **`embeddings`**: Semantic search using embeddings - best for semantic similarity
  - Requires: `OPENAI_API_KEY` environment variable OR `embeddings_url` in config
...
# String match (default) - simple substring matching
client = MCPClient(
    config="config.json",
    code_mode=CodeModeConfig(
        enabled=True,
...
# Fuzzy search - better for typos and partial matches
client = MCPClient(
    config="config.json",
    code_mode=CodeModeConfig(
        enabled=True,
        semantic=SemanticSearchConfig(mode="fuzzy")
    )
)
# Requires: pip install thefuzz[speedup]
...
# Requires: OPENAI_API_KEY env var OR embeddings_url
```

**Example Usage:**

```python
# With embeddings mode, semantic queries work better
result = await client.search_tools("repo list repositories")
# Returns semantically similar tools even without exact keyword matches
```
