...
<Warning>
**Code Mode executes agent-generated code.** By default that code runs **in-process**
for speed. In-process execution is **not a security boundary**: restricting Python's
namespace cannot contain hostile code (introspection can reach arbitrary objects), so a
prompt-injected agent could run anything the host process can.

- **Trusted code only** (local dev, your own prompts, CI): the default in-process
  executor is fine. It applies best-effort hardening, not isolation.
- **Untrusted or production input**: isolate execution. Pass a custom `code_executor`
  (see [Security](#security)) so code runs in a real sandbox (e.g. an E2B cloud sandbox).
</Warning>
...
Code Mode runs agent-generated Python. **Where that code runs is a security decision.**

### The default executor is not a sandbox

With `code_mode=True`, code runs in-process via the `VMCodeExecutor`, which uses `exec`.
This is **not a security boundary** and is only appropriate for trusted code. Restricting
the namespace does not stop a determined payload: chains like
`().__class__.__base__.__subclasses__()` reach arbitrary objects and can rebuild
`__import__`. This is a known limitation of in-process execution, not a bug.

As defense-in-depth (risk reduction, not a guarantee) the in-process executor:

- rejects `import`, dunder attribute access (`__class__`, `__subclasses__`, ...), denied
  names, and dunder string literals **before** running the code, and
- withholds introspection builtins such as `getattr` and `type` from the namespace.

These make accidental and low-effort escapes harder. They do not make in-process
execution safe for untrusted input.

### Running untrusted code

For untrusted or production input, isolate execution with your own executor via the
`code_executor` parameter:

```python
from mcp_use import MCPClient
from mcp_use.client.executors import BaseCodeExecutor

class MySandboxExecutor(BaseCodeExecutor):
    async def execute(self, code: str, timeout: float = 30.0):
        # Run `code` in a real sandbox (container, microVM, E2B, ...),
        # proxying MCP tool calls back to the host.
        ...
        return {"result": ..., "logs": [], "error": None, "execution_time": 0.0}

client = MCPClient(config="config.json", code_mode=True, code_executor=MySandboxExecutor)
```

`code_executor` accepts:

- `None` or `"vm"` (default) — the in-process `VMCodeExecutor` (trusted code only)
- a `BaseCodeExecutor` subclass — instantiated with the client (as shown above)
- a `BaseCodeExecutor` instance — your own pre-built sandboxed backend
- a `(code, timeout)` callable — returns an execution-result dict. Custom executors
  own timeout enforcement (the built-in VM executor honors `timeout`).

<Warning>
Isolating the *interpreter* is only half the job. A sandboxed executor still proxies MCP
tool calls back to the host, so a prompt-injected script can invoke any tool the client is
connected to (a confused-deputy risk). Scope tool access to what the workload actually
needs in addition to sandboxing the code.
</Warning>

<Note>
This executor seam mirrors the TypeScript SDK, which ships both a VM executor and an E2B
cloud-sandbox executor. A built-in E2B executor for Python is planned; until then,
implement `BaseCodeExecutor` to plug in the sandbox of your choice.
</Note>
...
#### `__init__(code_mode=True, code_executor=None)`

```python
client = MCPClient(
    config="config.json",
    code_mode=True,        # Enable code execution mode
    code_executor=None,    # None/"vm" (default, in-process), a BaseCodeExecutor, or a callable
)
```

<Warning>
The default `code_executor` runs code in-process and is not a security boundary. For
untrusted input, pass a sandboxed `code_executor`. See [Security](#security).
</Warning>
...
# Async: asyncio (curated subset: sleep, gather, wait_for, wait, shield,
#         as_completed, Lock, Event, Semaphore, Queue, ...)
# Exceptions: Exception, ValueError, TypeError, etc.
```

`asyncio` is a curated facade, not the real module: the real one transitively re-exposes
`os`/`subprocess` through plain attribute chains (e.g. `asyncio.base_events.os`), so only the
coroutine helpers above are exposed.

**Blocked (defense-in-depth, not a sandbox):** `import`, `open`, `eval`/`exec`, dunder
attribute access (`__class__`, ...), interpreter-introspection attributes (`f_globals`,
`gi_frame`, `tb_frame`, ...), and introspection builtins like `getattr`/`type`. This
hardens the in-process executor but does not isolate it. See [Security](#security).
