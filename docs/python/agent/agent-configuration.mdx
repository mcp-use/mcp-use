...
agent = MCPAgent(
    ...
    metadata = {"emp_id": value, "email":"a.b@c.com"} # use this to send sensitive data into tools without passing to the LLM
    message_id = "choosen message id in chat usually like a UUID"
)
```
</CodeGroup>
...
- `metadata`: Dictionary of sensitive information to the tools which is not to be passed to LLM
- `message_id`: User specific message id for a chat message usually like a UUID
