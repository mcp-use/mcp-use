...
|----------|-------------------|--------|
| Development/Testing | gpt-4.1-mini, claude-sonnet-4-6 | Lower cost, good performance |
| Production/Complex | gpt-4.1, claude-opus-4-7 | Best performance |
...
llm = ChatOpenAI(model="gpt-4.1")  # No need to pass api_key
```
...
def get_model_for_task(task_type: str):
    if task_type == "complex_reasoning":
        return ChatOpenAI(model="gpt-4.1", temperature=0.1)
