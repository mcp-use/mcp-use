...
Values passed to `run()` override those set in `__init__()`:

```python
# Set defaults at initialization
server = MCPServer(name="My Server", host="0.0.0.0", port=3000)
server.run()  # Uses host="0.0.0.0", port=3000

# Override at runtime
server.run(host="127.0.0.1", port=8080)  # Overrides to 127.0.0.1:8080
```
