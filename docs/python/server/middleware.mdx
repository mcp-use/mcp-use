...
<Note>
**Note about initialize**: The `initialize` request is handled at the MCP protocol/session layer before middleware is invoked. This is by design - it's a bootstrap operation similar to TCP handshake or TLS negotiation. If you need to track initialize events, handle them directly in your telemetry system rather than through middleware.
</Note>
