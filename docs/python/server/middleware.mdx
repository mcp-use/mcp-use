...
`mcp-use` includes optimized middleware for common patterns, so you don't have to write them from scratch.

### Tool Caching
Use `ToolResultCachingMiddleware` to cache results of expensive tool calls (like database queries or API requests). 
It implements **LRU (Least Recently Used)** eviction and **TTL (Time-To-Live)** expiration to keep memory usage predictable.
...
server = MCPServer(
    name="my-server",
    middleware=[
        # Cache up to 1000 results for 5 minutes (300 seconds)
        ToolResultCachingMiddleware(
            max_size=1000,
            ttl_seconds=300.0
        )
    ]
...
```
<Tip>
**Concurrency Safe**: This middleware uses `asyncio.Lock` internally, ensuring that concurrent requests for the same key don't trigger race conditions or duplicate work.
</Tip>
