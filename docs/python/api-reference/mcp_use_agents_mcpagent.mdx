...
><ParamField body="metadata" type="dict[str, Any]" default="None" > Optional metadata to attach to each run, such as user identifiers
  or request context. This is passed through to tools and logging
  layers where supported. </ParamField>
><ParamField body="message_id" type="str" default="None" >  Optional unique identifier for the initial message or run. Useful
  for correlating logs and tool calls across systems. </ParamField>
