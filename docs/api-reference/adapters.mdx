...
Using Morph MCP for safe file editing with environment variable configuration:
...
async def setup_morph_integration():
    load_dotenv()  # Optional: loads from .env file
    
    # Configure Morph MCP server with actual env var values
    # (JSON configs don't expand ${VAR} syntax automatically)
    config = {
        "mcpServers": {
            "filesystem-with-morph": {
                ...
                "env": {
                    ...
                    "ALL_TOOLS": "false"  # Set to "true" to expose all filesystem tools
                }
            }
        }
    }
    ...
    # With ALL_TOOLS="false", Morph only exposes edit_file (safe mode)
    # With ALL_TOOLS="true", Morph exposes additional filesystem tools
...
# Usage
client, tools = await setup_morph_integration()
```

This pattern demonstrates how to properly pass environment variables to MCP servers. For production environments, keep `ALL_TOOLS="false"` to limit capabilities to only the safe edit tool. See [Morph documentation](../servers/morph.mdx) for more details.
