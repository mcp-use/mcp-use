...
graph TB
    ...
    subgraph "Integration Layer"
        Integration[• stream_with_formatting<br/>• format_and_print_result<br/>• output_from_result]
    end
    
    subgraph "Formatter"
        Format[• format_output<br/>• format_stream<br/>• _format_* helpers]
    end
    
    subgraph "Printer Layer"
        PanelPrinter[PanelPrinter<br/>• print_msg_and_resp<br/>• print_error]
        StreamPrinter[StreamPrinter<br/>• run_with_panels<br/>• live updates]
    end
    
    subgraph "Types"
        Type[• EventType<br/>• AgentOutputEvent<br/>• AgentOutput]
    end
    
    subgraph "Rich Library"
        Rich[Rich Components<br/>• Table, Console, Live<br/>• Markdown, JSON, Status]
    end
    
    subgraph "Configuration"
        Config[• OutputConfig<br/>• Customization options]
    end
...
<CardGroup cols={2}>
  <Card title="Panel Types" icon="layers">
    • **Message Panel** - Shows your input query in cyan<br/>
    • **Response Panel** - Displays the agent's final response in blue<br/>
    • **Step Panels** - Optional reasoning steps in green (when enabled)<br/>
  </Card>
  <Card title="Content Formatting" icon="code">
    • **Markdown rendering** for rich text output<br/>
    • **JSON formatting** for structured data<br/>
    • **Automatic truncation** of long intermediate results
  </Card>
</CardGroup>
