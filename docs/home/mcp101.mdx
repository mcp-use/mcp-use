...
Beyond tools and resources, mcp-use supports **MCP Apps**  -  interactive UI widgets that run directly inside Claude, ChatGPT, and other MCP clients. A tool returns a widget instead of plain text, and the client renders it.

See [MCP Apps](/typescript/server/mcp-apps).
