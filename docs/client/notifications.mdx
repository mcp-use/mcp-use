...
-   `ctx.send_tool_list_changed()` sends a `types.ToolsListChangedNotification`. This informs the client that the list of available tools has changed.
-   `ctx.send_resource_list_changed()` sends a `types.ResourcesListChangedNotification`. This informs the client that the list of available resources has changed.
-   `ctx.send_prompt_list_changed()` sends a `types.PromptsListChangedNotification`. This informs the client that the list of available prompts has changed.
...
async def handle_messages(message):
    ...
    if isinstance(message, types.ServerNotification):
        ...
        if isinstance(notification, types.ProgressNotification):
            ...
            print("Server prompts have changed. You may want to refresh them.")
        elif isinstance(notification, types.ResourcesListChangedNotification):
            print("Server resources have changed. You may want to refresh them.")
        elif isinstance(notification, types.ToolsListChangedNotification):
            print("Server tools have changed. You may want to refresh them.")
