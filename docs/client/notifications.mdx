...
-   `ctx.send_tool_list_changed()` sends a `types.ToolsListChangedNotification`. The client automatically refreshes its tool list when this is received (see [Tools documentation](/client/tools)).
-   `ctx.send_resource_list_changed()` sends a `types.ResourcesListChangedNotification`. The client automatically refreshes its resource list when this is received (see [Resources documentation](/client/resources)).
-   `ctx.send_prompt_list_changed()` sends a `types.PromptsListChangedNotification`. The client automatically refreshes its prompt list when this is received (see [Prompts documentation](/client/prompts)).
...
async def handle_messages(message):
    ...
    if isinstance(message, types.ServerNotification):
        ...
        elif isinstance(notification, types.PromptsListChangedNotification):
            print("Server prompts have changed (auto-refreshed in background)")
        elif isinstance(notification, types.ResourcesListChangedNotification):
            print("Server resources have changed (auto-refreshed in background)")
        elif isinstance(notification, types.ToolsListChangedNotification):
            print("Server tools have changed (auto-refreshed in background)")
