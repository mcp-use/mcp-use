...
and no separate project. Your Next.js routes keep working, and a single
`mcp-use dev` (or `build` / `start`) runs the MCP server beside them.
...
`src/mcp/` is discovered automatically: when you don't pass `--mcp-dir` or
`--entry` and the project has no legacy top-level entry (`index.ts`,
`src/index.ts`, ...), the CLI looks for `src/mcp/` and then `mcp/`. Pass
`--mcp-dir <dir>` to use a different folder, or `--entry src/mcp/server.ts`
to pin the entry file.
...
| Flag                  | Description                                                                     |
| --------------------- | ------------------------------------------------------------------------------- |
| `--mcp-dir <dir>`     | Folder holding the MCP entry + `resources/`. Auto-detected (`src/mcp`, `mcp`) when omitted. |
...
- **Scoped MCP server build.** The host Next.js app still owns the app build,
  so the CLI does not transpile every `.ts`/`.tsx` file in the project. It
  bundles only the MCP entry dependency graph into `dist/mcp/index.mjs`.
...
- **No `public/` copy.** The host Next.js app owns and serves `public/`;
  the MCP build does not duplicate it into `dist/`.

If the MCP server graph imports Next.js server-runtime modules such as
`server-only`, `next/headers`, or `next/cache`, the build replaces those imports
with inert MCP shims and prints a warning naming the importing file. This keeps
production `mcp-use start --mcp-dir` symmetrical with the normal CLI flow:
`start` runs the built JavaScript artifact with Node, not the TypeScript source
through `tsx`.
...
{
  "scripts": {
    "dev:all": "concurrently \"next dev\" \"mcp-use dev --port 3001\""
  }
}
```
...
- A project-root `tsconfig.json` with your `@/*` paths. The drop-in picks these
  up via Vite's `resolve.tsconfigPaths` (for widgets) and the scoped server
  bundle's tsconfig-aware resolver.
