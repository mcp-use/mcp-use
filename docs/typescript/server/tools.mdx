...
The recommended way to expose MCP Apps widgets is to **return JSX** from the tool handler: import a React component and render it with `_`-prefixed props for model output, status text, CSP, and other metadata.

Add **`/** @jsxImportSource mcp-use/jsx */`** to the server file that contains JSX. See [MCP Apps](./mcp-apps) for the full protocol overview.
...
- **JSX runtime** — Builds the same structured result as the internal `widget()` helper and registers inline widget metadata for `tools/list` and `resources/read`.
- **Props** — Regular props (`city`, `temp`, …) become `structuredContent` for `useWidget().props`. The model does not see them unless you also put them in `_output`.
- **`_output`** — What the model sees (use `text()`, `object()`, etc.).
- **`_invoking` / `_invoked`** — Host status strings.
- **`ToolRef`** — `server.tool()` returns a `ToolRef`; pass it to `useCallTool(ref)` in widgets for typed calls.
...
async (params, ctx) => {
  ...
  return text("Fallback for clients without Apps support.");
};
```

For migrating from `resources/`, `widget: { name }`, or `widget()`, see **[Migrating from resources/ and widgetMetadata](./migration-from-resources)**.

Legacy **`exposeAsTool`**, manual **`openai/outputTemplate`** wiring, and **`server.uiResource()`**-first flows are documented only under [UI widgets](./ui-widgets) (deprecated reference).
...
- [MCP Apps](./mcp-apps) — Inline JSX widgets and `_` props
