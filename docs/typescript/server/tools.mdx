...
<Tip>
**Response helpers**: Throughout this guide, you'll see examples using helpers like `text()`, `object()`, and `mix()`. These utilities create MCP-compatible tool results with proper typing and metadata. See the [Response helpers API reference](/typescript/api-reference/server/response-helpers) for exact signatures and return shapes.
</Tip>
...
The `ctx.log()` function accepts a log level (`'debug'`, `'info'`, `'notice'`, `'warning'`, `'error'`, `'critical'`, `'alert'`, `'emergency'`), a message string, and an optional logger name. See the [Tool context API reference](/typescript/api-reference/server/tool-context#log) for exact parameters and behavior.
...
- [Response helpers API reference](/typescript/api-reference/server/response-helpers) - Helper signatures, parameters, MIME types, and return shapes
