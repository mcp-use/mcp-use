...
The model sees the `output` content. The widget reads `props` with `useWidget().props`. See [MCP Apps](/typescript/server/mcp-apps) for widget workflow guidance and [useWidget()](/typescript/api-reference/react/usewidget) for the React hook API.
...
If your server adds or removes tools at runtime, notify connected clients so they can refresh their tool list.
...
<CardGroup cols={2}>
  ...
  <Card title="MCP Apps" icon="blocks" href="/typescript/server/mcp-apps">
