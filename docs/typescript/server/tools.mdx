...
## Use other schema libraries

`inputSchema` accepts a Standard Schema implementation that can produce JSON
Schema. Pass Zod and ArkType schemas directly. Wrap JSON Schema-first libraries
such as TypeBox with `fromJsonSchema()`.

### ArkType

ArkType implements the Standard Schema validation and JSON Schema interfaces.
Use `.describe()` to add field hints:

```typescript
import { type } from "arktype";
import { MCPServer } from "mcp-use";

const server = new MCPServer({ name: "arktype-server", version: "1.0.0" });
...
TypeBox produces JSON Schema rather than a Standard Schema validator. Wrap the
schema with `fromJsonSchema<T>()`; the adapter validates input and the generic
preserves the inferred TypeBox type:

```typescript
import { fromJsonSchema, MCPServer } from "mcp-use";
import Type from "typebox";

const server = new MCPServer({ name: "typebox-server", version: "1.0.0" });
