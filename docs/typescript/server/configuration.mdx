...
### Icons and Website URL (SEP-973)

You can add icons and a website URL to your server configuration. These are displayed in the MCP Inspector and help users identify your server:

```typescript
const server = createMCPServer('my-server', {
  version: '1.0.0',
  description: 'My MCP server',
  // Server icon - displayed in Inspector's server capabilities modal
  icons: [
    {
      src: 'https://example.com/logo.png',  // URL or data URI
      mimeType: 'image/png',                // Optional MIME type
      sizes: '512x512'                       // Icon size(s)
    }
  ],
  // Website URL - displayed as clickable link in Inspector
  websiteUrl: 'https://example.com'
})
```

**Icon Formats:**
- **URL-based icons**: Use HTTPS URLs pointing to icon images
- **Data URI icons**: Base64-encoded images (e.g., `data:image/png;base64,...`)
- **Multiple sizes**: Specify multiple sizes like `"16x16 32x32 48x48"` for responsive displays

**Security Note**: When using URL-based icons, ensure URLs are from trusted domains. The Inspector validates icon sources and provides fallback icons if loading fails.
...
const server = createMCPServer('my-server', {
  ...
  // Icons and website URL (SEP-973) - displayed in MCP Inspector
  icons: [
    {
  ...
  websiteUrl: 'https://example.com'  // Optional website URL
})
```
