...
| `NODE_ENV` | `production` disables dev features (inspector, type generation) | (none) |
| `MCP_DEBUG_LEVEL` | Request log verbosity: `info`, `debug`, or `trace` | `info` |
| `CSP_URLS` | Comma-separated extra URLs added to widget CSP `resource_domains` | (none) |
...
Sessions are lost on server restart. Suitable for local development and single-instance deployments. Use Redis for multi-instance or restart-resilient production.
