...
Run the build with the public origin before deploying. The server combines
this origin with its configured `basePath` (default `/mcp`) for the full MCP
endpoint:
...
- `<assetsPrefix>/_mcp-use/*` to static assets
- every other request to `server.fetch(request)`

Set `MCP_ASSETS_URL` to the complete public prefix that owns the
`_mcp-use` subtree. A Worker whose MCP endpoint is mounted at `/mcp`, for
example, can use `MCP_ASSETS_URL=https://worker.example.com/assets` and publish
`.mcp-use/build/views/` at `/assets/_mcp-use/views/`. The asset prefix is
independent from the MCP `basePath`.
...
`https://cdn.example.com/mcp-assets/_mcp-use/views/<view-name>/`. Publish
...
actual resource with the beta screenshot command:
