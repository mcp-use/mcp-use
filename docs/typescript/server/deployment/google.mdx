...
- Build interactive widgets (MCP Apps + ChatGPT) with inline JSX
...
## 6. Adding an interactive widget (inline JSX)

mcp-use builds **MCP Apps** widgets that also work in **ChatGPT**. Tools return **JSX** from the server file using **`/** @jsxImportSource mcp-use/jsx */`** — no `resources/` folder or `widget: { name }` is required for this pattern.

### Add a widget component

1. Create `components/AnimalCard.tsx` (for example next to your server entry):

   ```bash
   cloudshell edit ~/mcp-on-cloudrun/components/AnimalCard.tsx
   ```

2. Use a plain React component with `useWidget()`:
...
3. In your **server entry** (rename to `index.tsx` if needed), add the pragma and return JSX from a tool:
...
server.tool(
  {
    ...
    description:
      "Retrieves the details of a specific animal by its name and displays it in an interactive widget.",
    schema: z.object({
      name: z.string().describe("The name of the animal"),
    }),
  },
  async ({ name }) => {
    ...
    return (
      <AnimalCard
        ...
        trail={animal.trail}
        _output={text(
          `Found ${animal.name}, a ${animal.age}-year-old ${animal.species} in ${animal.enclosure}.`
        )}
        _invoking="Loading animal details..."
        _invoked="Animal details loaded"
      />
    );
  }
);
...
The bundler registers the widget and emits metadata for MCP Apps and ChatGPT. See [MCP Apps](/typescript/server/mcp-apps) and [Migrating from resources/ and widgetMetadata](/typescript/server/migration-from-resources).

For more context, see [Building ChatGPT Apps](/guides/chatgpt-apps-flow) (overview) and [Creating an MCP Apps server](/typescript/server/creating-mcp-apps-server).
