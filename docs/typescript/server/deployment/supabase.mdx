...
```bash
npm install
MCP_URL=https://<ref>.supabase.co \
MCP_ASSETS_URL=https://<ref>.supabase.co/functions/v1/mcp/assets \
npm run deploy:edge -- --project-ref <ref>
```

Persist the same public URLs in the Edge Function runtime:
...
maps `/functions/v1/mcp/assets/_mcp-use/*` to that generated tree and forwards
every other request to `server.fetch`. Configure the server with
`basePath: "/functions/v1/mcp"` so the SDK derives the full endpoint from the
origin exactly once. `MCP_ASSETS_URL` is the independent, complete prefix for
the co-located `_mcp-use` asset subtree; its `/assets` folder is not part of
the MCP server `basePath`.
