...
**Alternative**: You can also configure CSP per widget with `widgetMetadata.metadata.csp` instead of using the global `CSP_URLS` environment variable. See [Content Security Policy](../content-security-policy) for examples.
</Note>
