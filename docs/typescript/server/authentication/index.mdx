...
- [Custom Provider](/typescript/server/authentication/providers/custom) — Any provider with a custom `verifyToken` function

### OAuth Proxy

Use `oauthProxy` for providers that don't support remote client registration: Google, GitHub, Okta, Azure AD, or any provider where you register an application in a dashboard and receive a fixed `clientId`/`clientSecret`.
...
oauthProxy({
  ...
  verifyToken: jwksVerifier({ jwksUrl: '...', issuer: '...' }),
})
```

The `jwksVerifier` helper handles JWT signature verification, issuer checking, and optional audience validation (when `audience` is passed) against a remote JWKS endpoint. For non-JWT providers (e.g. GitHub opaque tokens), pass a custom `verifyToken` function instead.
