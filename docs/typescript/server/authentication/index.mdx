...
The `jwksVerifier` helper handles JWT signature verification, issuer checking, and audience validation against a remote JWKS endpoint. For non-JWT providers (e.g. GitHub opaque tokens), pass a custom `verifyToken` function instead.
