...
Use the Convex provider when you host your own OAuth authorization server using the [Convex OAuth component](https://www.convex.dev/components). MCP clients register directly with your Convex deployment through Dynamic Client Registration, and your MCP server verifies Convex-issued tokens.

This guide covers the Convex setup path. Use the [auth providers API reference](/typescript/api-reference/server/auth-providers#oauthconvexprovider) for exact `oauthConvexProvider()` options, defaults, and errors.

## Configure Convex

In your Convex deployment, set up the OAuth Provider component:

1. Install the [Convex OAuth component](https://www.convex.dev/components) in your Convex project.
...
Use [User Context](/typescript/server/authentication/user-context) for broader access-control patterns inside tools.
...
<CardGroup cols={2}>
  ...
  <Card
    title="Convex provider API reference"
    icon="terminal"
    href="/typescript/api-reference/server/auth-providers#oauthconvexprovider"
  >
