...
<Tip>
If you need to call the WorkOS Management API from your tool handlers, store your API key in any env var you like (for example `WORKOS_API_KEY`) and read it inside the tool — it isn't part of the OAuth provider config.
</Tip>
...
const server = new MCPServer({
  oauth: oauthWorkOSProvider({
    ...
    // Override supported scopes (advertised in OAuth metadata)
...
WorkOS uses **Dynamic Client Registration (DCR)**. MCP clients discover WorkOS's OAuth endpoints via `.well-known` passthrough and register themselves automatically — enable DCR in the WorkOS Dashboard under Connect → Configuration.
...
<Note>
WorkOS user-info extraction is handled internally by the provider using standard OIDC claims (`sub`, `email`, `name`, `org_id`, `roles`, etc.). These are already available on `context.auth`.
</Note>
...
**Solution:** Provide organization in the client-side auth URL:
```typescript
