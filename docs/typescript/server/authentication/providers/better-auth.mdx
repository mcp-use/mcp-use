...
With your `auth.ts` in place, run the Better Auth CLI to create the required tables:
...
// If you're having trouble connecting from the Inspector or a browser client, make sure these are set.
...
Better Auth requires a login page (where users authenticate) and a consent page (where users approve requested scopes). Mount these on the Hono app:
...
oauthBetterAuthProvider({
  ...
  // Optional: override supported scopes (advertised in OAuth metadata)
...
The authenticated user's claims are available on `ctx.auth`:
...
- [GitHub Example](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/oauth/better-auth)
