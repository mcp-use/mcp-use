...
# Optional override. Points the provider at a self-hosted or local Supabase
...
oauthSupabaseProvider({
  ...
  // JWT signature verification is ON by default. New ES256 projects
  // auto-fetch the JWKS, so no secret is needed. Legacy HS256 projects only:
...
<Tip>
  New Supabase projects sign tokens with **ES256** and expose a JWKS endpoint, and the provider fetches it automatically, no secret needed. Legacy **HS256** projects can still pass a `jwtSecret` (or set `MCP_USE_OAUTH_SUPABASE_JWT_SECRET`), but new projects do not need it.
</Tip>
