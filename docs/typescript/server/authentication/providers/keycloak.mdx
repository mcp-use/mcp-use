...
```typescript
// server.ts
import { MCPServer, oauthKeycloakProvider, object, error, text } from "mcp-use/server";
...
- **Scope of realm roles.** `ctx.auth.user.roles` only contains realm roles. Use `ctx.auth.user.permissions` (formatted as `client:role`) for per-client roles.
