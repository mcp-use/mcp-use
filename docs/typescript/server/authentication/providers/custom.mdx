...
Clients authenticate directly with your upstream provider. The MCP server only:

1. Exposes `.well-known/*` metadata (passed through from the upstream issuer).
2. Verifies bearer tokens on `/mcp/*` requests.
...
If your upstream provider supports Dynamic Client Registration and advertises `registration_endpoint` in its metadata, clients will register themselves automatically.
