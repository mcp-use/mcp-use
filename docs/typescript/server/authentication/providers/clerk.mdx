...
server.tool(
  ...
  async ({ documentId }, ctx) => {
    if (!ctx.auth.permissions?.includes('org:documents:delete')) {
      return error('Forbidden: org:documents:delete permission required')
    }
...
- [Runnable mcp-use + Clerk example](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/oauth/clerk)
