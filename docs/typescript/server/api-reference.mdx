...
#### `useWidget<TToolInput, TState>`

Main hook for MCP Apps / ChatGPT Apps SDK widgets. **`TToolInput`** (first) types **`toolInput`** / **`partialToolInput`**; **`TState`** (second) types persisted **`state`**. **`props`**, **`output`**, and **`metadata`** are loosely typed — prefer typing structured data on your **component** when using inline JSX.
