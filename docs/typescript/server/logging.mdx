...
One compact line per request, including session, client, the MCP method, and the outcome:
...
Logger.setDebug(0); // info level
Logger.setDebug(1); // info level (same as 0)
Logger.setDebug(2); // debug level
```

### Log Levels

The logger supports eight log levels (from least to most verbose):
...
Same as minimal but prefixes each line with a level emoji for quick visual scanning:

```text
14:23:45 [mcp-use] ℹ️ INFO: Server mounted at /mcp
14:23:46 [mcp-use] ℹ️ INFO: Session initialized: abc123
```
