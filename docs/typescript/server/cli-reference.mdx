...
Generate TypeScript type definitions for tools and widgets.

```bash
mcp-use generate-types [options]
```

**What happens during type generation:**

1. Analyzes your server's tool and widget definitions
2. Generates `.mcp-use/tool-registry.d.ts` with tool input/output types
3. Generates `.mcp-use/<widget>/types.ts` for widget props (optional)
4. Provides full TypeScript autocomplete in widgets
...
mcp-use generate-types --path ./my-server

# Use different server file
mcp-use generate-types --server src/server.ts
```

**When It Runs:**

Types are generated automatically in most workflows:
- **`postinstall` script**: Runs after `npm install` / `pnpm install`
- **`mcp-use dev`**: Regenerates on file changes (HMR)
- **Manual**: Run `mcp-use generate-types` to regenerate

<Tip>
**Setup**: Ensure `.mcp-use/` is included in your `tsconfig.json`:
...
The mcp-apps template includes this by default.
</Tip>
