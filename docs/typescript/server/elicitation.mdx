...
server.tool({
  ...
  description: 'Create account with validation'
    ...
    try {
      ...
      return error(`Validation failed: ${err instanceof Error ? err.message : String(err)}`);
    }
});
```
...
interface ElicitResult {
  ...
  // In form mode, `data` is strongly typed as `z.infer<typeof yourSchema>`
  // and is only present for the 'accept' action.
  data?: unknown;
}
```
...
server.tool({
  ...
  description: 'Tool with error handling',
    ...
    try {
      ...
      return error(`Error: ${err instanceof Error ? err.message : String(err)}. Please ensure your client supports elicitation.`);
    }
  });
```
...
📁 [examples/server/features/elicitation/](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/features/elicitation)
...
- Explore [sampling](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/features/sampling) for LLM completions
