...
- **Server-Side Validation**: Data is validated against the Zod schema before being returned
...
<Note>
**Automatic Validation**: When using the simplified API with Zod schemas (`ctx.elicit(message, zodSchema)`), the server automatically validates the returned data against your schema. Invalid data is rejected with clear error messages before reaching your tool logic.
</Note>
...
📁 [examples/server/elicitation-test/](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/elicitation-test)
...
```bash
cd libraries/typescript/packages/mcp-use/examples/server/elicitation-test
pnpm install
pnpm dev
```
