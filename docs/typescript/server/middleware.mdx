...
MCP middleware patterns use the `mcp:` prefix. Mount `server.getHandler()` in your framework when you need HTTP middleware around the MCP endpoint.
...
Mount the MCP handler inside Hono when middleware should run at the HTTP layer.
