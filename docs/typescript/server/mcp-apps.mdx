...
// city, temperature, conditions auto-injected from propSchema
const WeatherDisplay = ({ city, temperature, conditions }) => {
  const { isPending } = useWidget();

  if (isPending) {
    return <McpUseProvider autoSize><div>Loading weather...</div></McpUseProvider>;
  }
...
// itemId auto-injected from widgetMetadata.props schema
const InteractiveWidget = ({ itemId }) => {
  const { isPending: widgetPending } = useWidget();
  ...
  return (
    <McpUseProvider autoSize>
      <div>
        <button onClick={() => callTool({ id: itemId })}>
