...
Widgets can receive partial tool arguments in real time (`partialToolInput` / `isStreaming`) when the host streams them, enabling live previews as the LLM generates the tool call. See [useWidget  -  Streaming tool arguments](/typescript/react/usewidget#streaming-tool-arguments) for details.
...
See [useCallTool()](/typescript/react/usecalltool) for complete documentation.
...
- [Widget Components](/typescript/react/usewidget) - Explore React components and hooks
