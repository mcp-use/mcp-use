...
Widgets can receive partial tool arguments in real time (`partialToolInput` / `isStreaming`) when the host streams them, enabling live previews as the LLM generates the tool call. See [useWidget  -  Streaming tool arguments](/typescript/api-reference/react/usewidget#widget-lifecycle) for details.
...
See [useCallTool()](/typescript/api-reference/react/usecalltool) for complete documentation.
...
const { props, state, setState } = useWidget<Props, MyState>();
...
- [Widget Components](/typescript/api-reference/react/usewidget) - Explore React components and hooks
