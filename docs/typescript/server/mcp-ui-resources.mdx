...
<Warning>
**Prefer MCP Apps widgets for interactive UI.** Use [MCP Apps](/typescript/server/mcp-apps) with **inline JSX** on the server. To migrate older `resources/` or `widgetMetadata` setups, see [Migrating from resources/ and widgetMetadata](/typescript/server/migration-from-resources). **This page** documents **MCP-UI** resources (a different system from MCP Apps) for simpler or static embeds.
</Warning>
