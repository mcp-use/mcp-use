...
server.tool({
  ...
  return text(`LLM Analysis: ${response.content.text}`);
});
```
...
server.tool({
  ...
  if (ctx.client.can('sampling') && ctx.client.can('elicitation')) {
    ...
    return text(`Advanced analysis: ${analysis.content.text}`);
  } else if (ctx.client.can('sampling')) {
    // Use only sampling
    const result = await ctx.sample(`Process: ${params.input}`);
    return text(`Result: ${result.content.text}`);
  } else {
...
server.tool({
  ...
  if (ctx.client.supportsApps()) {
    // fetchData is a placeholder for your own data-loading logic
...
Text to analyze: ${params.text}`;
  ...
  const sentiment = response.content.text.trim().toLowerCase();
...
`ctx.sample()` and its options are documented in the [Tool context reference](/typescript/api-reference/server/tool-context). See `SampleOptions` for every option and `CreateMessageResult` for the return shape.
...
📁 [examples/server/features/sampling/](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/features/sampling)
