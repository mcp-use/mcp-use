...
Demonstrates **inline JSX** tool returns (`/** @jsxImportSource mcp-use/jsx */`), `_` props (`_output`, `_csp`, `_prefersBorder`, …), **`streamable()`** props, **`ToolRef`** / **`useCallTool`**, **`defineTools()`**, and **`ctx.client.supportsApps()`** for conditional widget vs text responses.

**Example location:** `libraries/typescript/packages/mcp-use/examples/server/features/inline-jsx-widgets`

**Run from package root (mcp-use package):**

```bash
pnpm run example:server:inline-jsx-widgets
```

See [MCP Apps](/typescript/server/mcp-apps) and [Migrating from resources/ and widgetMetadata](/typescript/server/migration-from-resources).

## MCP Apps dual-protocol example

**Location:** `libraries/typescript/packages/mcp-use/examples/server/ui/mcp-apps`

Demonstrates **inline JSX** (`index.tsx`) with `components/WeatherDisplay.tsx` — no `resources/` widget directory. Includes a programmatic `uiResource` HTML greeting for comparison.

**Run from the mcp-use package:**

```bash
pnpm run example:server:mcp-apps
```
