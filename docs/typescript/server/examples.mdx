...
This page collects MCP server examples built with the `mcp-use/server` framework. Prefer runnable example directories when you want to test behavior locally, and treat long inline examples as pattern references unless they include a run command.
...
Demonstrates **streaming tool arguments** to widgets: when an LLM streams tool call arguments, the widget receives partial input in real time via `partialToolInput` and `isStreaming` from [useWidget](/typescript/api-reference/react/usewidget#widget-lifecycle), enabling live code or JSON previews as the model generates them.
...
server.tool({
  ...
  schema: z.object({
    ...
    try {
      ...
      return error(`Error fetching weather for ${city}: ${(err as Error).message}`);
    }
  }
);
...
server.tool({
  ...
  schema: z.object({
    ...
    try {
      ...
      return error(`Error fetching forecast: ${(err as Error).message}`);
    }
  }
);
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text: `You are a weather reporter. ${styleInstructions[style] || styleInstructions.professional}.\n\n` +
                  `Please provide a weather report for ${city}. Current conditions: ${JSON.stringify(weather)}. 3-day forecast: ${JSON.stringify(forecast)}. Include current conditions, forecast summary, and any weather advisories.`
          }
        }
      ]
    }
...
server.tool({
  ...
  schema: z.object({
    ...
    try {
      ...
      return error(`Query error: ${(err as Error).message}`)
    }
  }
)
...
server.tool({
  ...
  schema: z.object({
    ...
    try {
      ...
      return error(`Error fetching schema: ${(err as Error).message}`)
    }
  }
)
...
server.resource({
  ...
  annotations: {
    ...
    try {
      ...
      return error(`Error fetching statistics: ${(err as Error).message}`)
    }
  }
)
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text: `You are a SQL expert. Generate efficient, safe SQL queries based on requirements.\n\n` +
                  `Generate a SQL query for the following requirement: "${description}"\n\n` +
...
```typescript
import { MCPServer, text, error, binary } from 'mcp-use/server'
import { z } from 'zod'
...
server.tool({
  ...
  schema: z.object({
    ...
    try {
      ...
      return error(`Error listing directory: ${(err as Error).message}`)
    }
  }
)
