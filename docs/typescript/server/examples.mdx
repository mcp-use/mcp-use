...
Demonstrates **streaming tool arguments** to widgets: when an LLM streams tool call arguments, the widget receives partial input in real time via `partialToolInput` and `isStreaming` from [useWidget](/typescript/react/usewidget#streaming-tool-arguments), enabling live code or JSON previews as the model generates them.
