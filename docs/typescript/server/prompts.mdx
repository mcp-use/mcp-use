...
server.prompt({
  ...
  schema: z.object({...}),                 // Argument definitions (optional)
}, async (args) => {...}) // generator function
```

`schema` is optional. Prompts without arguments can omit it entirely.
      ...
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `You are a helpful assistant that can answer questions about ${topic}.`,
          },
        },
      ],
    };
...
<Tip>
  **Response helpers**: Prompts support helpers like `text()`, `object()`,
  and `mix()` for cleaner implementations. The server automatically converts
  them to the required `GetPromptResult` format. See the [Response helpers API reference](/typescript/api-reference/server/response-helpers)
      ...
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `You are an expert ${language} developer. Review this code: ${code}`,
          },
        },
      ],
    };
...
server.prompt(
  ...
  async ({ topic }) => ({
    messages: [
      { role: "user", content: { type: "text", text: `Expert in ${topic}` } },
    ],
  })
);
      ...
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `Analyzing project ${projectId}...`,
          },
        },
      ],
    };
