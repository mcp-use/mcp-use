---
title: "MCP Apps quickstart"
description: "Create, run, and verify a TypeScript MCP App with a widget-returning tool."
icon: "rocket"
---

Create a TypeScript MCP App with a tool that returns a React widget, run it locally, and verify the widget in the Inspector.
...
- A running MCP server at `http://localhost:3000/mcp`
- A widget-returning tool that uses the `widget()` helper
- A React widget that receives props from the tool result
- A verified local result in the Inspector at `http://localhost:3000/inspector`
...
server.tool(
  {
    ...
    // Hosts such as ChatGPT expect explicit behavior hints on every tool.
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
      openWorldHint: false, // set true when calling external HTTP/APIs
    },
    // Describes structuredContent (here: widget props); helps validators and type generation.
    outputSchema: z.object({
      query: z.string(),
      results: z.array(
        z.object({ fruit: z.string(), color: z.string() })
      ),
    }),
  ...
  async ({ query }) => {
    ...
    return widget({
      props: { query: query ?? "", results },  // Widget-only data (useWidget().props); becomes structuredContent for hosts
...
<Note>
**ChatGPT and MCP catalogs**: Many hosts require `annotations.readOnlyHint`, `annotations.openWorldHint`, and `annotations.destructiveHint` (each an explicit boolean) on every tool, and recommend an `outputSchema` when the tool returns structured content, including widget tools, where `widget()` exposes `props` as `structuredContent` unless your `output` helper supplies its own.
</Note>
...
<Tip>
See [Widget Lifecycle](/typescript/api-reference/react/usewidget#widget-lifecycle) for complete patterns, and [Streaming tool arguments](/typescript/api-reference/react/usewidget#widget-lifecycle) for live previews while the model streams.
</Tip>

## Step 4: Add Traditional MCP Tools

You can mix widget-returning tools with traditional tools that return text or structured data. See the [Response helpers API reference](/typescript/api-reference/server/response-helpers) for `text()`, `object()`, and other utilities:
...
server.tool(
  {
    ...
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
      openWorldHint: true, // example assumes an external product API
    },
    outputSchema: z.object({
      id: z.string(),
      name: z.string(),
...
See [useCallTool()](/typescript/api-reference/react/usecalltool) for more patterns.
...
const MyWidget: React.FC = () => {
  // Generic order is <TProps, TState, TOutput, ...>, so skip past TState to type the output.
  const { props, output } = useWidget<MyProps, {}, MyOutput>();

  // props = structuredContent from widget({ props }) in the tool result
  // output = structuredContent from your output helper, when you return one
...
`useWidget()` also exposes a lower-level `callTool(name, args)` that resolves to a single promise. Prefer the [`useCallTool`](/typescript/api-reference/react/usecalltool) hook from Step 5 when you want loading and error state managed for you; reach for `useWidget().callTool` only for one-off, fire-and-forget calls:
...
Widgets can maintain state across interactions. You can read `state` and `setState` from `useWidget()`, or use the dedicated `useWidgetState` hook for a tuple API when you only need state:

```tsx
import { useWidget, useWidgetState } from "mcp-use/react";
...
// State-only: useWidgetState returns a [state, setState] tuple
const ThemeToggle: React.FC = () => {
  const [theme, setTheme] = useWidgetState<string>("light");
...
- [Widget Components](/typescript/api-reference/react/usewidget) - Explore React hooks and components
...
server.tool(
  ...
  async ({ productId }) =>
    object({ id: productId, name: "Widget", price: 29 })
);
...
- ✅ Define tools with `annotations`, optional `outputSchema`, `widget: { name, invoking, invoked }`, and return `widget({ props, output })`
