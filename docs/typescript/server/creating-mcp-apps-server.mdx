...
<Tip>
See [Widget Lifecycle](/typescript/react/usewidget#widget-lifecycle) for complete patterns, and [Streaming tool arguments](/typescript/react/usewidget#streaming-tool-arguments) for live previews while the model streams.
</Tip>
...
See [useCallTool()](/typescript/react/usecalltool) for more patterns.
...
`useWidget()` also exposes a lower-level `callTool(name, args)` that resolves to a single promise. Prefer the [`useCallTool`](/typescript/react/usecalltool) hook from Step 5 when you want loading and error state managed for you; reach for `useWidget().callTool` only for one-off, fire-and-forget calls:
...
- [Widget Components](/typescript/react/usewidget) - Explore React hooks and components
