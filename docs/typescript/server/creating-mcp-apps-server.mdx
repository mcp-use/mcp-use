...
`useWidget()` also exposes a lower-level `callTool(name, args)` that resolves to a single promise. Prefer the [`useCallTool`](/typescript/server/widget-components/usecalltool) hook from Step 5 when you want loading and error state managed for you; reach for `useWidget().callTool` only for one-off, fire-and-forget calls:
