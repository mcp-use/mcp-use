...
<Tip>
Check out the [complete notifications example](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/mcp-use/examples/server/features/notifications) for a working implementation with progress tracking, custom broadcasts, and resource updates.
</Tip>
...
// You can also send to a specific client by session ID
...
When using `ctx.sample()` in tool callbacks, progress notifications are automatically sent every 5 seconds, but only when the client supplied a `progressToken` on the tool call:

```typescript
import { text, type ToolContext } from 'mcp-use/server';
...
server.tool({
  ...
  cb: async (params, ctx?: ToolContext) => {
    ...
    // (only when the client supplied a progressToken on the tool call)
...
- [Sessions API reference](/typescript/api-reference/server/sessions) - Configure session management
