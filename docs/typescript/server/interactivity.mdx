...
This guide assumes your widget already renders with `useWidget()`. See [Build widgets](/typescript/server/build-widgets) first if you still need the server and widget setup.
...
- Use [Model context](/typescript/server/model-context) to decide what the model should know about these interactions.
