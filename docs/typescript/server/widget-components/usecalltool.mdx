...
Ensure `.mcp-use/` is included in your `tsconfig.json`:
...
<Note>
All templates include this by default. For existing projects created before this was added, add `.mcp-use/**/*` to your include paths.
</Note>

### Automatic Type Generation

Types are generated automatically:
- **On install**: The `postinstall` script runs `mcp-use generate-types`
- **During dev**: Types regenerate when tools change (HMR)
- **Manual**: Run `mcp-use generate-types` anytime
...
## How Automatic Typing Works

When you run `mcp-use dev` or `npm install`, types are automatically generated from your tool definitions:

**1. Define your tool with Zod schemas:**
...
**2. Types generate automatically:**

```typescript
// .mcp-use/tool-registry.d.ts (auto-generated - don't edit!)
...
**3. Use in widgets with full autocomplete:**

```tsx
const { callTool, data } = useCallTool("search-flights");
//                                      ^ Autocomplete shows all tool names!

// Input parameters are fully typed
...
if (data) {
  data.structuredContent.flights.forEach(flight => {
    //                   ^ Full autocomplete for output structure
