...
Starting from v1.19+, widgets automatically receive props without needing `useWidget` calls:

```tsx
// resources/weather-display.tsx
import { McpUseProvider, useWidget } from "mcp-use/react";

// Props are auto-injected and auto-typed from widgetMetadata.props!
...
// productId, name, price are auto-injected from widgetMetadata.props
const ProductWidget = ({ productId, name, price }) => {
  const {
    // Tool output (what the model sees) and state
    output,
    state,
    setState,
    ...
    // Availability
    isAvailable,
  // Type params: skip TProps (props come via component signature), type output and state
  } = useWidget<never, { reviews: Array<{ rating: number; comment: string }> }, {}, ProductState>();
  ...
  const handleGetReviews = async () => {
    const result = await callTool("get-product-reviews", { productId });
    // Handle result
  };
...
**Example (zero-config pattern — props in component signature):**

```tsx
// city and temperature come directly from widgetMetadata.props schema
const MyWidget = ({ city, temperature }) => {
  const { isPending } = useWidget();

  if (isPending) {
    return <McpUseProvider autoSize><LoadingSpinner /></McpUseProvider>;
  }

  // Safe to use props now
  return (
    <McpUseProvider autoSize>
...
```tsx
// code and language come from widgetMetadata.props schema
const CodePreviewWidget = ({ code, language }) => {
  ...
  const displayCode = isStreaming && partialToolInput?.code != null
    ...
    : code ?? "";
  const displayLang = (isStreaming ? partialToolInput?.language : language) ?? "text";
...
### 1. Props via Component Signature

Props are auto-injected into your component signature from the `widgetMetadata.props` schema:

```tsx
// ✅ New zero-config pattern — props auto-typed from schema
const MyWidget = ({ city, temperature }) => {
  const { isPending, callTool } = useWidget(); // Framework features only
  // ...
};
...
The first pattern is recommended: props come directly in the function signature, typed automatically by the dev server from your Zod schema.
