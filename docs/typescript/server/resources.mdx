...
<Tip>
**Response helpers**: This guide uses helpers like `text()`, `object()`, and `markdown()` for cleaner resource implementations. See the [Response helpers API reference](/typescript/api-reference/server/response-helpers) for exact signatures and return shapes.
</Tip>
...
To define resource content, use the response helpers.
...
Static resources and resource templates support multiple callback signatures. Use the simplest one that meets your needs:

```typescript
import { text, object, error } from 'mcp-use/server';

// No parameters - a static resource with fixed content
server.resource({
    name: 'welcome',
    uri: 'app://welcome'
...
// Just URI - when you need the matched URI
server.resourceTemplate({
    name: 'echo',
    uriTemplate: 'echo://{path}'
...
server.resourceTemplate({
  ...
  async (uri, { id }, ctx) => {
    ...
    const user = ctx.auth.user; // UserInfo
