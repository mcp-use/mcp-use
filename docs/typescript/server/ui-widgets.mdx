...
Widgets can call other MCP tools using the `useCallTool` hook (recommended) or `useWidget().callTool`:
...
// Define your tools for type safety
type MyWidgetTools = {
  'search_cities': {
    input: { query: string };
    output: { cities: Array<{ name: string; country: string }> };
  };
};

function MyWidget() {
  // In widget context, just pass the tool name
  ...
  return (
    <button 
      onClick={() => callTool({ query: 'tokyo' })}
      disabled={isPending}
    >
      {isPending ? 'Searching...' : 'Search Cities'}
    </button>
  );
}
```
...
<Tip>
  The `useCallTool` hook provides loading states (`isPending`, `isSuccess`, `isError`), 
  type safety, and lifecycle callbacks. See the [useCallTool documentation](/typescript/client/use-call-tool) 
  for full details.
</Tip>

**Using useWidget().callTool:**
