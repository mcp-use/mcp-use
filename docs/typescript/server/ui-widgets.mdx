...
- `MCP_URL`: Base URL for widget assets and public files. Used by Vite's `base` option during build.
...
<Note>
**Static Deployments**: Set both `MCP_URL` (for assets) and `MCP_SERVER_URL` (for API calls) when deploying to platforms like Supabase where widgets are served from static storage.
</Note>
