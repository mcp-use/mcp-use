...
CSP_URLS=https://myserver.com/storage/*
...
- `CSP_URLS`: (Optional) Additional domains to whitelist in widget Content Security Policy. Supports comma-separated list and wildcards (e.g., `https://*.supabase.co/storage/*`). Required for static deployments where widget assets are served from different domains.
