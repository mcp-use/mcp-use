...
<Warning>
**Deprecated for new projects.** The recommended approach is [MCP Apps](/typescript/server/mcp-apps) with **inline JSX** tool returns (`/** @jsxImportSource mcp-use/jsx */`). To move off `resources/`, `widgetMetadata`, `widget: { name }`, or manual `openai/` metadata, follow [Migrating from resources/ and widgetMetadata](/typescript/server/migration-from-resources). **This page** stays available for **legacy `server.uiResource()`**, **`type: "appsSdk"`**, and ChatGPT-specific edge cases.
</Warning>

<Info>
This page covers **ChatGPT Apps SDK** details and legacy registration APIs. For the standard **MCP Apps** flow (recommended), see [MCP Apps](/typescript/server/mcp-apps). mcp-use can still expose **`type: "mcpApps"`** `uiResource` definitions when you need explicit HTML templates; new projects should prefer **inline JSX** first.
...
These status texts are displayed as animated shimmer text (pending) and static text (complete) in the MCP Inspector. You can override them via `metadata.invoking`/`metadata.invoked` in your widget registration, or use `appsSdkMetadata` for raw key access:
