...
// props auto-injected from widgetMetadata.props schema
const ProductSearchResult = ({ items }) => {
...
```tsx
// Props come in component signature, isPending from useWidget hook
const MyWidget = ({ city, temperature }) => {
  const { isPending } = useWidget();

  if (isPending) return <McpUseProvider autoSize><LoadingSpinner /></McpUseProvider>;

  // Now safe to use props — city, temperature are defined
  return <McpUseProvider autoSize><div>{city} - {temperature}°C</div></McpUseProvider>;
};
```
...
<Note>
The `inputs` field expects a Zod schema that defines the shape of the parameters passed to your widget. This schema is used to generate the tool's input validation and to auto-type the props injected into your widget component.
</Note>
...
Widgets are called by the model in the same way as tools. Props from your `widgetMetadata.props` schema are automatically injected into your component signature — no hook needed for props access.
...
// query is auto-injected and auto-typed from the schema above
const ProductSearchResult = ({ query }) => {
  const { isPending } = useWidget();
  if (isPending) return <McpUseProvider autoSize><div>Loading...</div></McpUseProvider>;
  return <McpUseProvider autoSize><div>Query: {query}</div></McpUseProvider>;
};
```
...
```tsx
// city and temperature come from widgetMetadata.props schema
const MyWidget = ({ city, temperature }) => {
  const { isPending } = useWidget();
...
**Pattern 3: Streaming preview** (live update as LLM generates args)

```tsx
const MyWidget = ({ city, temperature }) => {
  const { isPending, isStreaming, partialToolInput } = useWidget();

  // Show partial input during streaming for live preview
  const displayCity = isStreaming ? partialToolInput?.city ?? "..." : city;

  return (
    <McpUseProvider autoSize>
      <div>
        <h1>{displayCity}</h1>
        {!isPending && <p>{temperature}°C</p>}
      </div>
    </McpUseProvider>
  );
};
...
// widgetProps (origin) come from widgetMetadata.props schema
const InteractiveWidget = ({ origin }) => {
