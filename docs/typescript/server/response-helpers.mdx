...
server.tool(
  ...
  async ({ url }) => {
    try {
      ...
      return error(`Failed to fetch data: ${(err as Error).message}`);
    }
  }
);
```
...
```typescript
import { object, type TypedCallToolResult } from "mcp-use/server";
...
- [UI Widgets](./mcp-apps) - Creating MCP App widgets
