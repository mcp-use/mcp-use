...
### Inline JSX (recommended)

Return a React element from your tool handler and use **`_output`** for the model-visible summary. The **`/** @jsxImportSource mcp-use/jsx */`** pragma selects the JSX runtime that turns the element into a proper `CallToolResult` and registers widget metadata.

See [MCP Apps](./mcp-apps), [Tools](./tools#returning-widgets-from-tools), and [Migrating from resources/ and widgetMetadata](./migration-from-resources).

### streamable()

Use **`streamable(initial)`** when a prop should update incrementally after the tool returns (for example streaming analysis text). Pass the `Streamable` as a normal prop; the server forwards updates to the widget. On the client, use **`useStreamableProps()`** inside the widget to subscribe.
...
server.tool(
  { name: "analyze", schema: z.object({ text: z.string() }) },
  async ({ text: inputText }) => {
    ...
    (async () => {
      for (const word of inputText.split(" ")) {
        ...
        analysis.update((prev) => (prev ? prev + " " : "") + word);
      }
      analysis.done();
    })();
...
| --- | --- |
| `streamable(initial)` | Create a streamable holder with a starting value |
| `.update(value)` / `.update(prev => next)` | Push a new value to subscribers |
| `.done()` | Finish streaming; resolves `.value` |
| `.current` | Synchronous snapshot (used for initial structured content) |
| `.value` | Promise of the final value after `.done()` |

### widget() (internal)

The **`widget({ props, output, metadata })`** helper still exists and is what the **JSX runtime** calls internally to build `content`, `structuredContent`, and `_meta`. Application code should prefer **returning JSX** from tools instead of calling `widget()` directly, unless you are doing something highly custom.

Per the [MCP Apps specification](https://github.com/modelcontextprotocol/ext-apps), the tool result fields have different visibility:

| Field | LLM sees it? | Widget sees it? |
| --- | --- | --- |
| `content` | **Yes** | Yes |
| `structuredContent` | **No** | Yes (as `props`) |
| `_meta` | **No** | Yes (as `metadata`) |

With JSX, non-`_` props map to `structuredContent`; **`_output`** supplies model-visible content; **`_meta`** maps to extra widget metadata.
...
- [Tools](./tools) — Creating MCP tools and inline JSX widgets
- [MCP Apps](./mcp-apps)
