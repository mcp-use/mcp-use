...
Use a stateful client that can send `resources/subscribe`, such as the TypeScript client CLI.

```bash
npm run dev
```

In another terminal, subscribe to the resource URI. Replace `my-server` with the server name from your client config.

```bash
npx mcp-use client my-server resources subscribe "settings://app"
```

Keep that command running. In a third terminal, call the update tool:

```bash
npx mcp-use client my-server tools call update_settings theme=dark
```
