...
6. ✅ Sets `MCP_URL` and `CSP_URLS` environment variables (before deployment)
...
<Note>
**Two URLs Required**: Supabase deployments need both URLs because widget files are served from static storage while API calls go to the edge function. `MCP_SERVER_URL` enables build-time URL injection for static deployments.
</Note>
...
# CSP_URLS: Additional domains for widget assets (storage bucket)
supabase secrets set CSP_URLS="https://YOUR_PROJECT_REF.supabase.co/storage/*" --project-ref YOUR_PROJECT_REF
```

<Note>
**Environment Variables:**
- `MCP_URL`: Whitelists the edge function domain for API calls from widgets
- `CSP_URLS`: Whitelists additional domains for widget assets (JS/CSS). Required for Supabase since assets are served from storage bucket. Supports wildcards and comma-separated values.
