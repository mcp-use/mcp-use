...
### 4. Configure Imports (Optional)

You can create an `import_map.json` file in your function directory:

```json
// supabase/functions/mcp-server/import_map.json
{
  "imports": {
...
However, using direct ESM imports (as shown in the example above) is recommended for Supabase.
...
```typescript
import { createMCPUseClient } from "mcp-use";

const client = createMCPUseClient({
...
// Now use the client to call tools, access resources, etc.
const response = await client.callTool("greet", { name: "World" });
```
...
If you're using MCP UI Widgets, they will work in production mode. The widgets are bundled and served from `dist/resources/widgets/`. Set `NODE_ENV=production` (which is automatic in Supabase) to use only built widgets without dev dependencies.
