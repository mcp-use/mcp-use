...
When deploying with `supabase functions deploy`, use one of these approaches:
...
**Option 2: API-based deployment without Docker**
If Docker is not available, use the `--use-api` flag (widgets will NOT be bundled):
```bash
supabase functions deploy mcp-server --use-api
```

**Important**: Without the `--use-api` flag and without Docker running, the deployment will fail if Docker is not available.
