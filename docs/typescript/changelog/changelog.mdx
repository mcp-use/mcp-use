...
<Update label="v1.29.1" description="June 2026">
  ## OAuth `/sse` auth fix & security dependency bumps
  Patch release closing an OAuth bearer-auth gap on the legacy SSE transport and resolving open Dependabot security advisories across the TypeScript workspace.

- **Fix**: Require bearer auth on `/sse` (and `/sse/*`) for OAuth-protected MCP servers  -  JSON-RPC was also mounted on `/sse` but middleware only covered `/mcp/*`, so unauthenticated clients could reach the handler; servers now advertise RFC 9728 path-scoped protected-resource metadata for `/sse` (`#1628`)
