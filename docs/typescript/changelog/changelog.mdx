...
<Update label="v1.33.0" description="June 2026">
  ...
  ## MCP Apps primary widget runtime & monorepo deploy triggers
  Minor release making MCP Apps the primary widget runtime (with an Apps SDK compatibility fallback), enforcing `outputSchema` at the tool return position, and adding monorepo auto-deploy controls to the CLI. Inspector ships major v11.0.0 (see inspector changelog).
