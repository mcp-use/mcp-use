...
Use the Inspector to confirm that the server and widget work. The Inspector renders MCP Apps widgets; it does not emulate ChatGPT.
...
If the tool runs but no widget appears, check that `widget.name` exactly matches the folder under `resources/` and that the handler returns `widget(...)`. For dual-protocol and ChatGPT-only cases, see [Debug widgets in the Inspector](/inspector/debugging-chatgpt-apps).
