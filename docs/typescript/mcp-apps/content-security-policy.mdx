...
Set **`MCP_URL`** to your server's public origin. The SDK combines it with
`basePath` for the OAuth resource URL and default `ui.domain`; its origin is
also added to CSP `connectDomains`:
...
This is the complete asset prefix. The SDK appends only `/_mcp-use/...`;
it does not combine `MCP_ASSETS_URL` with the server's `basePath`.

For example, `MCP_URL=https://myserver.com` with `basePath: "/api/mcp"`
produces the full endpoint and default `ui.domain`
`https://myserver.com/api/mcp`. An explicit `view.domain` overrides that
default.

When `MCP_ASSETS_URL` is unset, assets use the same origin and `basePath` as
the MCP server (`MCP_URL`, or the request origin behind a proxy).
...
```env
MCP_URL=https://PROJECT.supabase.co
