...
Starts the HTTP server. Mounts MCP protocol endpoints (at `/mcp` and `/sse`), the inspector UI (at `/mcp/inspector`), widget routes, and OAuth routes (if configured), then begins listening. Resolves once the server is listening. During HMR reload (when the CLI manages the lifecycle) this is a no-op.
