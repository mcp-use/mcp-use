...
- Single instance, sessions can reset on restart: `InMemorySessionStore` + `InMemoryStreamManager` (the defaults).
- Single instance with hot reload, sessions should survive a restart: `FileSystemSessionStore`.
