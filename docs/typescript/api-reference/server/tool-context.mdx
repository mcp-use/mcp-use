...
`RequestContext` is the second argument (`ctx`) passed to tool, resource, and prompt callbacks. It exposes request cancellation, the current HTTP request, OAuth identity when configured, stateless elicitation, progress and logging helpers, and the client metadata declared in the current request.
