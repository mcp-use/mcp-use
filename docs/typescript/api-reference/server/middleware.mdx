...
Middleware runs in FIFO order (first registered = outermost). `ctx.params` is mutable before `next()`, and `ctx.request` is the originating HTTP `Request` when present. Throwing rejects the request (surfaced as an MCP error result).
