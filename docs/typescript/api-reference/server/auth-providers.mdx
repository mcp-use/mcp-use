...
- **Proxy mode** (`oauthProxy`) for providers without Dynamic Client Registration support (Google, GitHub, pre-registered Auth0 web apps, enterprise IdPs). The server mediates the flow: it exposes a `/register` endpoint returning the configured `clientId`, injects `clientId`/`clientSecret` at token exchange, and passes upstream tokens through without minting its own.
