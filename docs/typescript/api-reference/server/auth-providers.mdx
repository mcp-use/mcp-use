...
- **DCR-direct providers** (`oauthAuth0Provider`, `oauthBetterAuthProvider`, `oauthClerkProvider`, `oauthKeycloakProvider`, `oauthScalekitProvider`, `oauthSupabaseProvider`, `oauthWorkOSProvider`, `oauthCustomProvider`). The MCP server proxies metadata discovery to the upstream authorization server and verifies tokens. Clients communicate directly with the upstream for authorize, token, and register.
...
Create a Scalekit OAuth provider for an MCP server.

```ts
import { oauthScalekitProvider } from "mcp-use/oauth/scalekit";
```

Builds an `OAuthProvider` backed by Scalekit MCP Auth. Advertised metadata uses the resource-scoped issuer. The verifier accepts both the environment-root issuer and the resource-scoped issuer, and binds audience to the `res_…` resource id. `environmentUrl` falls back to `MCP_USE_OAUTH_SCALEKIT_ENVIRONMENT_URL` and `resourceId` falls back to `MCP_USE_OAUTH_SCALEKIT_RESOURCE_ID`. Throws `Error` if `environmentUrl` or `resourceId` is missing, and `TypeError` if `resourceId` is empty or does not start with `res_`, if `audience` is empty, or if `issuerBoundAccessTokens` is `true`. An optional `audience` is required *together with* the resource id. It does not replace the `res_…` check.

**Parameters**

> <ParamField body="options" type="ScalekitOAuthProviderOptions" default="{}">
>   {" "}
>   Scalekit environment URL, resource id, optional audience override, and MCP
>   resource metadata. Values passed here override the corresponding
>   environment variables.{" "}
> </ParamField>

**Returns**

> <ResponseField name="returns" type="OAuthProvider<ScalekitOAuthUser>" />

**Signature**

```ts wrap
function oauthScalekitProvider(
...
const server = new MCPServer({
  name: "scalekit-oauth-example",
  version: "1.0.0",
  description: "MCP server with Scalekit OAuth authentication",
