...
| Custom class | `BaseCodeExecutor` subclass | Implement `execute()` + `cleanup()` |
| Custom function | `CodeExecutorFunction` | `(code, timeout?) => Promise<ExecutionResult>`, called with the client as `this`; owns its own runtime cleanup |
