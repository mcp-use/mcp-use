...
Transport configuration for one MCP server. Node.js clients support stdio and HTTP configs. Browser clients support HTTP configs only.
...
><ResponseField name="env" type="Record<string, string>">Optional environment variables for stdio servers.</ResponseField>
...
- [Client configuration](/typescript/client/client-configuration): guide to stdio and HTTP server configuration.
