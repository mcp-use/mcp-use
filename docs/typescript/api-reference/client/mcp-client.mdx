...
icon: "router"
github: "https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client.ts"
---

<Callout type="info" title="Source Code">
  View the source code for the Node.js client on GitHub:{" "}
  <a
    href="https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client.ts"
    target="_blank"
    rel="noopener noreferrer"
  >
    https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client.ts
  </a>
  <br />
  View the shared base client implementation:{" "}
  <a
    href="https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client/base.ts"
    target="_blank"
    rel="noopener noreferrer"
  >
    https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client/base.ts
  </a>
  <br />
  View the browser client implementation:{" "}
  <a
    href="https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client/browser.ts"
    target="_blank"
    rel="noopener noreferrer"
  >
    https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/src/client/browser.ts
  </a>
</Callout>

`MCPClient` stores server configuration and creates
[`MCPConnection`](/typescript/api-reference/client/mcp-session) instances. The
connection API hides legacy v1 sessions while preserving modern v2 stateless
requests.
...
Use the same `@mcp-use/client` import in browser bundles. Package conditions
select the HTTP-only browser implementation automatically.
...
const connection = await client.connect("filesystem");
...
> <ParamField
>   body="config"
>   type="string | MCPClientConfigShape | undefined"
>   default="undefined"
> >
>   Inline client configuration, path to a JSON config file, or omitted for an
>   empty client.
> </ParamField>
> <ParamField
>   body="options"
>   type="MCPClientOptions | undefined"
>   default="undefined"
> >
>   Client behavior options such as code mode, sampling, elicitation, and
...
> <ParamField body="cfg" type="Record<string, any>" required="true">
...
Connects to a configured server, negotiates legacy v1 or modern v2, and returns
a ready [`MCPConnection`](/typescript/api-reference/client/mcp-session).
...
Legacy alias for creating a connection. Prefer `connect()`; modern v2 servers are
sessionless.

**Parameters**

> <ParamField body="serverName" type="string" required="true">
>   Name of the server in `config.mcpServers`.
> </ParamField>
> <ParamField body="autoInitialize" type="boolean" default="true">
>   When `true`, connects and runs the MCP initialize handshake before
>   resolving.
> </ParamField>
...
>   Unique server name.
> </ParamField>
> <ParamField body="serverConfig" type="ServerConfig" required="true">
...
> <ParamField body="filepath" type="string" required="true">
...
>   Code to execute.
> </ParamField>
> <ParamField body="timeout" type="number | undefined" default="undefined">
...
> <ResponseField name="mcpServers" type="Record<string, ServerConfig>">
...
> </ResponseField>
> <ResponseField name="onSampling" type="OnSamplingCallback">
>   Optional default callback for server sampling requests.
> </ResponseField>
> <ResponseField name="onElicitation" type="OnElicitationCallback">
>   Optional default callback for server elicitation requests.
> </ResponseField>
> <ResponseField name="onNotification" type="OnNotificationCallback">
...
> </ResponseField>
> <ResponseField name="authProvider" type="unknown">
...
> <ResponseField name="codeMode" type="boolean | CodeModeConfig">
...
> </ResponseField>
> <ResponseField name="onNotification" type="OnNotificationCallback">
>   Handle server notifications when a server config does not provide its own
>   handler.
> </ResponseField>
> <ResponseField name="samplingCallback" type="OnSamplingCallback">
>   Deprecated alias for `onSampling`.
> </ResponseField>
> <ResponseField name="elicitationCallback" type="OnElicitationCallback">
...
> <ResponseField
>   name="executor"
>   type='"vm" | "e2b" | CodeExecutorFunction | BaseCodeExecutor'
> >
>   Executor implementation. Defaults to `"vm"`.
> </ResponseField>
> <ResponseField
>   name="executorOptions"
>   type="VMExecutorOptions | E2BExecutorOptions"
> >
...
```ts wrap
import { acceptWithDefaults, MCPClient } from "@mcp-use/client";
