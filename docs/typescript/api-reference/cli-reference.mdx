...
<CardGroup cols={2}>
  ...
  <Card title="Skills and types" icon="wand" href="#skills-add">
    `mcp-use skills add` installs agent skills. `mcp-use generate-types` writes
    widget type augmentation.
  </Card>
</CardGroup>
...
<Tip>`create-mcp-use-app` installs the CLI as a project dependency.</Tip>
...
| `--host <host>`     | Bind address. Use `0.0.0.0` to listen on all interfaces.                 | `127.0.0.1`                    |
...
- When `@mcp-use/inspector` is installed in the project, it is mounted at `/mcp/inspector` and opens automatically unless `--no-open` is set. Generated projects include it as a dev dependency. If it is absent, the MCP server still starts and prints an install command.
...
- Each `views/<name>/view.tsx` is built under `.mcp-use/build/views/<name>/`; its mode-neutral view map is stored in the existing `.mcp-use/build/manifest.json`.
- The build is transpile-only. Run `tsc --noEmit` as a separate project script when type checking is required.
- TypeScript path aliases come from the selected project's `tsconfig.json`.
- If `MCP_ASSETS_URL` is set, view manifest paths are rewritten to that asset origin using the server's `basePath`.
...
- Production builds do not include Inspector UI, proxy, or OAuth routes. Use `mcp-use dev` for visual inspection.
