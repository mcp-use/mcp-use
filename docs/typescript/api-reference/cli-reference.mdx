...
- If `MCP_ASSETS_URL` is set for an external build, view paths are rewritten beneath that complete asset prefix. The server's `basePath` is not appended. Inline view bundles are not rewritten.
...
| `MCP_URL`                   | `dev`, `start`, server runtime            | **Server public origin** (`.origin` only). Combined with `basePath` for the OAuth resource URL and default `ui.domain`; its origin feeds CSP `connectDomains` and dev HMR websocket. Lower priority than `CSP_URLS`. Preserved when already set. | Local server or tunnel origin       |
| `MCP_ASSETS_URL`            | `build`, server runtime                   | **Complete assets URL prefix** (origin + optional path) for view JS/CSS and `public/`. The SDK appends only `/_mcp-use/...`, never the server `basePath`. When unset, assets fall back to the MCP server origin plus `basePath`. | None (falls back to server route)  |
