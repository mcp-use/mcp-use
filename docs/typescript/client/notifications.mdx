...
<Info>
  **Bidirectional**: Both clients and servers can send notifications. Servers
  notify clients of changes, while clients notify servers of environment changes
...
<Tip>
  For information on sending notifications from your MCP server, see the [Server
  Notifications](/typescript/server/notifications) guide.
</Tip>
...
```typescript
import { MCPClient, Root } from "@mcp-use/client";
...
```typescript
import { HttpConnector, MCPSession } from "@mcp-use/client";
...
```typescript
import { MCPClient, Notification, NotificationHandler } from "@mcp-use/client";
...
3. **Streaming Transport**: Streamable HTTP transport

<Note>
  Notifications don't work in stateless edge environments where sessions aren't
