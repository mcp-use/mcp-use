...
Configure how `search_tools` finds relevant tools:
...
const client = new MCPClient(
  ...
  },
  {
    codeMode: {
      enabled: true,
      semantic: {
        mode: "embeddings", // "string_match" (default), "fuzzy", or "embeddings"
        embeddingsUrl: "https://api.example.com/v1/embeddings", // Optional: for embeddings mode
      },
    },
  }
);
...
- **`string_match`** (default): Simple substring matching - fast, no dependencies
- **`fuzzy`**: Fuzzy string matching using `fuse.js` - better for typos and partial matches
  - Requires: `yarn add fuse.js`
- **`embeddings`**: Semantic search using embeddings - best for semantic similarity
  - Requires: `OPENAI_API_KEY` environment variable OR `embeddingsUrl` pointing to OpenAI-compatible API
...
The `search_tools` function supports different search modes for finding relevant tools:

#### String Match (Default)

Simple substring matching - fast and requires no dependencies:
...
**Example:**
```javascript
// Finds tools containing "repo" or "list" in name/description
const result = await search_tools("repo list");
// Returns: list_repositories, search_repositories, list_repos, etc.
```

#### Fuzzy Search

Fuzzy string matching for better handling of typos and partial matches:
...
**Requirements:** `yarn add fuse.js`

**Example:**
```javascript
// Finds tools even with typos or partial matches
const result = await search_tools("repo list repositories");
// Better matches for: list_repositories, search_repositories
```

#### Embeddings Search

Semantic search using embeddings for finding tools by meaning:
...
const client = new MCPClient(config, {
  codeMode: {
    ...
    semantic: {
      mode: "embeddings",
      // Option 1: Use OpenAI API (requires OPENAI_API_KEY env var)
      // embeddingsUrl: undefined
      
      // Option 2: Use custom OpenAI-compatible API
...
- `embeddingsUrl` pointing to OpenAI-compatible embeddings API

**Example:**
```javascript
// Finds semantically similar tools even without exact keyword matches
const result = await search_tools("repo list repositories");
// Returns: list_repositories, search_repositories, list_repos
// Even if they don't contain all the exact words
```

**Note:** Embeddings are cached per tool to improve performance on subsequent searches.
...
interface CodeModeConfig {
  ...
  // Semantic search configuration for search_tools
  semantic?: SemanticSearchConfig;
}

interface SemanticSearchConfig {
  // Search mode: "string_match" (default), "fuzzy", or "embeddings"
  mode?: "string_match" | "fuzzy" | "embeddings";

  // URL for embeddings API (required for embeddings mode if OPENAI_API_KEY not set)
