...
<Tip>
  **Perfect for React Apps**: The hook handles all the complexity of MCP
  connections, letting you focus on building your UI. Just call `useMcp()` and
  you're ready to go.
</Tip>
...
function ConnectionStatus() {
  ...
  switch (mcp.state) {
    ...
    case "pending_auth":
      return <button onClick={mcp.authenticate}>Click to Authenticate</button>;
...
interface UseMcpOptions {
   ...
   * The `clientConfig` is automatically derived from `clientInfo` for OAuth registration.
   */
  clientConfig?: {
    name?: string; // Client name (for OAuth registration)
    version?: string; // Client version (for OAuth registration)
    uri?: string; // Client URI/homepage (for OAuth registration)
    logo_uri?: string; // Client logo URI (for OAuth registration)
  };
  clientInfo?: {
    name: string; // Client name (sent to MCP server and used for OAuth)
    title?: string; // Client display name
    version: string; // Client version (used for OAuth)
    description?: string; // Client description
    icons?: Array<{
      // Client icons (first icon used for OAuth logo_uri)
      src: string;
      mimeType?: string;
      sizes?: string[];
    }>;
    websiteUrl?: string; // Client website URL (used for OAuth uri)
  };
  ...
  ) => Promise<CreateMessageResult>; // Handle sampling requests
  onElicitation?: (
    params: ElicitRequestFormParams | ElicitRequestURLParams
  ) => Promise<ElicitResult>; // Handle elicitation requests
}
```

### Client Information

The `useMcp` hook uses `clientInfo` to identify your client to both MCP servers and OAuth providers. The `clientConfig` option is deprecated and will be automatically derived from `clientInfo`.

**How clientInfo maps to OAuth registration:**

- `clientInfo.name` → OAuth `client_name`
- `clientInfo.websiteUrl` → OAuth `client_uri`
- `clientInfo.icons?.[0]?.src` → OAuth `logo_uri` (first icon)
- `clientInfo.version` → OAuth client metadata

**Default Behavior:**

If `clientInfo` is not provided, the hook automatically uses mcp-use defaults:
...
// OAuth registration will automatically use:
// - client_name: "MyApp"
// - client_uri: "https://example.com"
// - logo_uri: "https://example.com/icon.png"
```

**Note:** The `clientInfo` you provide will be merged with the defaults, so you only need to specify the fields you want to override.

**Deprecated: clientConfig**

The `clientConfig` option is deprecated. If you still use it, you'll see a deprecation warning. The OAuth registration values will be automatically derived from `clientInfo` instead.
...
function MyComponent() {
  ...
  if (mcp.state === "pending_auth") {
    return <button onClick={mcp.authenticate}>Sign in to Linear</button>;
  }
...
<Note>
  **For browser apps**: Use dynamic import of `mcp-use/browser` (shown below) to
  avoid bundling server-side code. **For Node.js apps**: You can import directly
  from `mcp-use`.
</Note>
