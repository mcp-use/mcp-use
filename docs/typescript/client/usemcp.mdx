...
interface UseMcpOptions {
  ...
  maxRetries?: number                      // Max automatic reconnection attempts after 404 errors (default: unlimited)
...
The hook automatically handles 404 errors (invalid session IDs) that occur after server restarts by clearing the session and reconnecting. You can configure the maximum number of retry attempts.
...
function RobustConnection() {
  const mcp = useMcp({
    ...
    maxRetries: 3,    // Limit automatic reconnection attempts after 404 errors
  })
  ...
  useEffect(() => {
    if (mcp.state === 'failed') {
      ...
      } else if (mcp.error?.includes('Max retry attempts')) {
        // Max retries exceeded - user needs to manually reconnect
        alert('Session expired. Please reconnect manually.')
      }
    }
  }, [mcp.state, mcp.error])
...
When the server restarts, the client may have an old session ID. The hook automatically detects 404 errors (invalid session) and reconnects:

- **Default behavior**: Unlimited retries (keeps trying to reconnect)
- **With `maxRetries`**: Stops after the specified number of attempts

```typescript
// Unlimited retries (default)
const mcp = useMcp({
  url: 'http://localhost:3000/mcp'
  // Will keep retrying indefinitely on 404 errors
})

// Limited retries
const mcp = useMcp({
  url: 'http://localhost:3000/mcp',
  maxRetries: 3  // Stop after 3 failed reconnection attempts
})
```

When `maxRetries` is exceeded, the connection enters `'failed'` state with an error message indicating manual reconnection is needed. You can then call `mcp.retry()` to manually reconnect.
