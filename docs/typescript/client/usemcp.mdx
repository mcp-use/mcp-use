...
`@mcp-use/client/react` provides comprehensive React integration through `McpClientProvider` and the `useMcp` hook. The provider-based approach is recommended for managing multiple MCP server connections in your application.
...
- **v1/v2 negotiation**: One hook for legacy sessionful and modern sessionless servers
- **Normalized metadata**: Protocol version, capabilities, instructions, and extensions
...
```typescript
import { McpClientProvider, useMcpClient, useMcpServer } from "@mcp-use/client/react";
...
function ServerCard({ server }) {
  ...
  return (
    <div>
      <h3>{server.serverInfo?.name || server.displayName}</h3>
      <p>
        Protocol: {server.protocolEra} ({server.protocolVersion})
      </p>
...
Proxy fallback is opt-in and requires an explicit MCP proxy URL. OAuth uses a
separate BFF URL so transport routing never changes OAuth resource identity:

```typescript
<McpClientProvider
  defaultProxyConfig={{
    proxyAddress: "https://inspector.example.com/inspector/api/proxy",
  }}
  defaultOAuthProxyUrl="https://inspector.example.com/inspector/api/oauth"
...
The SDK still sees the upstream MCP URL. Only the injected fetch routes MCP
request bytes through `proxyAddress`; OAuth metadata, registration, and token
requests use `defaultOAuthProxyUrl`.
...
**CIMD or pre-registered OAuth client**

Prefer an HTTPS Client ID Metadata Document when the authorization server
advertises CIMD. Keep a pre-registered public client ID for providers with an
...
Browser clients are public PKCE clients. Confidential client secrets belong on a
backend and are rejected by the browser provider. See
[Authentication](/typescript/client/authentication#registration-strategy).
...
function ResourceViewer({ serverId, uri }: { serverId: string; uri: string }) {
  ...
  return (
    <div>
      <h3>
        {server.displayName} - Resource: {uri}
      </h3>
      <pre>{content}</pre>
    </div>
  );
...
function ServerManager() {
  ...
  return (
    <div>
      ...
      {servers.map((server) => (
        <div key={server.id}>
          <h4>{server.serverInfo?.name || server.displayName}</h4>
...
```typescript
import { McpClientProvider, LocalStorageProvider } from "@mcp-use/client/react";
...
Handle interactive server requests (AI sampling, form elicitation). For elicitation, you can use helpers from `@mcp-use/client` (e.g. `acceptWithDefaults`, `validate`) inside your callback; see [Elicitation](/typescript/client/elicitation).
      ...
      {servers.map((server) => (
        <div key={server.id}>
          {server.pendingSamplingRequests.map((request) => (
            <div key={request.id}>
              <h4>{server.displayName} needs AI assistance</h4>
...
```typescript
import type { ReconnectionOptions } from "@mcp-use/client/react";
...
<Note>
  `autoReconnect` controls the **application-level** health monitoring (HEAD
  request polling and reconnect triggers). `reconnectionOptions` controls the
  **transport-level** retry behavior within the MCP SDK when the SSE stream
  drops. Both can be used together for robust connection handling.
</Note>
