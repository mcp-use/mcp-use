...
To enable sampling, provide an `onSampling` function when initializing the MCPClient. You can import `OnSamplingCallback`, `CreateMessageRequestParams`, and `CreateMessageResult` from `@mcp-use/client` so you don't need to import from the MCP SDK.
...
For a full runnable v1/v2 example, see [sampling-client.ts](https://github.com/mcp-use/mcp-use/blob/main/libraries/typescript/packages/client/examples/node/communication/sampling-client.ts).
...
import type { CreateMessageRequestParams, CreateMessageResult } from '@mcp-use/client'
...
For runnable client examples, see the [client examples](https://github.com/mcp-use/mcp-use/tree/main/libraries/typescript/packages/client/examples), including focused sampling, elicitation, notifications, completion, capabilities, OAuth, and MCP Apps coverage.
