...
import { McpClientProvider, useMcpServer } from '@mcp-use/client/react'
...
server.prompt(
  ...
  async ({ extension }) => ({
    messages: [
      {
        role: "user",
        content: { type: "text", text: `Find ${extension} files` },
      },
    ],
  }),
);
...
The four-server example matrix runs this client against mcp-use v1 and v2,
demonstrating prompt and resource-template completion:
...
See `examples/node/communication/completion-client.ts`.
