...
| [Server](#mcp-server) | The MCP Server framework implementation for TypeScript, with MCP Apps, ChatGPT, and MCP-UI support | [Examples](https://github.com/mcp-use/mcp-use/tree/main/examples/server) |
...
It improves the official Model Context Protocol SDK with support for Edge Runtime, **MCP Apps** widgets (inline JSX returns), ChatGPT-compatible metadata, and MCP-UI. It supports all official MCP features and achieves a conformance score of 100/100 based on the official MCP Conformance Tests.
...
- Example widget UI as **plain React components**; tools return **inline JSX** (`/** @jsxImportSource mcp-use/jsx */`) for MCP Apps + ChatGPT.
- Pre-configured server metadata (title, icons, websiteUrl) with your project name.
- Hot Module Reloading (HMR) — modify tools, prompts, resources, and widget components without restarting or dropping connections.
- Automatically launches an MCP Inspector in your browser to test your server.

### Project structure

After creation, your project will look similar to:

```
my-mcp-server/
├── components/                    # React components imported by the server (typical)
├── public/                        # Static assets (icons, images, etc.)
├── index.tsx                      # MCP server entry (TSX when using inline JSX)
├── package.json
├── tsconfig.json
└── README.md
```

Exact files depend on the template; see [Project templates](/typescript/server/templates).
...
- [Core features](/typescript/server/tools): MCP tools, prompts, and resources.
- [MCP Apps](/typescript/server/mcp-apps): Inline JSX widgets (ChatGPT + MCP Apps clients).
- [Migrating from resources/ and widgetMetadata](/typescript/server/migration-from-resources): If you have older `resources/` or `widgetMetadata` code.
- [Configuration](/typescript/server/configuration): Advanced configuration and deployment.
- [Deploy your server](/typescript/server/deployment/mcp-use): Production deployment
