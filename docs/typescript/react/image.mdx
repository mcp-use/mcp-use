...
Any other `img` attributes (`alt`, `className`, `style`, `width`, etc.) pass straight through to the underlying `<img>` element.
...
The component reads these window globals (injected by the MCP server or at build time):
