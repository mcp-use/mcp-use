...
| `children` | `ReactNode` | No | Optional. If provided, this component becomes a scope boundary, and any nested `<ModelContext>` components become children in the tree. |
...
Multiple `<ModelContext>` at the same level in the component tree are siblings, and they produce a flat list at that depth:
...
| Method | Description |
|--------|-------------|
| `modelContext.set(key, content)` | Register or update a named context entry. The `key` is a stable identifier, so calling `set` with the same key overwrites the previous value. |
...
| `<ModelContext>` / `modelContext.set()` | Declarative annotations about what the user is *currently seeing*. Set it and forget it, and the framework handles the rest. |
