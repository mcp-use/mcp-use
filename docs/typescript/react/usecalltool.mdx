...
type State =
  ...
  | { status: "success"; data: CallToolResponse; error: undefined }
...
| `data`          | `CallToolResponse \| undefined`                               | Tool result (only available when `isSuccess`)     |
| `error`         | `unknown`                                                     | Error value (only available when `isError`)       |
...
| ------------ | ------------------------------------------------------------------- | --------------------------------- |
| `onSuccess`  | `(data: CallToolResponse, args: TArgs) => void`                     | Called when tool succeeds         |
| `onError`    | `(error: unknown, args: TArgs) => void`                             | Called when tool fails            |
| `onSettled`  | `(data: CallToolResponse \| undefined, error: unknown \| undefined, args: TArgs) => void` | Called after success or error     |
...
Tool calls return a normalized `CallToolResponse` object:

```typescript
type CallToolResponse = {
  content: Array<{ type: string; text?: string; [key: string]: any }>;

  // Structured content (if the tool returned it)
  structuredContent?: Record<string, unknown>;

  isError?: boolean;

  // Text content joined together as a convenience
  result: string;
...
const FlightSearchWidget = () => {
  ...
  const handleSearch = () => {
    callTool(
      ...
      {
        onSuccess: (result) => {
          console.log(`Found ${result.structuredContent?.flights?.length ?? 0} flights`);
        },
        onError: (err) => {
...
- [useWidget()](/typescript/react/usewidget) - For accessing widget props and context
