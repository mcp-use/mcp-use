...
| `toolInput` | `TToolInput`                                                            | The tool call input arguments that produced this widget                          |
...
| `maxWidth`    | `number \| undefined`               | Maximum width available (pixels), when provided by the host |
| `userAgent`   | `UserAgent`                         | Device capabilities (`device`, `capabilities`) |
| `locale`      | `string`                            | Current locale (e.g., `"en-US"`)               |
| `timeZone`    | `string`                            | Current IANA time zone (e.g., `"America/New_York"`), with a browser fallback |
...
| Property            | Type                 | Description                                                                                                                                 |
| ------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `isAvailable`       | `boolean`            | Whether a widget host bridge is available. This is `true` when connected through MCP Apps, ChatGPT Apps SDK, or the Inspector bridge, and `false` during SSR or plain browser rendering without a host. |
...
| `hostContext`      | `HostContext \| undefined`                    | Full SEP-1865 host context (styles, container dimensions, platform, locale, etc.). `undefined` in ChatGPT Apps SDK. |
...
const ProductWidget: React.FC = () => {
  const {
    ...
    // Availability
    isAvailable,
  } = useWidget<ProductProps, ProductState, ProductOutput, {}>();
...
The hook automatically detects which provider is active:

- **mcp-apps**: Hosted iframe using SEP-1865 postMessage (Claude, Goose, ChatGPT via Apps SDK, and any SEP-1865 host)
- **mcp-ui**: URL parameters fallback (e.g. the Inspector or dev environments); safe defaults are applied when no data is present
...
Height notifications are handled automatically. Wrap your widget in [`McpUseProvider`](./mcpuseprovider) with `autoSize={true}` (the default) and the provider observes your content and reports intrinsic height changes to the host for you, including zero-height collapse when your widget conditionally renders `null`.
