...
When a tool returns an MCP Apps widget (`_meta.ui.resourceUri`, `text/html;profile=mcp-app`), the Inspector renders it below the tool result and exposes widget debug controls. Rendering follows the MCP Apps standard via `@mcp-ui/client`. The Inspector does not emulate ChatGPT or Apps SDK-only widgets.
...
- CSP violations, per-widget RPC logs, and console capture

See [Debug widgets in the Inspector](/inspector/debugging-chatgpt-apps) for the full workflow, including dual-protocol servers and ChatGPT developer-mode verification.
...
<CardGroup cols={2}>
  ...
  <Card title="Debug widgets" icon="bug" href="/inspector/debugging-chatgpt-apps">
    Preview MCP Apps widgets, layout, CSP, and dual-protocol servers.
  </Card>
