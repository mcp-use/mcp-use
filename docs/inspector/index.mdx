...
When a server advertises the experimental `io.modelcontextprotocol/skills` extension, the Inspector adds a **Skills** tab. Each skill appears as a folder containing its `SKILL.md` and supporting resources.
...
Skills are enabled in each new chat by default. One or two skills appear as individual removable chips; larger catalogs use a compact **Skills** chip with an individual picker. Removing a skill excludes its metadata and previous skill-read results from later model requests. The visible transcript is not deleted.

Chat initially exposes only each enabled skill's name, description, origin, and URI. The model can load verified `SKILL.md` instructions and supporting resources only when they become relevant. The Inspector never executes skill scripts or lets `allowed-tools` widen the selected MCP tool permissions.
