...
When you configure `MCPServer` instead of calling `mountInspector` yourself, use `basePath: '/'` for the root: server config validation requires an absolute pathname and rejects `''`. The Inspector treats `''` and `'/'` alike, so both land at `/inspector`.
...
| ------------------------- | ---------------- | ----------------------------------- | ------------------------------------------------------- |
| `basePath`                | `string`         | `'/mcp'`                            | MCP prefix; Inspector mounts at `${basePath}/inspector`, so `/inspector` when it is `''` or `'/'` |
