...
`basePath` defaults to `/mcp`, so the Inspector lives at `/mcp/inspector`. Set `basePath: '/'` for `/inspector`, or use another MCP prefix. An empty string is rejected: `basePath` must be an absolute pathname, so it has to start with `/`.
...
| ------------------------- | ---------------- | ----------------------------------- | ------------------------------------------------------- |
| `basePath`                | `string`         | `'/mcp'`                            | MCP prefix; Inspector mounts under it, at `/inspector` when `basePath` is `'/'` |
