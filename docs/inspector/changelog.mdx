...
<Update label="v20.3.12" description="September 2026">
  ## Mixed authentication release alignment
  Patch release paired with `mcp-use` v2.7.0; rebuilds the bundled Inspector with no Inspector-specific source changes.

- **Update**: rebuild bundled workspace code and synchronize published internal package metadata
- **Update**: Updated mcp-use to v2.7.0
</Update>

<Update label="v20.3.11" description="September 2026">
  ## TanStack Start release alignment
  Patch release paired with `mcp-use` v2.6.0; there are no Inspector-specific source changes.

- **Update**: Updated mcp-use to v2.6.0
</Update>

<Update label="v20.3.10" description="September 2026">
  ## Scrollable MCP App results
  Patch release paired with `mcp-use` v2.5.2, keeping tall tool results accessible.

- **Fix**: tall MCP App tool results scroll within the result panel instead of extending beyond it (`#2598`)
- **Update**: Updated mcp-use to v2.5.2
</Update>

<Update label="v20.3.9" description="September 2026">
  ## Managed chat model branding & provider selection
  Patch release paired with `mcp-use` v2.5.1, showing the selected cloud model accurately and simplifying provider selection.

- **Fix**: managed chat badges use the selected cloud model's name and provider logo instead of the default transport model's branding (`#2575`)
- **Enhancement**: the Manufact cloud provider picker offers OpenAI, Anthropic, and Google; existing selections from other providers remain visible with accurate labels, while evals retain the full catalog and BYOK OpenRouter remains available (`#2576`)
- **Update**: `MCPServerConfig.auth_token` is deprecated but retained for type compatibility; use `authToken` to send an authentication token (`#2486`)
- **Update**: update Hono to `^4.13.5` (`#2492`)
- **Update**: Updated mcp-use to v2.5.1
</Update>

<Update label="v20.3.8" description="September 2026">
  ## Bundled Inspector rebuild
  Patch release paired with `mcp-use` v2.5.0, rebuilding the standalone Inspector with the agent OpenRouter fix.

- **Fix**: rebuild the standalone Inspector with the OpenRouter Responses API agent fix and repair published framework peer metadata (`#2504`)
- **Update**: Updated mcp-use to v2.5.0
</Update>

<Update label="v20.3.7" description="September 2026">
  ## Workspace package alignment
  Patch release paired with `mcp-use` v2.4.3; there are no Inspector-specific source changes.

- **Update**: Updated mcp-use to v2.4.3
</Update>

<Update label="v20.3.6" description="September 2026">
  ## Publish artifact preservation
  Patch release paired with `mcp-use` v2.4.2, keeping package builds intact while npm pack runs.
...
- **Update**: Updated mcp-use to v2.4.2
</Update>

<Update label="v20.3.5" description="September 2026">
  ## Chat widgets use the connection id
  Patch release paired with `mcp-use` v2.4.1, rendering chat MCP Apps against the registered connection.

- **Fix**: chat MCP Apps render with the registered MCP connection id while preserving the logical server id for chat state and storage (`#2404`)
- **Update**: Updated mcp-use to v2.4.1
</Update>

<Update label="v20.3.4" description="September 2026">
  ## Request duration & screenshot init failures
  Patch release paired with `mcp-use` v2.4.0, showing RPC duration, failing screenshots when an MCP App fails to initialize, and raising the embedded chat step cap.

- **Enhancement**: RPC traffic rows show request duration in milliseconds instead of wall-clock time; unmatched rows omit the timing column (`#2364`)
- **Fix**: `mcp-use screenshot` fails with `view_load_failed` when the MCP App fails to initialize (`#2355`)
- **Enhancement**: embedded client-side chats can configure up to 100 agent steps (`#2401`)
- **Update**: Updated mcp-use to v2.4.0
</Update>

<Update label="v20.3.3" description="August 2026">
  ## RPC latency in the timing column
  Patch release paired with `mcp-use` v2.3.4, moving response latency into the traffic timing column and labeling colocated Inspector logs.

- **Enhancement**: response rows show latency in the traffic timing column instead of repeating it in the method label; unmatched rows still show the wall-clock time (`#2349`)
- **Enhancement**: when Inspector shares the `mcp-use dev` process, proxy and OAuth BFF logs are prefixed `[inspector]` (`#2351`)
- **Update**: Updated mcp-use to v2.3.4
</Update>

<Update label="v20.3.2" description="August 2026">
  ## Model tool-limit warnings
  Patch release paired with `mcp-use` v2.3.3, making model tool limits visible before a chat request is sent.

- **Enhancement**: chat warns when a connected MCP server exposes more than the documented 128 tools supported by OpenAI or Gemini models (`#2333`)
- **Update**: Updated mcp-use to v2.3.3
</Update>

<Update label="v20.3.1" description="August 2026">
  ## Correlated JSON-RPC responses
  Patch release paired with `mcp-use` v2.3.2, adding request context and timing to JSON-RPC response rows.

- **Enhancement**: response rows show the matched request method and elapsed time and flag errors while keeping simultaneous bidirectional requests isolated by source, server, widget, direction, and ID type (`#2303`)
- **Update**: Updated mcp-use to v2.3.2
</Update>

<Update label="v20.3.0" description="August 2026">
  <Frame>
    <img src="https://mcp-use.com/api/og/release?v=20.3.0&title=Reusable%20connection%20settings&date=2026-08&lang=inspector" alt="Release 20.3.0" />
  </Frame>
  ## Reusable connection settings
  Minor release paired with `mcp-use` v2.3.1, exposing the Inspector connection form and configuration state for embedded consumers.

- **New**: `@mcp-use/inspector/client` exports `ConnectionSettingsForm`, `useConnectionFormState`, custom-header controls, connection configuration types, and normalization helpers, with optional consumer URL validation (`#2319`)
- **Update**: Updated mcp-use to v2.3.1
</Update>
...
<Update label="v12.0.6" description="July 2026">
  ## CLI version heading
  Patch release paired with `mcp-use` v1.34.6; includes the unpublished 12.0.4 docs-link fix.

- **Enhancement**: the CLI help heading shows the installed Inspector version
- **Documentation**: Inspector README drops dead documentation links
- **Update**: Updated mcp-use to v1.34.6
</Update>
