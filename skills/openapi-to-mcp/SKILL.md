...
- The user wants a **widget-driven MCP App** where most/every tool renders a custom UI in the chat → use `mcp-apps-builder` instead. This skill can layer a widget on one or two specific tools, but if widgets are the whole point, the other skill is the right starting frame. Confirm with the user during step 1.
