...
def sample_manifest():
    ...
    return {
        ...
        "user_config": {
            "api_key": {"type": "string", "title": "API Key", "description": "Your API key", "required": True}
        },
    }
...
class TestDXTParser:
    ...
    def test_missing_manifest(self, create_dxt_file):
        ...
        with pytest.raises(DXTManifestError, match="manifest.json not found"):
            with DXTParser(str(dxt_path)) as _:
                pass
...
class TestDXTFunctions:
    ...
    def test_load_dxt_config_no_user_config_required(self, create_dxt_file):
        ...
        manifest = {
            ...
            "author": {"name": "Test Author"},
            "server": {"type": "node", "mcp_config": {"command": "node", "args": ["${__dirname}/server/index.js"]}},
        }
        ...
        manifest = {
            ...
            "server": {
                ...
                "mcp_config": {
                    "command": "node",
                    "args": ["${__dirname}/server/index.js"],
                    "env": {"HOME_DIR": "${HOME}", "TEMP_DIR": "${TMPDIR}"},
                },
            },
        }
